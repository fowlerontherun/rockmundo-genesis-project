-- Production parity overlay for simplified Festival automatic sponsorship and budget forecasting.
...
BEGIN
  ...
  IF NOT FOUND OR r.state <> 'completed' THEN RAISE EXCEPTION 'festival_runtime_not_complete' USING ERRCODE='P0001'; END IF;
  SELECT * INTO d FROM public.festival_runtime_completion_digests WHERE runtime_id=r.id;
  IF NOT FOUND THEN RAISE EXCEPTION 'festival_runtime_digest_missing' USING ERRCODE='P0001'; END IF;
  ...
  SELECT id INTO existing_id FROM public.festival_simplified_edition_results WHERE festival_edition_id=e.id;
  ...
  FROM public.festival_artist_programmes ap
  JOIN public.festival_artist_bookings bk ON bk.festival_artist_programme_id=ap.id
  WHERE ap.festival_edition_id=e.id AND bk.status IN ('confirmed','awaiting_schedule','scheduled');
