...
BEGIN
 ...
 IF has_function_privilege('authenticated','public.claim_festival_performance_simulation_job(text)','EXECUTE') OR has_function_privilege('authenticated','public.complete_festival_performance_simulation_job(uuid,text,text,jsonb)','EXECUTE') THEN RAISE EXCEPTION 'browser can execute canonical simulation worker'; END IF;
 IF NOT EXISTS(SELECT 1 FROM pg_indexes WHERE schemaname='public' AND tablename='festival_crowd_recalculation_requests' AND indexdef LIKE '%UNIQUE%runtime_session_id, runtime_day_id, idempotency_key%') THEN RAISE EXCEPTION 'crowd idempotency scope missing'; END IF;
 IF NOT EXISTS(SELECT 1 FROM pg_indexes WHERE schemaname='public' AND tablename='festival_performance_simulation_jobs' AND indexdef LIKE '%UNIQUE%runtime_performance_id%') THEN RAISE EXCEPTION 'one simulation per performance invariant missing'; END IF;
 IF NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgrelid='public.festival_performance_simulation_results'::regclass AND tgname='festival_performance_result_immutable' AND tgenabled<>'D') THEN RAISE EXCEPTION 'performance result immutability missing'; END IF;
 IF NOT EXISTS(SELECT 1 FROM pg_trigger WHERE tgrelid='public.festival_runtime_outcome_snapshots'::regclass AND tgname='festival_runtime_outcome_snapshot_immutable' AND tgenabled<>'D') THEN RAISE EXCEPTION 'outcome snapshot immutability missing'; END IF;
