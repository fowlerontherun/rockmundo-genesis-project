...
BEGIN
 ...
 IF position('FOR UPDATE' IN definition)=0 OR position('active_lease_busy' IN definition)=0 OR position('festival_lifecycle_idempotency_conflict' IN definition)=0 THEN
 ...
 IF position('TG_OP' IN definition)=0 OR position('festival_payment_resolution_evidence_required' IN definition)=0 THEN RAISE EXCEPTION 'terminal trigger is unsafe or unaudited'; END IF;
