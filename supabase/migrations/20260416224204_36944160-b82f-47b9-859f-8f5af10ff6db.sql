-- Fix tax calculation to exclude owner deposits, transfers, and investments.
-- Only actual earned revenue (gig income, sales, fees, royalties, etc.) should be taxed.
...
BEGIN
  ...
  LOOP
    ...
    -- Revenue: only EARNED income, NOT owner deposits/transfers/investments/dividends
    SELECT
      COALESCE(SUM(CASE
        WHEN ct.amount > 0
         AND COALESCE(ct.transaction_type, '') NOT IN ('investment','transfer_in','transfer_out','dividend','owner_deposit','capital_injection','loan')
         AND COALESCE(ct.category, '') NOT IN ('owner_transfer','owner_deposit','capital_injection','loan','transfer','intercompany')
        THEN ct.amount END), 0),
      COALESCE(SUM(CASE
        WHEN ct.amount < 0
         AND COALESCE(ct.category, '') <> 'tax'
         AND COALESCE(ct.transaction_type, '') NOT IN ('transfer_out','dividend')
        THEN -ct.amount END), 0)
...
-- Void the previously-generated inflated tax bills (they were based on owner deposits)
DELETE FROM public.company_tax_records
 WHERE status = 'pending'
   AND created_at >= now() - interval '24 hours';

-- Regenerate with the corrected logic
