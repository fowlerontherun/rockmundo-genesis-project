...
BEGIN
 ...
 UPDATE public.festival_edition_settlement_effects SET status='applying',claim_token=token,worker_identity=nullif(btrim(p_worker_identity),''),lease_expires_at=now()+make_interval(secs=>least(greatest(p_lease_seconds,15),300)),attempt_count=attempt_count+1,expected_settlement_version=(SELECT settlement_version FROM public.festival_edition_settlements WHERE id=sid),canonical_handler=CASE effect_type WHEN 'performance_result' THEN 'apply_festival_performance_result_effect' WHEN 'band_fans' THEN 'apply_festival_band_fans_effect' WHEN 'band_fame' THEN 'apply_festival_band_fame_effect' WHEN 'member_xp' THEN 'apply_festival_member_xp_effect' WHEN 'band_chemistry' THEN 'apply_festival_band_chemistry_effect' WHEN 'song_familiarity' THEN 'apply_festival_song_familiarity_effect' WHEN 'song_popularity' THEN 'apply_festival_song_popularity_effect' WHEN 'festival_company_reputation' THEN 'apply_festival_company_reputation_effect' WHEN 'festival_company_fame' THEN 'apply_festival_company_fame_effect' WHEN 'artist_relationship' THEN 'apply_festival_artist_relationship_effect' WHEN 'sponsor_relationship' THEN 'apply_festival_sponsor_relationship_effect' WHEN 'achievement_award' THEN 'apply_festival_achievement_effect' WHEN 'licence_progress' THEN 'apply_festival_licence_progress_effect' WHEN 'world_event' THEN 'apply_festival_world_event_effect' WHEN 'notification' THEN 'apply_festival_notification_effect' WHEN 'tax_projection' THEN 'apply_festival_tax_projection_effect' END WHERE id=e.id RETURNING * INTO e;
 RETURN to_jsonb(e);
END $$;
