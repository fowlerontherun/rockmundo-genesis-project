...
-- Seed canonical TOTP achievements into the existing achievement catalogue.
-- Achievement rewards are recognition only: TOTP fame is settled separately below.
WITH rows(slug,name,description,tier,display_order,icon_key,points) AS (VALUES
  ('totp-tv-debut','TV Debut','Perform on Top of the Pops for the first time.','bronze'::public.achievement_tier,610,'tv',10),
  ('totp-top-20-performer','Top 20 Performer','Perform on Top of the Pops with a song in the UK Top 20.','silver'::public.achievement_tier,620,'radio',15),
  ('totp-top-10-performer','Top 10 Performer','Perform on Top of the Pops with a song in the UK Top 10.','gold'::public.achievement_tier,630,'trophy',20),
  ('totp-number-one','Top of the Pops','Perform on Top of the Pops while your song is UK number one.','platinum'::public.achievement_tier,640,'crown',30),
  ('totp-household-name','Household Name','Make 5 Top of the Pops appearances.','silver'::public.achievement_tier,650,'star',20),
  ('totp-television-regular','Television Regular','Make 10 Top of the Pops appearances.','gold'::public.achievement_tier,660,'tv',30),
  ('totp-pop-institution','Pop Institution','Make 25 Top of the Pops appearances.','platinum'::public.achievement_tier,670,'award',45),
  ('totp-legend-of-the-pops','Legend of the Pops','Make 50 Top of the Pops appearances.','legendary'::public.achievement_tier,680,'crown',75)
)
INSERT INTO public.achievements (
  slug,name,description,category,tier,achievement_type,is_active,is_hidden,is_repeatable,
  display_order,icon_key,points,balance_version,requirements,rewards
)
SELECT
  r.slug,r.name,r.description,'performance',r.tier,'career_milestone'::public.achievement_type,
  true,false,false,r.display_order,r.icon_key,r.points,1,
  jsonb_build_object('source','top_of_the_pops'),
  '{}'::jsonb
FROM rows r
...
-- Cosmetic/profile badge recognition for each TOTP achievement.
INSERT INTO public.achievement_rewards (achievement_id,reward_type,reward_key,amount,settlement_policy,metadata)
SELECT a.id,'badge',a.slug,1,'automatic',jsonb_build_object('source','top_of_the_pops')
FROM public.achievements a
WHERE a.slug IN (
  'totp-tv-debut','totp-top-20-performer','totp-top-10-performer','totp-number-one',
  'totp-household-name','totp-television-regular','totp-pop-institution','totp-legend-of-the-pops'
)
AND NOT EXISTS (
  SELECT 1 FROM public.achievement_rewards ar
  WHERE ar.achievement_id = a.id AND ar.reward_type = 'badge' AND ar.reward_key = a.slug
);
...
BEGIN
  ...
  -- Idempotency guard: once history exists, completion is settled forever.
  ...
  IF v_history_id IS NOT NULL THEN
    RETURN (
      SELECT jsonb_build_object(
        ...
        'performance_id',h.performance_id,
        'appearance_number',h.appearance_number,
        'fame_awarded',h.fame_awarded,
  ...
  FROM public.totp_appearance_history
  WHERE band_id = v_perf.band_id;
  v_appearance_number := v_prior_appearances + 1;

  v_raw_fame := public.totp_raw_fame_for_rank(v_inv.qualifying_rank);
  -- First appearance gets a modest debut lift. Repeated appearances retain value but
  -- gradually diminish to a 60% floor so TOTP cannot become a farmable fame loop.
  IF v_appearance_number = 1 THEN
    v_raw_fame := round(v_raw_fame * 1.20);
  END IF;
  v_lifetime_factor := greatest(0.60, 1.0 / (1.0 + (v_prior_appearances * 0.04)));
  -- Match the game's existing collective-fame long-tail progression curve.
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'episode_id',v_perf.episode_id,
      'episode_number',v_episode.episode_number,
      'qualifying_rank',v_inv.qualifying_rank,
      'appearance_number',v_appearance_number,
      'raw_fame_gain',v_raw_fame,
      'lifetime_factor',v_lifetime_factor,
      'progression_factor',v_progression_factor
    )
  );

  -- Player fame is deliberately smaller than the collective band reward. Touring/NPC
  -- members are excluded, matching the main band-fame distribution model.
  FOR v_member IN
    SELECT bm.profile_id,bm.vocal_role,p.user_id
  ...
  LOOP
    ...
    IF lower(coalesce(v_member.vocal_role,'')) LIKE '%lead%' THEN
      v_member_share := round(v_member_share * 1.20);
    END IF;
    IF v_member.profile_id = v_band.leader_id THEN
      v_member_share := round(v_member_share * 1.15);
    END IF;
    UPDATE public.profiles
    SET fame = coalesce(fame,0) + greatest(1,v_member_share)
  ...
  -- One authoritative event per performance is reused as the source id for all
  -- non-repeatable TOTP achievements, preserving duplicate-completion protection.
  INSERT INTO public.achievement_events(profile_id,band_id,event_type,source_table,source_id,payload,is_authoritative)
  VALUES (NULL,v_perf.band_id,'top_of_the_pops_performance','totp_performances',v_perf.id,
    jsonb_build_object('rank',v_inv.qualifying_rank,'appearance_number',v_appearance_number,'fame_awarded',v_effective_fame),true)
  ON CONFLICT (event_type,source_id) DO UPDATE
  SET payload = EXCLUDED.payload
  RETURNING id INTO v_achievement_event_id;
  ...
  FOR v_member IN
    ...
    WHERE bm.band_id = v_perf.band_id
      ...
      AND coalesce(bm.is_touring_member,false) = false
  LOOP
    PERFORM public.complete_achievement_for_event(v_member.profile_id,'totp-tv-debut',v_achievement_event_id,'live');
    IF v_inv.qualifying_rank <= 20 THEN
      PERFORM public.complete_achievement_for_event(v_member.profile_id,'totp-top-20-performer',v_achievement_event_id,'live');
    END IF;
    IF v_inv.qualifying_rank <= 10 THEN
      PERFORM public.complete_achievement_for_event(v_member.profile_id,'totp-top-10-performer',v_achievement_event_id,'live');
    END IF;
    IF v_inv.qualifying_rank = 1 THEN
      PERFORM public.complete_achievement_for_event(v_member.profile_id,'totp-number-one',v_achievement_event_id,'live');
    END IF;
    IF v_appearance_number >= 5 THEN
      PERFORM public.complete_achievement_for_event(v_member.profile_id,'totp-household-name',v_achievement_event_id,'live');
    END IF;
    IF v_appearance_number >= 10 THEN
      PERFORM public.complete_achievement_for_event(v_member.profile_id,'totp-television-regular',v_achievement_event_id,'live');
    END IF;
    IF v_appearance_number >= 25 THEN
      PERFORM public.complete_achievement_for_event(v_member.profile_id,'totp-pop-institution',v_achievement_event_id,'live');
    END IF;
    IF v_appearance_number >= 50 THEN
      PERFORM public.complete_achievement_for_event(v_member.profile_id,'totp-legend-of-the-pops',v_achievement_event_id,'live');
    END IF;
  END LOOP;

  -- Completion notification is informational only; rewards were already settled above.
  INSERT INTO public.notifications(user_id,type,message)
  SELECT DISTINCT bm.user_id,'system',
...
CREATE OR REPLACE FUNCTION public.totp_public_history(
...
AS $$
  ...
  FROM (
    SELECT
      ...
      h.completed_at,
      jsonb_build_object(
        'episode_date',h.episode_date,
        'episode_number',h.episode_number,
        'performance_id',h.performance_id,
        'band_id',h.band_id,
        'band_name',b.name::text,
        'song_id',h.song_id,
        'song_title',s.title,
        'qualifying_rank',h.qualifying_rank,
        'running_order',h.running_order,
        'stage_key',h.stage_key,
        'appearance_number',h.appearance_number,
        'fame_awarded',h.fame_awarded,
        'presenter_intro',h.presenter_intro,
...
AS $$
  SELECT jsonb_build_object(
    'band_id',p_band_id,
    'appearances',count(*),
    'best_chart_rank',min(qualifying_rank),
    ...
    'first_appearance',min(episode_date),
    'latest_appearance',max(episode_date)
  )
  FROM public.totp_appearance_history
...
COMMENT ON TABLE public.totp_appearance_history IS 'Permanent, immutable history of completed Top of the Pops appearances and one-time fame settlement.';
