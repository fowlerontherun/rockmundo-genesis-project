-- Fix auto_complete_songwriting_sessions to use locked_until instead of session_end
...
BEGIN
  ...
  FOR v_project IN
    ...
    WHERE p.status IN ('draft', 'in_progress', 'writing')
      AND COALESCE(p.music_progress, 0) >= 2000
      AND COALESCE(p.lyrics_progress, 0) >= 2000
      AND NOT EXISTS (
