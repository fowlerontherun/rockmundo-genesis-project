-- Repair scheduling schema drift discovered after the August mobile practice pass.
--
-- Several historical migrations replace player_scheduled_activities_activity_type_check
-- with smaller whitelists. The July 2026 social-activities migration accidentally
-- removed skill_practice and several existing activity types, while the wellness
-- runtime now emits wellness_recovery and wellness_medical. Rebuild the constraint
-- as the union of the known live activity families, then restate the repaired
-- practice RPCs so this migration can be used as the production repair point.
