-- Fix auto_complete_songwriting_sessions function to use completed_at instead of non-existent status column
...
BEGIN
  -- Complete expired in-progress sessions (sessions where completed_at is NULL and scheduled_end has passed)
  FOR v_session IN
    SELECT 
      s.*,
      p.user_id,
  ...
  LOOP
    ...
    -- Update session with completed_at timestamp instead of status
