...
RETURNS TABLE(band_id uuid, band_name text, member_role text, member_status text, can_apply boolean, can_negotiate boolean, can_sign boolean, can_manage_bookings boolean, active_application_count bigint, active_festival_contract_count bigint)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT b.id, b.name::text, bm.role::text, coalesce(bm.member_status,'active')::text,
  ...
  WHERE bm.user_id = auth.uid() AND coalesce(bm.member_status,'active')='active'
...
DECLARE
  ...
  authority boolean; status_ok boolean; window_ok boolean;
  reasons text[] := '{}'; warnings text[] := '{}'; hard text[] := '{}'; advisory text[] := '{}';
  outcome text;
BEGIN
  ...
  RETURN jsonb_build_object('outcome',outcome,'can_apply',outcome<>'blocked','authority_allowed',authority,'edition_status_allowed',status_ok,'application_window_open',window_ok,'existing_application_id',app.id,'existing_application_status',app.status,'existing_contract_id',c.id,'hard_conflicts',hard,'advisory_conflicts',advisory,'requested_duration_limits',jsonb_build_object('minimum_minutes',5,'maximum_minutes',240),'currency_code',e.currency_code,'available_slot_types',COALESCE((SELECT jsonb_agg(DISTINCT ss.slot_type ORDER BY ss.slot_type) FROM public.festival_stage_slots ss WHERE ss.festival_id=e.festival_id AND ss.status='open'),'[]'::jsonb),'application_deadline',e.application_close_at,'reasons',reasons,'warnings',warnings);
END $$;
...
DECLARE
  c public.festival_contracts%ROWTYPE;
  total integer := 0; maxsec integer := 0; minsec integer := 60;
  invalid text[] := '{}'; duplicates text[] := '{}'; unavailable text[] := '{}';
  warnings text[] := '{}'; blockers text[] := '{}';
  seen uuid[] := '{}'; item jsonb; sid uuid;
BEGIN
