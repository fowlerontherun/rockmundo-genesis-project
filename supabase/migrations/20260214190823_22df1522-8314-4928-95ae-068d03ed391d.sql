...
DECLARE
    ...
    v_masterpiece_roll numeric;
    v_masterpiece_chance numeric;
    v_total_skill numeric;
BEGIN
    FOR v_session IN
        SELECT 
            ss.id as session_id,
    ...
    LOOP
        ...
        -- MASTERPIECE ROLL: rare chance for near-max players to write a legendary song
        -- Requires high combined skill (composing + lyrics >= 30) to even have a chance
        -- Chance scales from 0% at skill 30 to ~2% at skill 40 (both maxed at 20)
        -- A masterpiece bypasses the soft cap, landing in the 800-950 range
        v_total_skill := v_composing_skill + v_lyrics_skill;
        v_masterpiece_chance := GREATEST(0, (v_total_skill - 30) * 0.002);
        v_masterpiece_roll := RANDOM();
        
        IF v_masterpiece_roll < v_masterpiece_chance AND v_total_skill >= 30 THEN
            -- Masterpiece! Quality 800-950 (needs perfect recording to reach 1000)
            v_raw_quality := 800 + (RANDOM() * 150)::numeric;
            v_diminishing_quality := v_raw_quality; -- bypass soft cap
        ELSIF v_raw_quality <= 500 THEN
        ...
        SET 
            ...
            auto_completed = true,
            notes = CASE 
                WHEN v_masterpiece_roll < v_masterpiece_chance AND v_total_skill >= 30 THEN '⚡ MASTERPIECE! Lightning in a bottle — a once-in-a-lifetime composition!'
