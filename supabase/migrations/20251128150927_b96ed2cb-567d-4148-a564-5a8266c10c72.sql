-- Fix auto_complete_songwriting_sessions function to use correct column names
...
BEGIN
  -- Complete expired in-progress sessions (where completed_at is NULL and session_end has passed)
  FOR v_session IN
    SELECT 
      s.*,
      p.user_id,
  ...
  LOOP
    ...
    v_session_hours := EXTRACT(EPOCH FROM (v_session.session_end - v_session.session_start)) / 3600.0;
    ...
    -- Update session with completed_at timestamp
    UPDATE songwriting_sessions
    SET 
      completed_at = NOW(),
      xp_earned = FLOOR(10 + v_session_quality * 0.5)::integer,
      auto_completed = true,
      music_progress_gained = FLOOR(v_session_quality * 0.5)::integer,
      lyrics_progress_gained = FLOOR(v_session_quality * 0.5)::integer
