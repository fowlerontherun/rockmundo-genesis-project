-- Phase 0 broadcast contract: persist explicit per-track external rights and
-- immutable/versioned presenter audio metadata alongside the editorial episode plan.
-- These fields are then copied into the frozen episode manifest.
...
COMMENT ON COLUMN public.totp_episode_plans.presenter_audio IS
  'Versioned recorded presenter assets keyed by performance UUID, including script checksum, media SHA-256 and duration.';
...
BEGIN
  ...
  IF jsonb_typeof(v_presenter_audio) <> 'object' THEN
    RAISE EXCEPTION 'presenter_audio must be a JSON object keyed by performance id';
  END IF;
