...
declare
  ...
  v_filter text := lower(coalesce(p_activity_filter, 'all'));
begin
  ...
  if v_filter not in ('all', 'streams', 'sales', 'digital', 'vinyl', 'cd', 'cassette') then
    raise exception 'Invalid activity filter';
  end if;
  ...
  sales as (
    ...
    where v_filter <> 'streams'
      and (v_filter in ('all', 'sales') or lower(rf.format_type::text) = v_filter)
  ...
  return coalesce(v_result, jsonb_build_object('period_kind', v_period, 'activity_filter', v_filter, 'rows', '[]'::jsonb, 'release_totals', '[]'::jsonb));
end;
$$;

revoke all on function public.get_release_manager_city_analytics(uuid, uuid, text, text) from public;
grant execute on function public.get_release_manager_city_analytics(uuid, uuid, text, text) to authenticated;
