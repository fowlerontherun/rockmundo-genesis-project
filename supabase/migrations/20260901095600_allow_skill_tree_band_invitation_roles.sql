-- Band performance role options are sourced from src/data/skillTree.ts in the client.
-- Do not maintain a second, narrower SQL whitelist here: it previously rejected
-- valid roles such as specific guitars, percussion, brass, strings and electronic instruments.
...
BEGIN
  ...
  normalized_instrument := NULLIF(btrim(COALESCE(requested_instrument_role, '')), '');
  IF normalized_instrument IS NULL THEN
    RAISE EXCEPTION 'Choose a performance role.' USING ERRCODE = '22023';
  END IF;
