-- Festival licence operating limits are use-time ceilings, just like attendance capacity.
-- Permanent infrastructure may be built ahead, but an annual Festival only materialises
-- the number of active stages permitted by the current licence and Run Festival rechecks
-- stage/act ceilings authoritatively.
...
-- Patch the active materialiser rather than copying its large implementation. This keeps
-- the inherited canonical function and the compact production-reconciliation variant aligned.
DO $$
DECLARE
  definition text;
...
-- Keep the deployed readiness implementation as the base and add a final operating-ceiling
-- layer. This is additive so a reconciliation variant cannot silently lose these checks.
DO $$
BEGIN
...
-- Re-materialise editable annual Festivals so active stages immediately respect the licence.
-- Ticket owner-confirmation state is retained because hidden materialisation does not satisfy
-- the owner-confirmation trigger boundary.
DO $$
DECLARE
  row record;
BEGIN
