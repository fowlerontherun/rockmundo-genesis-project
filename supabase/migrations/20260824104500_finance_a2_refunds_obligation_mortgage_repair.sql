-- Finance backlog A2: replay-safe booking refunds, obligation retry hardening,
-- and mortgage schedule-version synchronisation.
...
BEGIN
  ...
  IF v_refund.id IS NOT NULL THEN
    RETURN jsonb_build_object(
      ...
      'paymentSource', v_refund.payment_source
    );
  END IF;
...
BEGIN
  ...
  SELECT * INTO v_booking FROM public.band_rehearsals
  WHERE id = p_rehearsal_id FOR UPDATE;
  ...
  RETURN jsonb_build_object(
    'idempotent', v_booking.status = 'cancelled',
    'bookingId', p_rehearsal_id,
    'refund', v_refund
  );
END;
$$;
...
DECLARE
  v_profile_id uuid := public._caller_profile_id();
  v_booking public.recording_sessions;
  v_payment public.booking_payments;
  v_refund jsonb;
BEGIN
  IF v_profile_id IS NULL THEN RAISE EXCEPTION 'active_profile_required'; END IF;

  SELECT * INTO v_booking FROM public.recording_sessions
  WHERE id = p_recording_id FOR UPDATE;
  IF v_booking.id IS NULL THEN RAISE EXCEPTION 'recording_not_found'; END IF;

  SELECT * INTO v_payment FROM public.booking_payments
  WHERE booking_type = 'recording' AND booking_id = p_recording_id FOR UPDATE;
  IF v_payment.id IS NULL THEN RAISE EXCEPTION 'booking_payment_not_found'; END IF;

  IF v_payment.band_id IS NOT NULL THEN
    IF NOT public._band_active_member(v_payment.band_id, v_profile_id) THEN RAISE EXCEPTION 'not_band_member'; END IF;
  ELSIF v_payment.profile_id <> v_profile_id THEN
    RAISE EXCEPTION 'recording_not_owned_by_caller';
  END IF;

  IF v_booking.status NOT IN ('scheduled', 'cancelled') THEN RAISE EXCEPTION 'recording_not_cancellable'; END IF;

  v_refund := public._refund_atomic_booking_payment(v_payment.id, p_reason, p_idempotency_key);

  UPDATE public.recording_sessions SET status = 'cancelled' WHERE id = p_recording_id;
  UPDATE public.player_scheduled_activities
  SET status = 'cancelled', updated_at = now()
  WHERE linked_recording_id = p_recording_id AND status NOT IN ('completed', 'cancelled');

  RETURN jsonb_build_object(
    'idempotent', v_booking.status = 'cancelled',
    'bookingId', p_recording_id,
    'refund', v_refund
  );
END;
$$;
...
-- ---------------------------------------------------------------------------
-- Obligation idempotency and retry policy
-- ---------------------------------------------------------------------------

WITH ranked AS (
  SELECT id,
         row_number() OVER (PARTITION BY schedule_id ORDER BY attempted_at, id) AS corrected_attempt_number
  FROM public.financial_obligation_attempts
)
UPDATE public.financial_obligation_attempts a
SET attempt_number = r.corrected_attempt_number
FROM ranked r
WHERE a.id = r.id
  AND a.attempt_number IS DISTINCT FROM r.corrected_attempt_number;
...
BEGIN
  ...
  v_result := public.process_financial_obligation_payment_internal(
    p_schedule_id, p_idempotency_key, p_as_of_date, p_as_of_timestamp
  );

  IF v_result->>'status' = 'failed' THEN
    SELECT count(a.id), o.max_attempts
    ...
    IF COALESCE(v_attempt_count, 0) >= COALESCE(v_max_attempts, 1) THEN
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM public.financial_obligation_schedule s
    JOIN public.financial_obligations o ON o.id = s.obligation_id
    WHERE s.id = p_schedule_id
...
BEGIN
  ...
  FOR r IN
    ...
    WHERE o.status IN ('active', 'grace_period', 'retrying', 'failed')
      ...
      AND (SELECT count(*) FROM public.financial_obligation_attempts a WHERE a.schedule_id = s.id) < o.max_attempts
      AND NOT EXISTS (
  ...
  LOOP
    ...
    BEGIN
      ...
      IF v_result->>'status' IN ('posted', 'already_paid') THEN v_posted := v_posted + 1;
      ELSIF v_result->>'status' IN ('retry_not_due', 'already_attempted') THEN v_skipped := v_skipped + 1;
...
DECLARE
  ...
  v_missed integer;
  v_next_due date;
  v_status public.financial_obligation_status;
BEGIN
  SELECT
    COALESCE(sum(
      CASE
  ...
  FROM public.financial_obligation_schedule s
  LEFT JOIN public.debt_records d ON d.schedule_id = s.id AND d.status = 'open'
  ...
  ) THEN v_status := 'collections';
  ELSIF v_next_due IS NULL THEN v_status := 'completed';
  ELSIF EXISTS (
  ...
  ) THEN v_status := 'retrying';
  ELSIF v_outstanding > 0 THEN v_status := 'failed';
  ...
  SET outstanding_balance_minor = v_outstanding,
      ...
      status = CASE WHEN status IN ('paused', 'cancelled', 'written_off') THEN status ELSE v_status END,
      updated_at = timezone('utc', now()),
      completed_at = CASE WHEN v_status = 'completed' THEN COALESCE(completed_at, timezone('utc', now())) ELSE completed_at END
...
BEGIN
  ...
  FOR r IN
    SELECT * FROM public.mortgage_schedule_lines
    WHERE mortgage_contract_id = p_mortgage_contract_id
  ...
  LOOP
    INSERT INTO public.financial_obligation_schedule (
      ...
      r.id,
      v_schedule_version,
      concat_ws(':', v_obligation_id::text, 'mortgage_schedule_line', r.id::text, v_schedule_version::text),
      concat_ws(':', 'mortgage', p_mortgage_contract_id::text, v_schedule_version::text, r.instalment_number::text),
  ...
  WHERE s.obligation_id = v_obligation_id
    ...
    AND NOT EXISTS (
      SELECT 1 FROM public.mortgage_schedule_lines m
      WHERE m.mortgage_contract_id = p_mortgage_contract_id
        AND m.schedule_version = v_schedule_version
...
REVOKE ALL ON FUNCTION public.sync_mortgage_financial_obligation_schedule(uuid) FROM PUBLIC, anon, authenticated;
...
DECLARE
  v_contract_id uuid;
BEGIN
  IF TG_OP = 'DELETE' THEN v_contract_id := OLD.mortgage_contract_id;
  ELSE v_contract_id := NEW.mortgage_contract_id;
  END IF;
