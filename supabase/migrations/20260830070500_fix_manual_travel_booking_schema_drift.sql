-- Production was repaired directly first on 2026-08-30.
--
-- Manual travel had drifted from the current schema in two ways:
-- 1. the authoritative travel RPC depended on city_development before that table
--    existed in production;
-- 2. a live RPC revision attempted to INSERT player_scheduled_activities.duration_minutes,
--    but duration_minutes is now a GENERATED ALWAYS column.
--
-- city_development is supplied by the later city-development schema. This migration
-- re-establishes the authoritative RPC against the current scheduled-activity schema
-- and deliberately lets Postgres derive duration_minutes from start/end timestamps.
...
BEGIN
  ...
  INSERT INTO public.player_travel_history(
    ...
    p_transport_type,
    v_total_cost,
    greatest(1, ceil(v_adjusted_duration)::integer),
    v_departure,
    v_departure,
    v_arrival,
