-- Deploy the Festival Management owner bootstrap RPC required by the frontend.
-- This intentionally backports the idempotent 20291216080000 repair migration under
-- a deployable 2026 timestamp so production migration runners pick it up promptly.
...
BEGIN
  ...
  WHERE e.festival_id=v_festival.id AND (v_resolution.edition_id IS NULL OR e.id=v_resolution.edition_id)
    AND (v_is_admin OR v_is_owner OR EXISTS (SELECT 1 FROM public.festival_edition_management_roles r WHERE r.edition_id=e.id AND r.profile_id=v_profile_id AND r.status='active' AND (r.ends_at IS NULL OR r.ends_at>now())))
...
REVOKE ALL ON FUNCTION public.resolve_festival_management_identifier(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.festival_owner_management_bootstrap(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.resolve_festival_management_identifier(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.festival_owner_management_bootstrap(uuid) TO authenticated;
