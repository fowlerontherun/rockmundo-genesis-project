...
CREATE OR REPLACE FUNCTION public.start_songwriting_session(p_profile_id uuid, p_project_id uuid, p_effort_hours integer DEFAULT 1, p_session_type text DEFAULT 'balanced'::text, p_idempotency_key text DEFAULT NULL::text, p_activity_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_project.status IN ('completed','converted') OR v_project.song_id IS NOT NULL THEN RAISE EXCEPTION 'This songwriting project is already complete.' USING ERRCODE='P0001'; END IF;
  ...
  IF p_activity_id IS NULL THEN
    ...
    VALUES (v_user, p_profile_id, 'songwriting', v_start, v_end, 'in_progress', 'Songwriting: ' || COALESCE(v_project.title,'Untitled'), 'Working on song composition', jsonb_build_object('project_id', p_project_id, 'session_id', v_session_id, 'duration_hours', p_effort_hours, 'session_type', p_session_type)) RETURNING id INTO v_activity_id;
  ELSE
    v_activity_id := p_activity_id;
    UPDATE public.player_scheduled_activities SET status='in_progress', scheduled_start=v_start, scheduled_end=v_end, metadata=COALESCE(metadata,'{}'::jsonb)||jsonb_build_object('project_id', p_project_id, 'session_id', v_session_id, 'duration_hours', p_effort_hours, 'session_type', p_session_type) WHERE id=p_activity_id AND profile_id=p_profile_id AND status='scheduled';
...
CREATE OR REPLACE FUNCTION public.schedule_songwriting_session(p_profile_id uuid, p_project_id uuid, p_scheduled_start timestamp with time zone, p_effort_hours integer, p_session_type text DEFAULT 'balanced'::text, p_idempotency_key text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF v_project.profile_id <> p_profile_id THEN RAISE EXCEPTION 'This project belongs to another character.' USING ERRCODE='P0001'; END IF;
  IF v_project.status IN ('completed','converted') OR v_project.song_id IS NOT NULL THEN RAISE EXCEPTION 'The selected project is no longer available.' USING ERRCODE='P0001'; END IF;
  ...
  INSERT INTO public.player_scheduled_activities(user_id, profile_id, activity_type, scheduled_start, scheduled_end, status, title, description, metadata)
  VALUES (v_user, p_profile_id, 'songwriting', p_scheduled_start, v_end, 'scheduled', 'Songwriting: ' || COALESCE(v_project.title,'Untitled'), 'Planned songwriting session', jsonb_build_object('project_id', p_project_id, 'session_id', v_session_id, 'duration_hours', p_effort_hours, 'session_type', p_session_type, 'lifecycle', 'scheduled_in_progress_completed')) RETURNING id INTO v_activity_id;
  UPDATE public.songwriting_sessions SET progress_breakdown = COALESCE(progress_breakdown,'{}'::jsonb) || jsonb_build_object('activity_id', v_activity_id) WHERE id=v_session_id;
