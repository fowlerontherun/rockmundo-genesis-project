-- Canonical festival administration and owner-management consolidation.
...
) RETURNS public.festivals LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
...
RETURNS jsonb LANGUAGE sql SECURITY DEFINER SET search_path=public AS $$
  WITH e AS (SELECT * FROM public.festival_editions WHERE id=p_edition_id), facts AS (
    SELECT (SELECT status::text FROM e) lifecycle_state,
      (SELECT count(*) FROM public.festival_performance_sessions s WHERE s.edition_id=p_edition_id AND s.status='completed') completed_sessions,
      ...
      (SELECT count(*) FROM public.festival_performance_outcomes o WHERE o.edition_id=p_edition_id AND coalesce(o.invalidated_at, NULL) IS NOT NULL) invalidated_outcomes,
      (SELECT count(*) FROM public.festival_contracts c WHERE c.edition_id=p_edition_id AND c.status IN ('active','awaiting_signatures')) unsettled_contracts,
  ...
  ) SELECT jsonb_build_object('lifecycle_state',lifecycle_state,'completed_sessions',completed_sessions,'missing_outcomes',missing_outcomes,'invalidated_outcomes',invalidated_outcomes,'unsettled_contracts',unsettled_contracts,'unresolved_incidents',0,'attendance_finalised',lifecycle_state IN ('settling','completed'),'ledger_readiness','deferred','permit_insurance_issues',0,'pending_effects',pending_effects,'blockers',jsonb_build_array(CASE WHEN lifecycle_state NOT IN ('settling','completed') THEN 'edition_not_in_settlement_state' END, CASE WHEN missing_outcomes>0 THEN 'missing_outcomes' END),'warnings',jsonb_build_array(CASE WHEN unsettled_contracts>0 THEN 'contracts_waiting_for_next_pr_settlement' END),'ready_for_settlement', lifecycle_state IN ('settling','completed') AND missing_outcomes=0 AND invalidated_outcomes=0) FROM facts;
$$;
