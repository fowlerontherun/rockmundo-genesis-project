-- Complete the remaining Festival My Day activity types already modelled by C6.
-- Camping, VIP, vendor and free-time blocks previously reserved timetable space
-- but could never reach completed, so their C6 context effects were unreachable.
...
BEGIN
  ...
  IF v_item.activity_type NOT IN (
  ...
  CASE v_item.activity_type
    ...
    WHEN 'camping' THEN NULL;
    WHEN 'vip' THEN NULL;
    WHEN 'vendor' THEN NULL;
    WHEN 'free_time' THEN NULL;
  END CASE;
