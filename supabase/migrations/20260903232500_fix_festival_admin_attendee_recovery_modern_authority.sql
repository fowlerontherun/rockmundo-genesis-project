-- Production-forward correction: the active simplified Festival system is
-- festival_editions_v2 / festival_companies. Production does not yet carry the
-- repository-only legacy bridge table, so admin attendee tooling must accept the
-- modern edition id directly and audit to the modern company audit stream.
...
BEGIN
  ...
  SELECT edition.* INTO v_edition FROM public.festival_editions_v2 edition WHERE edition.id=p_edition_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'festival_modern_edition_not_found' USING ERRCODE='P0001'; END IF;

  IF p_idempotency_key IS NOT NULL THEN
    SELECT audit.metadata->'result' INTO v_existing
    FROM public.festival_company_audit_log audit
    WHERE audit.festival_company_id=v_edition.festival_company_id
      AND audit.action='admin_attendee_reconciled'
  ...
  FROM public.festival_player_attendance attendance
  WHERE attendance.id=p_attendance_id AND attendance.festival_edition_id=p_edition_id
  FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'festival_attendance_not_found_for_edition' USING ERRCODE='P0001'; END IF;
  ...
  SELECT attendance.* INTO v_attendance FROM public.festival_player_attendance attendance WHERE attendance.id=p_attendance_id FOR UPDATE;
  ...
  ELSIF v_attendance.status IN ('left_early','completed') THEN
    v_wristband_healthy := public._festival_ensure_attendance_wristband(v_attendance.id);
    PERFORM public._festival_close_attendance_schedule_lock(v_attendance.id,'admin_reconcile_terminal_attendance');
  END IF;

  SELECT attendance.* INTO v_attendance FROM public.festival_player_attendance attendance WHERE attendance.id=p_attendance_id;
  ...
  SELECT v_edition.festival_company_id, company.company_id, v_actor,
    'admin_attendee_reconciled', p_idempotency_key,
    jsonb_build_object(
      'festivalEditionId',p_edition_id,
  ...
  WHERE company.id=v_edition.festival_company_id;
...
REVOKE ALL ON FUNCTION public.admin_reconcile_festival_attendance(uuid,uuid,text,text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_reconcile_festival_attendance(uuid,uuid,text,text) TO authenticated, service_role;
