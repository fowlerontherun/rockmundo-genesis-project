-- Provide resilient fallbacks when provisioning new profiles
CREATE OR REPLACE FUNCTION public.get_default_city_id()
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
DECLARE
  preferred_cities uuid[] := ARRAY[
    '8d2e9d90-7f57-4df4-bc51-6f4743625f59'::uuid, -- London
    'a5bf9a04-5c3b-4c7c-99bb-f3a4ed2d64d6'::uuid  -- Portsmouth
  ];
  selected_city uuid;
BEGIN
  FOREACH selected_city IN ARRAY preferred_cities LOOP
    SELECT id INTO selected_city
    FROM public.cities
    WHERE id = selected_city
    LIMIT 1;

    EXIT WHEN selected_city IS NOT NULL;
  END LOOP;

  IF selected_city IS NULL THEN
    SELECT id INTO selected_city
    FROM public.cities
    WHERE unlocked IS TRUE
    ORDER BY name ASC
    LIMIT 1;
  END IF;
