...
BEGIN
  ...
  WITH tx AS (
    SELECT t.amount_minor AS amount_minor
    FROM public.bank_transactions t
    WHERE t.profile_id = pid AND COALESCE(t.currency_code,'GBP') = currency
  )
  SELECT
    COALESCE(SUM(CASE WHEN tx.amount_minor >= 0 THEN ABS(tx.amount_minor) ELSE 0 END),0),
    COALESCE(SUM(CASE WHEN tx.amount_minor < 0 THEN ABS(tx.amount_minor) ELSE 0 END),0)
  ...
  WITH months AS (
    ...
    SELECT months.m AS month_start,
      COALESCE(SUM(CASE WHEN tx.amount_minor >= 0 THEN tx.amount_minor ELSE 0 END),0)::bigint AS month_income,
      COALESCE(SUM(CASE WHEN tx.amount_minor < 0 THEN ABS(tx.amount_minor) ELSE 0 END),0)::bigint AS month_expenses
    FROM months LEFT JOIN tx ON date_trunc('month', tx.created_at) = months.m
    GROUP BY months.m
  )
  SELECT
    COALESCE(jsonb_agg(jsonb_build_object(
      'monthKey', to_char(monthly.month_start,'YYYY-MM'),
      'month', to_char(monthly.month_start,'Mon'),
      'incomeMinor', monthly.month_income,
      'expensesMinor', monthly.month_expenses,
      'currencyCode', currency
    ) ORDER BY monthly.month_start), '[]'::jsonb),
    COALESCE(ROUND(AVG(monthly.month_income))::bigint,0),
    COALESCE(ROUND(AVG(monthly.month_expenses))::bigint,0)
  INTO monthly_json, monthly_income, monthly_expenses
  FROM monthly;

  SELECT COALESCE(jsonb_object_agg(s.source, s.amount_total), '{}'::jsonb)
  INTO earnings_json
  FROM (
    SELECT initcap(replace(COALESCE(t.tx_type,'transaction'),'_',' ')) AS source, SUM(t.amount_minor)::bigint AS amount_total
    FROM public.bank_transactions t
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'balanceMinor', g.balance_total,
    'availableBalanceMinor', g.balance_total
  ) ORDER BY g.currency_code), '[]'::jsonb)
  INTO other_json
  FROM (
    SELECT COALESCE(a.currency_code,'GBP') AS currency_code, SUM(a.balance_minor)::bigint AS balance_total
    FROM public.bank_accounts a
    WHERE a.profile_id = pid AND a.status = 'active' AND COALESCE(a.currency_code,'GBP') <> currency
    GROUP BY 1
  ) g;
