...
CREATE POLICY "City development is viewable by everyone" ON public.city_development FOR SELECT USING (true);
...
INSERT INTO public.city_development (
  city_id, economy, infrastructure, transport, public_safety, healthcare,
  culture, music_scene, tourism, quality_of_life, education
)
SELECT
  c.id, 50, 50,
...
CREATE POLICY "City development history is viewable by everyone" ON public.city_development_history FOR SELECT USING (true);
...
RETURNS TABLE (
  economy_rating smallint, infrastructure_rating smallint, transport_rating smallint,
  public_safety_rating smallint, healthcare_rating smallint, culture_rating smallint,
  music_scene_rating smallint, tourism_rating smallint, quality_of_life_rating smallint,
  education_rating smallint, economy_revenue_multiplier numeric, audience_demand_multiplier numeric,
  travel_cost_multiplier numeric, travel_duration_multiplier numeric, incident_risk_multiplier numeric,
  recovery_multiplier numeric, festival_demand_multiplier numeric, tax_base_multiplier numeric,
  logistics_multiplier numeric, local_talent_multiplier numeric
)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_temp AS $$
  SELECT
    d.economy, d.infrastructure, d.transport, d.public_safety, d.healthcare,
    d.culture, d.music_scene, d.tourism, d.quality_of_life, d.education,
...
UPDATE public.city_project_types SET effects = effects || '{"infrastructure_rating":3,"music_scene_rating":3,"culture_rating":1}'::jsonb WHERE slug = 'build_music_venue';
UPDATE public.city_project_types SET effects = effects || '{"infrastructure_rating":5,"music_scene_rating":5,"culture_rating":2,"tourism_rating":2}'::jsonb WHERE slug = 'build_concert_hall';
UPDATE public.city_project_types SET effects = effects || '{"transport_rating":8,"infrastructure_rating":4,"tourism_rating":2}'::jsonb WHERE slug = 'upgrade_train_network';
UPDATE public.city_project_types SET effects = effects || '{"culture_rating":6,"music_scene_rating":4,"tourism_rating":4}'::jsonb WHERE slug = 'music_festival_sponsorship';
UPDATE public.city_project_types SET effects = effects || '{"culture_rating":4,"tourism_rating":2,"quality_of_life_rating":2}'::jsonb WHERE slug = 'public_art_program';
UPDATE public.city_project_types SET effects = effects || '{"education_rating":7,"music_scene_rating":3,"culture_rating":2}'::jsonb WHERE slug = 'music_education_grant';
UPDATE public.city_project_types SET effects = effects || '{"economy_rating":5}'::jsonb WHERE slug = 'tax_office_modernization';
UPDATE public.city_project_types SET effects = effects || '{"tourism_rating":8,"economy_rating":2,"quality_of_life_rating":1}'::jsonb WHERE slug = 'tourism_campaign';
UPDATE public.city_project_types SET effects = effects || '{"quality_of_life_rating":4,"infrastructure_rating":1}'::jsonb WHERE slug = 'noise_reduction_initiative';
UPDATE public.city_project_types SET effects = effects || '{"public_safety_rating":8,"quality_of_life_rating":2,"music_scene_rating":1}'::jsonb WHERE slug = 'public_safety_boost';
UPDATE public.city_project_types SET effects = effects || '{"healthcare_rating":8,"quality_of_life_rating":4}'::jsonb WHERE slug = 'healthcare_subsidy';

UPDATE public.city_projects p
SET effects = p.effects || t.effects, updated_at = now()
FROM public.city_project_types t
WHERE p.project_type_id = t.id AND p.status = 'in_progress';
...
BEGIN
  ...
  INSERT INTO public.city_development (city_id) VALUES (NEW.city_id) ON CONFLICT (city_id) DO NOTHING;

  SELECT * INTO v_before FROM public.city_development WHERE city_id = NEW.city_id FOR UPDATE;
  ...
  INSERT INTO public.city_development_history (city_id, project_id, mayor_id, source, deltas, before_state, after_state)
  VALUES (NEW.city_id, NEW.id, NEW.mayor_id, 'city_project', COALESCE(v_deltas, '{}'::jsonb),
          to_jsonb(v_before) - 'updated_at', to_jsonb(v_after) - 'updated_at');
