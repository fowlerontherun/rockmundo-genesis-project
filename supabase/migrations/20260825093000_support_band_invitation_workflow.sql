...
BEGIN
  ...
  -- Fresh authoritative validation at acceptance time. This is deliberately
  -- repeated even though candidate eligibility was checked when the offer was made.
  IF NOT EXISTS (
    SELECT 1
    FROM public.find_available_support_bands(
      v_gig.band_id,
  ...
  -- Block every active real support-band member for the show window.
  -- Reuse activity_type='gig' to remain compatible with existing activity constraints;
  -- metadata identifies the support role unambiguously.
  ...
  -- Once accepted, conflicting pending offers for the same support band/show window expire.
  ...
  -- Other pending candidates for this now-filled gig also expire.
  UPDATE public.gig_support_slots
  SET status = 'expired', updated_at = now()
