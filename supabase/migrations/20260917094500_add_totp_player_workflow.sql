...
BEGIN
  ...
  SELECT * INTO v_inv FROM public.totp_invitations WHERE id = p_invitation_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Top of the Pops invitation not found'; END IF;
  SELECT p.id INTO v_leader_profile FROM public.profiles p WHERE p.user_id = auth.uid() AND p.id = (SELECT leader_id FROM public.bands WHERE id = v_inv.band_id) LIMIT 1;
  IF v_leader_profile IS NULL THEN RAISE EXCEPTION 'Only the band leader can respond to this invitation'; END IF;
  ...
  SELECT DISTINCT bm.user_id, 'system', CASE WHEN p_response = 'accepted' THEN format('%s accepted Top of the Pops: %s will be performed in London. All active band members must attend studio check-in.', v_band_name, v_song_title) ELSE format('%s declined its Top of the Pops invitation for %s.', v_band_name, v_song_title) END
  FROM public.band_members bm WHERE bm.band_id = v_inv.band_id AND bm.user_id IS NOT NULL AND coalesce(bm.member_status, 'active') = 'active' AND coalesce(bm.is_touring_member, false) = false;
  RETURN p_response;
END;
$$;
...
BEGIN
  ...
  SELECT * INTO v_inv FROM public.totp_invitations WHERE id = p_invitation_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Top of the Pops invitation not found'; END IF;
  SELECT * INTO v_episode FROM public.totp_episodes WHERE id = v_inv.episode_id;
  SELECT p.id INTO v_leader_profile FROM public.profiles p WHERE p.user_id = auth.uid() AND p.id = (SELECT leader_id FROM public.bands WHERE id = v_inv.band_id) LIMIT 1;
  IF v_leader_profile IS NULL THEN RAISE EXCEPTION 'Only the band leader can check the band into the studio'; END IF;
  IF v_inv.status NOT IN ('accepted', 'checked_in') THEN RAISE EXCEPTION 'The invitation must be accepted before studio check-in'; END IF;
  IF v_inv.status = 'checked_in' THEN RETURN jsonb_build_object('status', 'checked_in', 'already_checked_in', true); END IF;
  IF now() < (v_episode.check_in_at - interval '2 hours') THEN RAISE EXCEPTION 'Studio check-in has not opened yet'; END IF;
  IF now() > (v_episode.check_in_at + interval '45 minutes') THEN RAISE EXCEPTION 'Studio check-in has closed'; END IF;

  WITH required_members AS (
    SELECT p.id, coalesce(p.display_name, p.username, 'Band member') AS display_name, p.current_city_id, coalesce(p.is_traveling, false) AS is_traveling
    FROM public.band_members bm JOIN public.profiles p ON p.id = bm.profile_id
    WHERE bm.band_id = v_inv.band_id AND bm.profile_id IS NOT NULL AND bm.user_id IS NOT NULL AND coalesce(bm.member_status, 'active') = 'active' AND coalesce(bm.is_touring_member, false) = false
  )
  SELECT count(*), count(*) FILTER (WHERE current_city_id = v_episode.city_id AND is_traveling = false), coalesce(array_agg(display_name) FILTER (WHERE current_city_id IS DISTINCT FROM v_episode.city_id OR is_traveling = true), '{}')
  INTO v_required_count, v_present_count, v_missing_names FROM required_members;

  IF v_required_count = 0 THEN RAISE EXCEPTION 'No active player-controlled band members were found'; END IF;
  IF v_present_count <> v_required_count THEN RAISE EXCEPTION 'All active band members must be in London and not travelling. Missing: %', array_to_string(v_missing_names, ', '); END IF;
  UPDATE public.totp_invitations SET status = 'checked_in', checked_in_at = now(), updated_at = now() WHERE id = p_invitation_id;
  RETURN jsonb_build_object('status', 'checked_in', 'members_required', v_required_count, 'members_present', v_present_count, 'checked_in_at', now());
END;
$$;
...
BEGIN
  ...
  RETURN jsonb_build_object('id', v_episode.id, 'episode_number', v_episode.episode_number, 'episode_date', v_episode.episode_date, 'status', v_episode.status, 'check_in_at', v_episode.check_in_at, 'broadcast_at', v_episode.broadcast_at, 'presenter_key', v_episode.presenter_key, 'broadcast_profile', v_episode.broadcast_profile, 'performances', v_performances);
END;
$$;
...
BEGIN
  ...
  SELECT p_episode_id, e.invitation_id, e.band_id, e.song_id, e.running_order,
    ...
    'totp_classic', format('At number %s this week, please welcome %s performing %s!', e.qualifying_rank, b.name, s.title)
...
DECLARE
  ...
  v_episode_date date;
BEGIN
  SELECT p.user_id, b.name::text INTO v_leader_user, v_band_name FROM public.bands b JOIN public.profiles p ON p.id = b.leader_id WHERE b.id = NEW.band_id;
  ...
  IF v_leader_user IS NOT NULL THEN
    INSERT INTO public.notifications (user_id, type, message) VALUES (v_leader_user, 'system', format('Top of the Pops invitation! %s is invited to London on %s to perform %s, currently #%s in the qualifying UK chart.', v_band_name, v_episode_date, v_song_title, NEW.qualifying_rank));
  END IF;
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER totp_notify_new_invitation AFTER INSERT ON public.totp_invitations FOR EACH ROW EXECUTE FUNCTION public.totp_notify_new_invitation();
