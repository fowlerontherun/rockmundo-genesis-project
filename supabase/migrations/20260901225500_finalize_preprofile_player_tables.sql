...
-- environment where public.profiles already existed. These definitions are
-- idempotent so existing production databases are reconciled without data loss.
...
CREATE POLICY "Players can react to community posts"
  ON public.community_post_reactions
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY "Players can update their community reactions"
  ON public.community_post_reactions
  FOR UPDATE
  TO authenticated
  USING (
...
CREATE POLICY "Players can remove their community reactions"
  ON public.community_post_reactions
  FOR DELETE
  TO authenticated
  USING (
...
-- Side-hustle mini-game progression ------------------------------------------
...
CREATE INDEX IF NOT EXISTS side_hustle_progress_profile_idx
  ON public.side_hustle_progress (profile_id);
CREATE INDEX IF NOT EXISTS side_hustle_minigame_attempts_profile_idx
