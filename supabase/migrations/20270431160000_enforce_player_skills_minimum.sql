-- Ensure a minimum skill baseline of 5 points across all tracked player skills
DO $$
DECLARE
...
BEGIN
  ...
  FROM (
    ...
    WHERE pc.conname = 'player_skills_value_bounds_check'
      AND pc.conrelid = 'public.player_skills'::regclass
  ) AS bounds;
    ...
    IF EXISTS (
      ...
      EXECUTE format(
        'UPDATE public.player_skills
          ...
          WHERE %1$I IS NULL OR %1$I < 5',
        column_name
      );

      EXECUTE format('ALTER TABLE public.player_skills ALTER COLUMN %I SET DEFAULT 5', column_name);
      ...
      check_fragments := array_append(check_fragments, format('%I BETWEEN 5 AND %s', column_name, max_value::text));
    END IF;
  END LOOP;
  ...
  IF array_length(present_skill_columns, 1) IS NOT NULL THEN
    ...
    EXECUTE 'ALTER TABLE public.player_skills ADD CONSTRAINT player_skills_value_bounds_check CHECK (' ||
      array_to_string(check_fragments, ' AND ') || ')';
  END IF;
END;
$$;

-- Provision new accounts with baseline skill scores of 5
...
-- Ensure resets also restore the 5-point skill baseline
