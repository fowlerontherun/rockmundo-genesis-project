-- Fix auto_complete_songwriting_sessions to NOT reference completed_at on songwriting_projects
-- The songwriting_projects table does not have a completed_at column
...
BEGIN
    ...
    LOOP
        -- Get skills from player_skills table (columns are direct integer values)
        SELECT 
            COALESCE(songwriting, 0) + COALESCE(composition, 0),
            COALESCE(creativity, 0)
        INTO v_composing_skill, v_lyrics_skill
        FROM player_skills
        WHERE user_id = v_session.user_id;
        
        -- Get attributes from player_attributes table
        SELECT 
        ...
        -- Update project progress (removed completed_at as it doesn't exist on this table)
        ...
        IF v_is_complete THEN
            -- Only update status, not completed_at (column doesn't exist)
            UPDATE songwriting_projects
            SET 
                status = 'completed',
                updated_at = NOW()
