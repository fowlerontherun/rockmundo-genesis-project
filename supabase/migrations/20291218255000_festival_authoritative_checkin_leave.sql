-- Authoritative Festival attendee mutations.
-- The browser may display readiness, but these RPCs re-check every material
-- rule inside the mutation transaction before changing attendance state.
...
BEGIN
  ...
  -- Idempotent retry after a successful check-in.
  ...
  -- Consume admission and establish physical Festival presence atomically.
  ...
  -- The AFTER UPDATE attendance trigger issues the wristband before this
  -- statement returns, so the response can confirm collectible ownership.
  SELECT EXISTS (
    SELECT 1
    FROM public.festival_player_memorabilia memorabilia
    WHERE memorabilia.attendance_id = v_attendance.id
...
GRANT EXECUTE ON FUNCTION public.check_in_to_festival(uuid) TO authenticated;
...
BEGIN
  ...
  -- Idempotent retry after the player has already left early.
...
GRANT EXECUTE ON FUNCTION public.leave_festival_early(uuid) TO authenticated;

COMMENT ON FUNCTION public.check_in_to_festival(uuid) IS
  'Atomically revalidates Festival admission, dates, location and travel state, consumes the admission ticket, transitions attendance to attending, and triggers wristband issuance.';

COMMENT ON FUNCTION public.leave_festival_early(uuid) IS
  'Allows only the active character to end an attending Festival lifecycle early; retries are idempotent.';
