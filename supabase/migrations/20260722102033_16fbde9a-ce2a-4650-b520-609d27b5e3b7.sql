...
BEGIN
  ...
  -- Log a scheduled activity for longer wellness actions so they appear in the timeline.
  -- Omit `duration_minutes` because it is a generated column on player_scheduled_activities.
  IF COALESCE(v_entry.duration_minutes, 0) >= 60 THEN
    BEGIN
      ...
      VALUES
        (v_uid, _profile_id, 'wellness_' || v_entry.category, v_start, v_end, 'in_progress', v_entry.name, v_entry.description,
...
-- One-time boost: restore health for all active characters by another 20 points (capped at 100)
UPDATE public.profiles
SET health = LEAST(100, COALESCE(health, 80) + 20),
    physical_health = LEAST(100, COALESCE(physical_health, health, 80) + 20),
    last_health_update = now();