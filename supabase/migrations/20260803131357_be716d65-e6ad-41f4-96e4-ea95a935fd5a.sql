...
GRANT SELECT ON public.festival_settlement_effect_configs TO authenticated;
GRANT ALL ON public.festival_settlement_effect_configs TO service_role;
ALTER TABLE public.festival_settlement_effect_configs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS festival_settlement_effect_configs_read ON public.festival_settlement_effect_configs;
CREATE POLICY festival_settlement_effect_configs_read ON public.festival_settlement_effect_configs FOR SELECT TO authenticated USING (true);
...
DROP POLICY IF EXISTS festival_edition_settlements_read ON public.festival_edition_settlements;
CREATE POLICY festival_edition_settlements_read ON public.festival_edition_settlements FOR SELECT TO authenticated USING (public.can_manage_festival_brand(festival_id) OR coalesce(public.has_role(auth.uid(),'admin'::public.app_role),false));
DROP POLICY IF EXISTS festival_settlement_events_read ON public.festival_settlement_events;
CREATE POLICY festival_settlement_events_read ON public.festival_settlement_events FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.festival_edition_settlements s WHERE s.id=settlement_id AND (public.can_manage_festival_brand(s.festival_id) OR coalesce(public.has_role(auth.uid(),'admin'::public.app_role),false))));
DROP POLICY IF EXISTS festival_effect_applications_read ON public.festival_effect_applications;
CREATE POLICY festival_effect_applications_read ON public.festival_effect_applications FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.festival_edition_settlements s WHERE s.id=settlement_id AND (public.can_manage_festival_brand(s.festival_id) OR coalesce(public.has_role(auth.uid(),'admin'::public.app_role),false))));
DROP POLICY IF EXISTS festival_fan_conversion_applications_read ON public.festival_fan_conversion_applications;
CREATE POLICY festival_fan_conversion_applications_read ON public.festival_fan_conversion_applications FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.festival_edition_settlements s WHERE s.id=settlement_id AND (public.can_manage_festival_brand(s.festival_id) OR coalesce(public.has_role(auth.uid(),'admin'::public.app_role),false))));
DROP POLICY IF EXISTS festival_contract_settlement_instructions_read ON public.festival_contract_settlement_instructions;
CREATE POLICY festival_contract_settlement_instructions_read ON public.festival_contract_settlement_instructions FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.festival_edition_settlements s WHERE s.id=settlement_id AND (public.can_manage_festival_brand(s.festival_id) OR coalesce(public.has_role(auth.uid(),'admin'::public.app_role),false))));
DROP POLICY IF EXISTS festival_settlement_transactions_read ON public.festival_settlement_transactions;
CREATE POLICY festival_settlement_transactions_read ON public.festival_settlement_transactions FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.festival_edition_settlements s WHERE s.id=settlement_id AND (public.can_manage_festival_brand(s.festival_id) OR coalesce(public.has_role(auth.uid(),'admin'::public.app_role),false))));
DROP POLICY IF EXISTS festival_edition_financial_results_read ON public.festival_edition_financial_results;
CREATE POLICY festival_edition_financial_results_read ON public.festival_edition_financial_results FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.festival_edition_settlements s WHERE s.id=settlement_id AND (public.can_manage_festival_brand(s.festival_id) OR coalesce(public.has_role(auth.uid(),'admin'::public.app_role),false))));
DROP POLICY IF EXISTS streaming_uplift_campaigns_read ON public.streaming_uplift_campaigns;
CREATE POLICY streaming_uplift_campaigns_read ON public.streaming_uplift_campaigns FOR SELECT TO authenticated USING (coalesce(public.has_role(auth.uid(),'admin'::public.app_role),false));
...
GRANT ALL ON public.festival_edition_settlements, public.festival_settlement_events, public.festival_effect_applications, public.festival_fan_conversion_applications, public.streaming_uplift_campaigns, public.festival_contract_settlement_instructions, public.festival_settlement_transactions, public.festival_edition_financial_results TO service_role;
