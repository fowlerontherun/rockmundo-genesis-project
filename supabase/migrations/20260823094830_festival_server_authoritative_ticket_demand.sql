-- Make Festival ticket demand a server-owned projection rather than a player-entered value.
-- Owners choose price and availability; marketing, reputation and upgrades drive demand.
...
REVOKE ALL ON FUNCTION public._festival_ticket_demand_basis_points(uuid, uuid, bigint) FROM PUBLIC;
REVOKE ALL ON FUNCTION public._festival_ticket_demand_basis_points(uuid, uuid, bigint) FROM anon;
REVOKE ALL ON FUNCTION public._festival_ticket_demand_basis_points(uuid, uuid, bigint) FROM authenticated;
...
BEGIN
  ...
  -- Deliberately ignore any client-provided expectedSellThroughBasisPoints.
