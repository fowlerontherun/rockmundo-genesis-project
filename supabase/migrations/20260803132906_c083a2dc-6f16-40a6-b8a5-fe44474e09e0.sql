...
ON CONFLICT (type_key) DO UPDATE SET
  ...
  icon = EXCLUDED.icon, color = EXCLUDED.color, creation_cost = EXCLUDED.creation_cost,
  starting_balance = EXCLUDED.starting_balance, weekly_operating_costs = EXCLUDED.weekly_operating_costs,
  base_tax_rate = EXCLUDED.base_tax_rate, updated_at = now();
...
GRANT SELECT ON public.festival_companies TO authenticated;
GRANT SELECT ON public.festival_editions_v2 TO authenticated;
GRANT SELECT ON public.festival_company_audit_log TO authenticated;
GRANT SELECT ON public.festival_company_founding_requests TO authenticated;
GRANT ALL ON public.festival_companies TO service_role;
GRANT ALL ON public.festival_editions_v2 TO service_role;
GRANT ALL ON public.festival_company_audit_log TO service_role;
GRANT ALL ON public.festival_company_founding_requests TO service_role;
...
CREATE POLICY "Festival owners can read own festival companies" ON public.festival_companies FOR SELECT TO authenticated USING (
...
CREATE POLICY "Festival owners can read own festival editions" ON public.festival_editions_v2 FOR SELECT TO authenticated USING (
...
CREATE POLICY "Festival owners can read own audit log" ON public.festival_company_audit_log FOR SELECT TO authenticated USING (
...
CREATE POLICY "Festival founders can read own founding requests" ON public.festival_company_founding_requests FOR SELECT TO authenticated USING (
  actor_user_id = auth.uid() OR coalesce(public.has_role(auth.uid(),'admin'::public.app_role), false)
);
