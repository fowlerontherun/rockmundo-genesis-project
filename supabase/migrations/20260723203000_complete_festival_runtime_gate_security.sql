-- Complete festival-company runtime gate and secure privileged founding internals.
-- Forward-only: no historical balances, companies, requests, audit rows, or capability values are modified.
...
BEGIN
  ...
  WHERE owner_id=v_user AND parent_company_id IS NULL AND company_type IS DISTINCT FROM 'festival'
    AND status = 'active' AND COALESCE(is_bankrupt,false)=false;
...
DECLARE
  ...
  v_fixture boolean := current_setting('app.allow_test_fixtures', true) = 'true';
BEGIN
  ...
  IF v_fixture AND current_setting('app.festival_foundation_pause_after_lock', true) = 'on' THEN PERFORM pg_sleep(2); END IF;
  IF v_fixture AND current_setting('app.festival_foundation_fail_after_lock', true) = 'on' THEN RAISE EXCEPTION 'festival_test_early_failure' USING ERRCODE='P0001'; END IF;
  ...
  SELECT available_balance_minor INTO v_available_minor FROM public.financial_accounts WHERE owner_type='player' AND owner_id=v_profile.id AND is_primary FOR UPDATE;
  IF v_available_minor IS NULL THEN RAISE EXCEPTION 'festival_primary_financial_account_required' USING ERRCODE='P0001'; END IF;
  IF v_available_minor < v_cost_minor THEN RAISE EXCEPTION 'insufficient_personal_funds' USING ERRCODE='P0001'; END IF;
  ...
  IF v_fixture AND current_setting('app.festival_foundation_fail_after_extension', true) = 'on' THEN RAISE EXCEPTION 'festival_test_extension_failure' USING ERRCODE='P0001'; END IF;
  ...
  IF v_fixture AND current_setting('app.festival_foundation_fail_after_finance', true) = 'on' THEN RAISE EXCEPTION 'festival_test_post_debit_failure' USING ERRCODE='P0001'; END IF;
