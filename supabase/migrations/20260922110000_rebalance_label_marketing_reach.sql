-- Rebalance PR/marketing so spend buys reach while song quality can still create organic breakouts.
-- Wealthy labels can fund genuinely large campaigns, with diminishing returns handled by the
-- daily demand engines rather than a low hard ceiling.
...
begin
  ...
  loop
    ...
    v_power := least(80,greatest(1,round(sqrt(v_per_release/10.0)*v_multiplier*2.5)::integer));
  ...
  loop
    ...
    v_hype:=least(120,greatest(1,round(sqrt(v_per_release/10.0)*v_multiplier)::integer));
    v_power:=least(100,greatest(1,round(sqrt(v_per_release/10.0)*v_multiplier*2.5)::integer));
