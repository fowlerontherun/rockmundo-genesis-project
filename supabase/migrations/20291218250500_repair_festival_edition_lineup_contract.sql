-- Keep the simplified annual Festival line-up truthful and edition-bound.
-- At least one real confirmed booking is required before the owner can finalise;
-- remaining performance slots may still be filled automatically by the runtime.
...
COMMENT ON FUNCTION public.save_festival_edition_artist_programme(uuid,uuid,integer,jsonb,jsonb,uuid,boolean)
IS 'Exact-edition simplified line-up save; final completion requires at least one confirmed booking and keeps remaining schedule slots eligible for automatic NPC fallback.';
