-- Drop functions whose return types are being changed
DROP FUNCTION IF EXISTS public.counter_festival_offer(uuid,integer,jsonb,text,text);
DROP FUNCTION IF EXISTS public.accept_festival_offer(uuid,integer,text);
DROP FUNCTION IF EXISTS public.create_festival_offer(uuid,uuid,uuid,jsonb,text);
DROP FUNCTION IF EXISTS public.submit_festival_application(uuid,uuid,jsonb,text);
DROP FUNCTION IF EXISTS public.sign_festival_contract(uuid,integer,public.festival_booking_side,jsonb,text);
DROP FUNCTION IF EXISTS public.cancel_festival_contract(uuid,text,text);
DROP FUNCTION IF EXISTS public.save_festival_setlist_draft(uuid,integer,jsonb,text);
DROP FUNCTION IF EXISTS public.submit_festival_setlist(uuid,text);
DROP FUNCTION IF EXISTS public.review_festival_setlist(uuid,text,text,text);
DROP FUNCTION IF EXISTS public.lock_festival_setlist(uuid,text);
DROP FUNCTION IF EXISTS public.festival_request_begin(text,text,uuid,text,text,uuid);
DROP FUNCTION IF EXISTS public.festival_validate_stage_slot(uuid,uuid,jsonb);

CREATE EXTENSION IF NOT EXISTS pgcrypto;

DO $$ BEGIN CREATE TYPE public.festival_stage_slot_reservation_status AS ENUM ('provisional','confirmed','released','expired'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.festival_booking_requests TO authenticated;
GRANT ALL ON public.festival_booking_requests TO service_role;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.festival_stage_slot_reservations TO authenticated;
GRANT ALL ON public.festival_stage_slot_reservations TO service_role;
GRANT SELECT ON public.festival_stage_slot_reservations TO anon;
ALTER TABLE public.festival_stage_slot_reservations ENABLE ROW LEVEL SECURITY;
CREATE UNIQUE INDEX IF NOT EXISTS festival_stage_slot_reservations_one_live_slot_idx ON public.festival_stage_slot_reservations(stage_slot_id) WHERE status IN ('provisional','confirmed');
CREATE UNIQUE INDEX IF NOT EXISTS festival_stage_slot_reservations_one_offer_idx ON public.festival_stage_slot_reservations(offer_id) WHERE offer_id IS NOT NULL AND status IN ('provisional','confirmed');
CREATE UNIQUE INDEX IF NOT EXISTS festival_stage_slot_reservations_one_contract_idx ON public.festival_stage_slot_reservations(contract_id) WHERE contract_id IS NOT NULL AND status IN ('provisional','confirmed');
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.festival_contract_versions TO authenticated;
GRANT ALL ON public.festival_contract_versions TO service_role;
...
CREATE UNIQUE INDEX IF NOT EXISTS festival_contract_setlists_one_current_idx ON public.festival_contract_setlists(contract_id) WHERE is_current;
CREATE UNIQUE INDEX IF NOT EXISTS festival_contract_setlists_content_idempotent_idx ON public.festival_contract_setlists(contract_id, content_hash) WHERE content_hash IS NOT NULL;

ALTER TABLE public.festival_contract_setlist_items ALTER COLUMN song_id SET NOT NULL;
DO $$ BEGIN
  ALTER TABLE public.festival_contract_setlist_items ADD CONSTRAINT festival_contract_setlist_items_song_fk FOREIGN KEY (song_id) REFERENCES public.songs(id) ON DELETE RESTRICT;
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
CREATE UNIQUE INDEX IF NOT EXISTS festival_contract_setlist_items_unique_song_idx ON public.festival_contract_setlist_items(setlist_id, song_id);
...
ALTER TABLE public.festival_stage_slots ADD CONSTRAINT festival_stage_slots_canonical_contract_fk FOREIGN KEY (canonical_contract_id) REFERENCES public.festival_contracts(id) ON DELETE SET NULL;
...
LANGUAGE sql IMMUTABLE SET search_path=public,extensions AS $$ SELECT encode(digest(COALESCE(p_value,'null'::jsonb)::text, 'sha256'), 'hex') $$;
...
DROP POLICY IF EXISTS "Actors view their booking requests" ON public.festival_booking_requests;
CREATE POLICY "Actors view their booking requests" ON public.festival_booking_requests FOR SELECT TO authenticated USING (actor_profile_id = public.current_profile_id_safe() OR public.has_role(auth.uid(),'admin'::public.app_role));

DROP POLICY IF EXISTS "Reservations readable by band or organiser" ON public.festival_stage_slot_reservations;
CREATE POLICY "Reservations readable by band or organiser" ON public.festival_stage_slot_reservations FOR SELECT TO authenticated USING (
  public.is_current_band_member(band_id)
  OR EXISTS (SELECT 1 FROM public.festival_editions e WHERE e.id = edition_id AND public.can_manage_festival_brand(e.festival_id))
  OR public.has_role(auth.uid(),'admin'::public.app_role)
);

DROP POLICY IF EXISTS "Contract versions readable by parties" ON public.festival_contract_versions;
CREATE POLICY "Contract versions readable by parties" ON public.festival_contract_versions FOR SELECT TO authenticated USING (
  EXISTS (SELECT 1 FROM public.festival_contracts c JOIN public.festival_editions e ON e.id = c.edition_id WHERE c.id = contract_id AND (public.is_current_band_member(c.band_id) OR public.can_manage_festival_brand(e.festival_id)))
  OR public.has_role(auth.uid(),'admin'::public.app_role)
);