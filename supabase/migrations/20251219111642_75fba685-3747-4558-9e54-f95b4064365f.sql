...
--
-- The canonical labels schema requires created_by and migrations do not run with
-- an authenticated request, so auth.uid() is NULL here. Reuse the deterministic
-- system catalogue owner established by 20251218143953 and the same stable-ID
-- convention. This also makes overlapping catalogue names idempotent on clean
-- database rebuilds.
...
SELECT
  ...
  seed.name,
  seed.description,
  seed.headquarters_city,
  seed.genre_focus,
  seed.reputation_score,
  seed.roster_slot_capacity,
  seed.marketing_budget,
  seed.market_share,
