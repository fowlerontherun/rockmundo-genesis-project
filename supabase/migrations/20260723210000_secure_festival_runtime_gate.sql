-- Secure festival-company runtime gate helpers and test-only instrumentation.
-- Forward-only: preserves existing data, balances, audit history and administrator rollout values.

CREATE OR REPLACE FUNCTION public._festival_test_fixtures_allowed()
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT current_setting('app.allow_test_fixtures', true) = 'true'
$$;

COMMENT ON FUNCTION public._festival_test_fixtures_allowed() IS
  'Test-only guard. Runtime failure/delay hooks are ignored unless app.allow_test_fixtures is set transaction-locally by the database test harness.';
...
RETURNS integer LANGUAGE sql STABLE SECURITY DEFINER SET search_path=public AS $$
  SELECT CASE WHEN p_company_type = 'festival' THEN public.festival_company_ownership_limit(p_profile_id) ELSE 3 END
$$;
...
BEGIN
  IF p_company_type = 'festival' THEN
    RETURN public.can_profile_found_festival_company(p_profile_id);
  END IF;
...
COMMENT ON FUNCTION public.company_ownership_limit(uuid,text) IS 'Deprecated compatibility helper. Use festival_company_ownership_limit for festival founding; non-festival calls receive the legacy generic limit.';
COMMENT ON FUNCTION public.can_profile_found_company(uuid,text) IS 'Deprecated compatibility helper. Use can_profile_found_festival_company for festival founding; non-festival calls do not read festival rollout configuration.';
...
BEGIN
  ...
  IF v_test AND current_setting('app.festival_foundation_delay_after_lock', true) ~ '^\d+(\.\d+)?$' THEN
    PERFORM pg_sleep(LEAST(current_setting('app.festival_foundation_delay_after_lock', true)::numeric, 5));
  END IF;
  ...
  IF v_test AND current_setting('app.festival_foundation_fail_after_extension', true) = 'on' THEN RAISE EXCEPTION 'festival_test_late_failure' USING ERRCODE='P0001'; END IF;
  ...
  IF v_test AND current_setting('app.festival_foundation_fail_after_debit', true) = 'on' THEN RAISE EXCEPTION 'festival_test_post_debit_failure' USING ERRCODE='P0001'; END IF;
...
REVOKE ALL ON FUNCTION public.finance_debit_player_personal_cash(uuid,bigint,public.financial_transaction_category,text,text,jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.finance_debit_player_personal_cash(uuid,bigint,public.financial_transaction_category,text,text,jsonb) FROM anon;
REVOKE ALL ON FUNCTION public.finance_debit_player_personal_cash(uuid,bigint,public.financial_transaction_category,text,text,jsonb) FROM authenticated;
REVOKE ALL ON FUNCTION public.festival_company_ownership_limit(uuid) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.can_profile_found_festival_company(uuid) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.company_ownership_limit(uuid,text) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.can_profile_found_company(uuid,text) FROM PUBLIC, anon, authenticated;
