...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => {
    ...
    class MockAudio {
      src = ''; preload = ''; volume = 1; muted = false; currentTime = 0; duration = 180; playbackRate = 1;
      ...
      load() { counters.loads++; }
      async play() {
        if (!this.active) { this.active = true; counters.playing++; }
        return undefined;
      }
      pause() {
        if (this.active) { this.active = false; counters.playing = Math.max(0, counters.playing - 1); }
        counters.paused++;
      }
...
test('audio activation, speed muting, seek cleanup, close and reopen use one active element', async ({ page }) => {
  ...
  await page.getByLabel('Device preview selector', { exact: true }).click();
  await page.getByRole('option', { name: '360 × 800', exact: true }).click();
