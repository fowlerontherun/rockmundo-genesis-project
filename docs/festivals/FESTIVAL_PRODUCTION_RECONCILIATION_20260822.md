...
The final clean-build differences are instead applied after `supabase db reset` by:

`supabase/reconciliation/festival/20260822_simplified_festival_post_bootstrap.sql`

The Festival database CI gate applies that overlay after every clean reset and then executes:
...
2. the Festival post-bootstrap overlay must then pass;
...
The repaired live schema was tested using rollback-only state changes. The certification exercised a runnable annual Festival through completion, verified deterministic runtime evidence, NPC filler, conservation-safe attendance, completion digests and immutable Results, then rolled the test transaction back so the real Shock Festival state was unchanged.
