...
## PR #1282 corrective runtime/security gate

PR #1281 introduced the replacement festival-company runtime gate but did not execute the complete database gate in this repository environment, and its concurrency script was still a capability-only placeholder. This corrective PR replaces that placeholder with a real overlapping two-session `found_festival_company(...)` test, keeps fixture controls transaction-local behind `app.allow_test_fixtures = true`, and moves the forced rollback point to after `finance_debit_player_personal_cash(...)` so the rollback covers the actual money debit.

The privileged finance bridge is internal-only: `finance_debit_player_personal_cash(uuid,bigint,financial_transaction_category,text,text,jsonb)` has all direct `PUBLIC`, `anon`, and `authenticated` execute rights revoked. The authenticated public entry point remains `found_festival_company(...)`; because it is `SECURITY DEFINER`, it can still call the helper without granting clients arbitrary profile IDs, amounts, categories, idempotency keys, or metadata.

The executable harness documents and asserts the successful balance movement from `1,000,000,000` minor units (`$10,000,000`) to `800,000,000` minor units (`$8,000,000`). It expects exactly one `festival_company_founding_fee` financial transaction for the successful request, exactly one founding request, one company, one festival extension, one shareholder row, and no company operating expense. The two-session test asserts both overlapping callers receive the same company and festival-company IDs, one response is the original success, one response is the saved idempotent result, the final wallet is `$8,000,000`, and no duplicate request, audit, company, extension, shareholder, or finance rows are created.

The post-debit rollback fixture now uses `app.festival_foundation_fail_after_finance = on` only when `app.allow_test_fixtures = true`. The runtime harness asserts a founder reset to `$10,000,000` still has `1,000,000,000` minor units after the forced failure and that rollback leaves no matching `companies`, `festival_companies`, `company_shareholders`, `festival_company_audit_log`, `festival_company_founding_requests`, `financial_transactions`, or `financial_ledger_entries` rows for that failed idempotency key.

Generic company-limit wrappers no longer delegate to festival rollout configuration. Festival founding calls `festival_company_ownership_limit(...)` and `can_profile_found_festival_company(...)`; the compatibility wrappers now use a generic active non-festival company count and a fixed generic limit so unrelated company creation is not silently governed by festival capability JSON.

The frontend now fails closed for malformed festival-company RPC responses. Capabilities and eligibility return complete disabled states when any required field is absent or malformed; owned festival-company arrays are rejected rather than partially coerced; malformed founding results throw before cache updates. Successful founding invalidates `companies`, `owned-festival-companies`, `festival-company-founding-eligibility`, `festival-company-capabilities`, `active-profile`, `user-cash-balance`, and the returned `festival-company-setup` query. Festival cards now route both incomplete and completed companies to the registered authorised setup/summary path `/companies/festivals/:festivalCompanyId/setup`, with labels `Continue setup` and `View setup summary`.

Validation commands requested for this PR are `npm ci`, `npm run typecheck`, `npm run lint`, `npm run test:unit`, `npm run build`, `npm run verify:supabase-rpcs`, and `npm run test:festivals:company-runtime`. Do not claim the database gate has passed unless `npm run test:festivals:company-runtime` has run successfully against a freshly reset local Supabase database with `SUPABASE_DB_URL` set. After this PR is merged and that database gate genuinely passes, the following PR can implement the festival configuration wizard and first annual festival edition.
