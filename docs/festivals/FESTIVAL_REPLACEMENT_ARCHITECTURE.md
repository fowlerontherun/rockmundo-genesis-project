...
## Canonical route ownership (2026-07 route migration)

`src/features/festivals/routes.ts` is the route contract. Public discovery is owned by the Festival-company launch directory at `/world/festivals`; its company identifier is explicitly a public slug or Festival-company UUID and an edition identifier is explicitly an edition UUID or year. Owner URLs use only Festival-company UUIDs and canonical annual-edition UUIDs. Generic company ownership and finance remain at `/company/:companyId`.

The owner edition shell owns overview, schedule, applications, contracts, operations, finance, live, settlement and history. It resolves authority once through `resolve_owner_festival_identifier`; browser code must not probe tables or infer cross-domain UUID meanings. Admin catalogue routes use Festival-company and edition UUIDs.
