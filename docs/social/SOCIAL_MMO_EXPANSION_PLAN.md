...
4. Add contribution logs for existing band actions. Initial server-authoritative, read-only contribution events now exist for completed band rehearsals, completed band recording sessions, and completed gig outcomes, with current-active-member-only visibility and no rewards. Recording contribution uses a small source adapter to credit verified session participants from the session owner and uploaded production tracks where available; future rehearsal and gig contribution now use narrow attendance/lineup records seeded by booking and finalized by completion flows; existing rehearsal and gig detail surfaces now show those read-only participant/lineup statuses to current band members, while older historical events remain less precise. Attendance and lineup product rules are now documented in `docs/social/ATTENDANCE_AND_LINEUP_RULES.md`; Rehearsal self-response RSVP now lets invited participants confirm or decline their own rehearsal row before the one-hour lock deadline, with schedule release/restore, audit logging, manager decline notification dedupe, and no rewards, penalties, XP, or chemistry changes. Lineup mutation, manager finalisation, corrections, disputes, rewards, penalties, XP, and chemistry changes remain future implementation work.
...
- Add broader band roles, permissions, and contribution history; guarded band invitation creation/response and application submission/approval/rejection now share the first server-authoritative recruitment permission and membership-entry helper path. Phase 4 attendance/lineup planning now recommends implementing rehearsal self-response, rehearsal finalisation, gig lineup management, performer confirmation/lock, and correction/dispute flows in narrow follow-up PRs before any progression consequences.
...
Phase 4 PR 10 performed the rehearsal attendance correction release-gate review before gig-lineup mutation work. The correction SQL harness was expanded with executable schema, grant, RLS, privacy, contribution-correction, and fixture-capability assertions, but the Supabase CLI was unavailable in the validation environment, so local reset and SQL harness execution were not completed. Static migration-order review found a P0 blocker: `20260711...` rehearsal attendance/correction migrations depend on `band_rehearsal_participants`, while `20290711030000_rehearsal_attendance_gig_lineups.sql` creates that table later in lexicographic migration order. Phase 4 is not complete, and gig-lineup mutation is not ready to begin until a migration-order repair PR proves clean reset plus participant/correction harness success.

## Phase 4 PR 11 factual status update

Phase 4's rehearsal attendance database baseline has a repository-level migration-order repair: contribution adapters and rehearsal participant/gig performer foundations now sort before the 2026 RSVP/finalisation/correction migrations, and the preserved late migration is idempotent against those earlier definitions. The clean-reset gate has **not** passed in this container because Supabase CLI, Docker, and `psql` are unavailable. Phase 4 therefore remains not complete, and gig-lineup mutation remains not ready until reset, migration status, SQL harnesses, generated types, and application checks pass in a fully provisioned Supabase environment.
