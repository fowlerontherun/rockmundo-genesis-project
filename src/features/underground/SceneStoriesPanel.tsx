...
export const SceneStoriesPanel = ({ profileId, surface }: { profileId: string | null; surface: SceneStorySurface }) => {
  ...
  const description = surface === "nightclub"
    ? "Your contacts, Heat and current reputation shape what happens after dark."
    : "Underground Cred, scandals and your current state unlock different off-grid opportunities.";

  const query = useQuery({
    queryKey: ["scene-stories", profileId, surface],
    enabled: Boolean(profileId),
    queryFn: async () => {
      const { data, error } = await (supabase as any).rpc("get_scene_story_opportunities", {
  ...
  const start = useMutation({
    mutationFn: async (slug: string) => {
      const { data, error } = await (supabase as any).rpc("start_scene_story", {
        p_profile_id: profileId,
        p_story_slug: slug,
        p_surface: surface,
      });
    ...
    onSuccess: (result) => {
      if (!result.ok) {
        toast.error(result.reason === "not_eligible" ? "That opportunity is no longer available." : "Could not start this scene story.");
      } else {
        toast.success("The night just took a turn...");
      }
  ...
  const resolve = useMutation({
    mutationFn: async ({ runId, choiceKey }: { runId: string; choiceKey: string }) => {
      const { data, error } = await (supabase as any).rpc("resolve_scene_story_choice", {
        p_profile_id: profileId,
        p_run_id: runId,
    ...
    onSuccess: (result) => {
      if (!result.ok) {
        toast.error(result.reason === "expired" ? "That opportunity expired." : "That choice could not be resolved.");
      } else if (result.outcome === "success") {
        toast.success(result.completed ? "Scene story completed." : "That went your way. The night continues...");
      } else {
        toast.warning(result.completed ? "The story ended messily." : "That did not go to plan. You still have a choice to make.");
      }
      if (profileId) invalidateSceneQueries(queryClient, profileId);
    },
      ...
      <CardContent className="space-y-3">
        {query.isLoading ? (
          <div className="flex items-center justify-center py-5 text-sm text-muted-foreground">
            <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Checking the scene...
          </div>
        ) : activeRun?.step ? (
          <div className="rounded-xl border bg-background/70 p-4 space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="font-semibold">{activeRun.step.title}</h3>
              <Badge variant="secondary" className="text-[10px]">
                <Flame className="mr-1 h-3 w-3" /> {riskLabel(activeRun.riskTier)}
              </Badge>
            </div>
            ...
            <div className="grid gap-2 md:grid-cols-2">
              {(activeRun.step.choices ?? []).map((choice) => (
                <Button
                  key={choice.key}
                  variant="outline"
                  className="h-auto min-h-16 justify-between gap-3 whitespace-normal px-3 py-2 text-left"
                  disabled={resolve.isPending}
                  onClick={() => resolve.mutate({ runId: activeRun.id, choiceKey: choice.key })}
                >
                  <span>
                    <span className="block font-medium">{choice.label}</span>
                    {choice.description && <span className="mt-0.5 block text-xs font-normal text-muted-foreground">{choice.description}</span>}
                  </span>
          ...
          <div className="grid gap-2 md:grid-cols-3">
            {opportunities.map((story) => (
              <button
                key={story.id}
                type="button"
                onClick={() => start.mutate(story.slug)}
                disabled={start.isPending}
                className="rounded-xl border bg-background/60 p-3 text-left transition-colors hover:border-primary/50 disabled:opacity-60"
              >
                <div className="flex items-start justify-between gap-2">
                  <span className="font-medium text-sm">{story.name}</span>
                  <Badge variant="outline" className="shrink-0 text-[10px]">Risk {story.riskTier}</Badge>
                </div>
          ...
          <div className="flex items-start gap-2 rounded-lg border border-dashed px-3 py-3 text-sm text-muted-foreground">
            ...
            <span>No special opportunities are circulating right now. Contacts, Heat, scandals, recovery and Underground Cred can all change what appears here.</span>
          </div>
        )}
      </CardContent>
    </Card>
