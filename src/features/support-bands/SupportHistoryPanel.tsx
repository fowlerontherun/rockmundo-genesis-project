...
import { Award, Handshake, Loader2, Star, Ticket, Users } from 'lucide-react';
...
export function SupportHistoryPanel({ bandId }: { bandId: string }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (ids.length > 0) {
        ...
        if (mounted && !names.error) {
          setBandNames(Object.fromEntries((names.data ?? []).map((row: any) => [row.id, row.name])));
        }
      }
  ...
  if (loading) {
    return <Card><CardContent className="flex items-center gap-2 p-6 text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" /> Loading support history...</CardContent></Card>;
  }

  return (
    <Card className="border-primary/20">
      <CardHeader>
        <CardTitle className="flex items-center gap-2"><Award className="h-5 w-5" /> Support Reputation & History</CardTitle>
        <CardDescription>Your band's track record as a support act and the benefits earned from completed shows.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-5">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm"><span>Successful support shows</span><span>{successRate}%</span></div>
          <Progress value={successRate} />
        </div>

        <div className="space-y-3">
          <h4 className="font-semibold">Recent support appearances</h4>
          {recent.length === 0 ? <p className="text-sm text-muted-foreground">No completed support shows yet.</p> : recent.map((row) => (
            <div key={row.id} className="rounded-lg border p-3">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  <p className="font-medium">Supporting {bandNames[row.headliner_band_id] ?? 'Headliner'}</p>
                  <p className="text-xs text-muted-foreground">{row.performed_at ? new Date(row.performed_at).toLocaleDateString() : 'Completed show'}</p>
                </div>
                <Badge variant="outline">+{row.reputation_gain} reputation</Badge>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2 text-sm md:grid-cols-4">
                <div className="flex items-center gap-1"><Ticket className="h-3.5 w-3.5 text-muted-foreground" />{money(row.support_payment)}</div>
                <div className="flex items-center gap-1"><Users className="h-3.5 w-3.5 text-muted-foreground" />+{row.support_fan_gain} fans</div>
                <div className="flex items-center gap-1"><Star className="h-3.5 w-3.5 text-muted-foreground" />{Number(row.performance_rating).toFixed(1)}/25</div>
