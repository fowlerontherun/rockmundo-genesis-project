...
export function WeeklyRecapCard() {
  ...
  const topActionLabel = recap?.topActionType
    ? (RELATIONSHIP_ACTION_REWARDS[recap.topActionType]?.label ??
      recap.topActionType)
    : null;
    ...
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-xs">
          Last 7 days of relationship activity
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {!hasActivity ? (
          <p className="text-sm text-muted-foreground">
            No social interactions in the past week. Use quick actions on a
            friend to start earning XP.
          </p>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="rounded-lg border bg-card p-2.5">
                <p className="text-[10px] tracking-wide text-muted-foreground flex items-center gap-1">
            ...
            {topActionLabel && (
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                ...
                <Badge variant="outline">
                  {topActionLabel} × {recap!.topActionCount}
                </Badge>
              </div>
            )}
          </>
