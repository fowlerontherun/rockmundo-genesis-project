...
export function AffinityMeter({ summary }: AffinityMeterProps) {
  const nextTier =
    FRIENDSHIP_TIERS.find((tier) => tier.minAffinity > summary.tierMinimum) ??
    null;
    ...
    <Card>
      <CardHeader className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <CardDescription>
            Track your bond strength and upcoming perks.
          </CardDescription>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-lg border bg-muted/40 p-3">
            ...
            <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
              {FRIENDSHIP_TIERS.find(
                (tier) => tier.id === summary.tierId,
          ...
          <div className="rounded-lg border bg-muted/40 p-3">
            ...
            <ul className="mt-2 space-y-1 text-sm text-muted-foreground">
              {FRIENDSHIP_TIERS.find(
                (tier) => tier.id === summary.tierId,
              )?.collabBoosts.map((boost) => <li key={boost}>• {boost}</li>)}
            </ul>
          </div>
        </div>
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Target {milestone.threshold} affinity
                  </p>
                </div>
