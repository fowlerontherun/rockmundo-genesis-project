...
describe("festival booking helpers", () => {
  it("validates all core application transitions", () => {
    ...
    expect(
      canTransitionApplication("offer_pending", "converted_to_contract"),
    ).toBe(true);
  ...
  it("validates contract transitions", () => {
    ...
    expect(canTransitionContract("awaiting_signatures", "fulfilled")).toBe(
      false,
    );
  });
  ...
  it("requires both contract signatures on the same version and hash", () => {
    expect(requiredSignatureSides("awaiting_signatures")).toEqual([
      "band",
      "organiser",
    ]);
    expect(
  ...
  it("compares canonicalised terms independently of object insertion order", () => {
    expect(
      termsChanged(
        { guarantee_fee_cents: 100, proposed_stage_name: "Main" },
        { proposed_stage_name: "Main", guarantee_fee_cents: 100 },
      ),
    ).toBe(false);
  });
  ...
  it("hashes ordered setlist content and validates duplicates/duration", () => {
    ...
    expect(validateFestivalSetlist([...items, items[0]], 1000).valid).toBe(
      false,
    );
  });
  ...
  it("maps schedule conflict errors", () => {
    expect(mapBookingError(new Error("hard schedule conflict")).code).toBe(
      "schedule_conflict",
    );
  });
  ...
  it("projects public lineup without economics", () => {
    const projected = projectPublicLineup({
      ...
      status: "active",
      terms_snapshot: {
        guarantee_fee_cents: 100000,
        proposed_stage_name: "Main",
      },
    });
    expect(projected).toEqual({
      edition_id: "edition",
      band_id: "band",
      stage_display_name: "Main",
      slot_type: null,
  ...
  it("builds stable query keys", () => {
    expect(festivalBookingKeys.contracts("band", "contract")).toEqual([
      "festivals",
      "booking",
      "contracts",
      "band",
    ...
    expect(festivalBookingKeys.publicLineup("edition")).toEqual([
      "festivals",
      "booking",
      "public-lineup",
      "edition",
