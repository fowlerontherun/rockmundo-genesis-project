...
import { getFestivalBookingMode, warnIfCanonicalUsesLegacyMutation } from '../mode';

describe('festival booking mode branching', () => {
  it('routes canonical editions before legacy compatibility', () => {
    expect(getFestivalBookingMode({ editionId: 'edition', legacyId: 'legacy' })).toBe('canonical');
