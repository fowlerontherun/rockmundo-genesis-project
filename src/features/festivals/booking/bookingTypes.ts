...
export interface FestivalTerms
  extends PerformanceScheduleTerms, MoneyTerms, MerchandiseTerms {
...
export function mapBookingError(error: unknown): BookingDomainError {
  ...
  if (normalized.includes("stale offer revision"))
    return new BookingDomainError(message, "stale_revision");
  if (normalized.includes("expired"))
    return new BookingDomainError(message, "expired_offer");
  if (normalized.includes("reservation was lost"))
    return new BookingDomainError(message, "lost_reservation");
  if (normalized.includes("version mismatch"))
    return new BookingDomainError(message, "version_mismatch");
  if (normalized.includes("schedule") || normalized.includes("overlap"))
    return new BookingDomainError(message, "schedule_conflict");
  if (normalized.includes("idempotency key reused"))
    return new BookingDomainError(message, "idempotency_conflict");
  if (
    normalized.includes("not authorised") ||
    normalized.includes("authentication")
  )
    return new BookingDomainError(message, "unauthorised");
...
export const festivalBookingKeys = {
  ...
  publicLineup: (editionId?: string) =>
    ["festivals", "booking", "public-lineup", editionId ?? "all"] as const,
  bandWorkspace: (bandId?: string) =>
    ["festivals", "booking", "band-workspace", bandId ?? "none"] as const,
  organiserWorkspace: (editionId?: string) =>
    [
      "festivals",
      "booking",
      "organiser-workspace",
  ...
  organiserApplications: (editionId?: string) =>
    [
      "festivals",
      "booking",
  ...
  offers: (bandId?: string, editionId?: string) =>
    [
      "festivals",
      "booking",
      "offers",
  ...
  contracts: (bandId?: string, contractId?: string) =>
    [
      "festivals",
      "booking",
      "contracts",
  ...
  setlist: (contractId?: string) =>
    ["festivals", "booking", "setlist", contractId ?? "none"] as const,
};
...
export function canTransitionApplication(
  from: FestivalApplicationStatus,
  to: FestivalApplicationStatus,
): boolean {
...
export function canTransitionContract(
  from: FestivalContractStatus,
  to: FestivalContractStatus,
): boolean {
  return from === to || contractTransitions[from].includes(to);
}

const setlistTransitions: Record<
...
export function canTransitionSetlist(
  from: FestivalSetlistStatus,
  to: FestivalSetlistStatus,
): boolean {
...
export function canCounterOffer(
  ...
  return (
    ...
    ["sent", "viewed", "countered"].includes(status)
  );
}

export function canAcceptRevision(
  proposedBy: BookingSide,
  acceptingSide: BookingSide,
): boolean {
  return proposedBy !== acceptingSide;
}

export function requiredSignatureSides(
...
export function bothSidesSigned(
  signatures: Array<{
  ...
  const sides = new Set(
    signatures
      .filter(
        (signature) =>
          signature.contract_version === version &&
          (!termsHash || signature.terms_hash === termsHash),
      )
...
export function calculateTotalDuration(
  ...
  return items.reduce(
    (total, item) => total + item.planned_duration_seconds,
    0,
  );
}
...
export function validateFestivalSetlist(
  items: FestivalSetlistItemInput[],
  maximumDurationSeconds: number,
  ...
  if (totalDurationSeconds > maximumDurationSeconds)
    warnings.push("Setlist exceeds contracted duration.");
  if (totalDurationSeconds < minimumDurationSeconds)
    warnings.push("Setlist is shorter than the minimum performance duration.");
  items.forEach((item, index) => {
    if (!item.song_id) warnings.push(`Item ${index + 1} is missing a song.`);
    if (seen.has(item.song_id))
      warnings.push(`Item ${index + 1} duplicates a song.`);
    seen.add(item.song_id);
    if (item.planned_duration_seconds < 30)
      warnings.push(
        `Item ${index + 1} is shorter than the minimum song duration.`,
      );
  });
  return {
...
export function ensureCurrentRevision(expected: number, current: number): void {
  if (expected !== current)
    throw new BookingDomainError("Stale offer revision", "stale_revision");
}

export function selectCurrentRevision<
  T extends { id: string; revision_number: number },
>(revisions: T[], currentRevisionId?: string | null): T | undefined {
  return currentRevisionId
    ? revisions.find((revision) => revision.id === currentRevisionId)
    : [...revisions].sort((a, b) => b.revision_number - a.revision_number)[0];
}

export function selectCurrentSetlistVersion<
  T extends { version: number; is_current?: boolean | null },
...
export function projectPublicLineup(contract: {
  edition_id: string;
  band_id: string;
  status: FestivalContractStatus;
  terms_snapshot?: FestivalTerms;
}) {
  ...
  return {
    ...
    slot_type:
      typeof terms.proposed_slot_type === "string"
        ? terms.proposed_slot_type
        : null,
