...
export async function createFestivalOffer(input: {
  editionId: string;
  bandId: string;
...
export async function counterFestivalOffer(input: {
  offerId: string;
...
export async function acceptFestivalOffer(
  offerId: string,
  revision: number,
  idempotencyKey: string,
) {
...
export async function declineFestivalOffer(
  offerId: string,
  reason: string,
  idempotencyKey: string,
) {
...
export async function listFestivalOffers(bandId?: string, editionId?: string) {
  let query = supabase
    .from("festival_contract_offers")
    .select("*, festival_offer_revisions(*)");
