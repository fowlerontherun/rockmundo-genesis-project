...
export async function getBandApplications(bandId: string, editionId?: string) {
  let query = supabase.from('festival_applications').select('*').eq('band_id', bandId);
...
export async function submitFestivalApplication(input: FestivalApplicationInput) {
...
export async function withdrawFestivalApplication(applicationId: string, reason: string, idempotencyKey: string) {
...
export async function listOrganiserApplications(editionId: string) {
  const { data, error } = await supabase.from('festival_applications').select('*').eq('edition_id', editionId).order('created_at');
  if (error) throw mapBookingError(error);
  return data ?? [];
}
...
export async function reviewFestivalApplication(input: {
  ...
  action: 'move_to_review' | 'shortlist' | 'waitlist' | 'reject' | 'create_offer';
  reason?: string;
