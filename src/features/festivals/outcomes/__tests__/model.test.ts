...
describe("festival audience and outcome model", () => {
  ...
  it("scores readiness, songs and aggregate outcomes deterministically", () => { const readiness = scoreReadiness("ready"); const song = calculateSongScore({ readinessScore: readiness, crowdFit: 80, baseSkill: 75, incidentPenalty: 4, pacingBonus: 2, variation: 1 }); expect(song).toBeGreaterThan(65); expect(aggregatePerformanceScore([song, song - 10], readiness, 3, 1)).toBeGreaterThan(60); });
