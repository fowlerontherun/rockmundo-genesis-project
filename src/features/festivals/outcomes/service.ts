...
import type { FestivalAudienceGeneration, FestivalStageCrowdSnapshot } from "./model";
...
export async function listFestivalSongOutcomes(sessionId: string) { const { data, error } = await supabase.from("festival_song_performance_outcomes").select("*").eq("session_id", sessionId).order("setlist_position"); if (error) throw error; return data; }
export async function listFestivalHighlights(sessionId: string) { const { data, error } = await supabase.from("festival_performance_highlights").select("*").eq("session_id", sessionId).order("created_at"); if (error) throw error; return data; }
