...
import { FestivalAttendeeHealth } from "./FestivalAttendeeHealth";
...
export function FestivalRuntimeDashboard({ editionId }: { editionId: string }) {
    ...
    return (
      <Card>
        <CardContent className="p-6">
          Loading festival runtime status…
        </CardContent>
      </Card>
    );
  }
  ...
  const rawTicketsSold = ticketing?.tickets_sold ?? data.tickets_sold;
  const ticketsSold =
    typeof rawTicketsSold === "number" ? rawTicketsSold : null;
  const capacity = numberOrZero(ticketing?.capacity);
  const sellThrough =
    capacity > 0 && ticketsSold !== null
      ? Math.min(100, Math.round((ticketsSold / capacity) * 100))
      : null;
  const slots = data.slots ?? [];
  const liveSlots = slots.filter((slot) =>
    ["in_progress"].includes(
      textOrFallback(slot.performance_session_status, "").toLowerCase(),
    ),
  ).length;
  ...
  const blockedSlots = slots.filter((slot) =>
    ["no_show", "abandoned", "failed"].includes(
      textOrFallback(slot.performance_session_status, "").toLowerCase(),
    ),
  ).length;
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            A read-only operational view sourced from the canonical edition
            summary.
          </p>
        </div>
        <Button
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card>
          ...
          <CardContent className="text-sm text-muted-foreground">
            {textOrFallback(lifecycle?.start_at, "Start not reported")} →{" "}
            {textOrFallback(lifecycle?.end_at, "End not reported")}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="text-sm text-muted-foreground">
            {sellThrough !== null
              ? `${sellThrough}% of ${capacity.toLocaleString()} capacity`
              : "Verified sales source not reported"}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-2xl">
              {numberOrZero(schedule?.occupied_slots)}/
              {numberOrZero(schedule?.total_slots)}
            </CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            {numberOrZero(schedule?.open_slots)} open ·{" "}
            {numberOrZero(schedule?.contracted_acts)} contracted
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="text-sm text-muted-foreground">
            {data.staff.length} staff · {data.permit_requirements.length} permit
            records
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2">
            <div className="rounded border p-3">
              ...
              <p className="text-2xl font-semibold">
                {numberOrZero(schedule?.published_acts)}
              </p>
            </div>
            <div className="rounded border p-3">
              <p className="text-sm text-muted-foreground">System acts</p>
              <p className="text-2xl font-semibold">
                {numberOrZero(schedule?.system_acts)}
              </p>
            </div>
            <div className="rounded border p-3">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Confirms what the canonical operations endpoint allows this admin
              session to inspect.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            <div className="flex items-center justify-between gap-2">
              ...
              <Badge
                variant={data.permissions?.can_manage ? "default" : "secondary"}
              >
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <Badge
                variant={
                  data.finance_access === "granted" ? "default" : "secondary"
                }
              >
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <Badge
                variant={
                  data.data_health_access === "granted"
                    ? "default"
                    : "secondary"
                }
              >
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <Badge variant="outline">
                {textOrFallback(
                  ticketing?.source ?? data.ticket_summary_source,
                  "Not reported",
                )}
              </Badge>
            </div>
      ...
      <FestivalAttendeeHealth editionId={editionId} />
    </div>
  );
}
