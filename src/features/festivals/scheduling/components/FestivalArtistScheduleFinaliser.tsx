...
export function FestivalArtistScheduleFinaliser({ editionId }: { editionId: string }) {
  ...
  const [moveSlotByContract, setMoveSlotByContract] = useState<Record<string, string>>({});
  ...
  const moveBooking = useMutation({
    mutationFn: ({
      ...
      currentStageSlotId: string;
    }) => moveFestivalArtistBookingSlot({
      contractId,
      targetStageSlotId,
      expectedCurrentStageSlotId: currentStageSlotId,
    }),
    onSuccess: async (_, variables) => {
      setMoveSlotByContract((current) => ({ ...current, [variables.contractId]: "" }));
      await refreshSchedule();
      toast.success("Band moved in the festival running order");
    },
    onError: async () => {
      await refreshSchedule();
      toast.error("The band could not be moved. The lineup may have changed; choose another empty slot and retry.");
    },
  });
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Bands are ordered by their stage slot. Move a confirmed act to another compatible empty slot at any time before the show; the band is notified automatically.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-2">
          {lineup.length === 0 ? (
            ...
            const moveTarget = contractId ? moveSlotByContract[contractId] ?? "" : "";
            const compatibleMoveSlots = item.isNpcDj
              ? []
              : slots.filter((slot) => slotMinutes(slot) >= (item.allocatedSetMinutes ?? 0));

            return (
              <div key={item.id} className="grid gap-3 rounded border p-3 lg:grid-cols-[9rem_minmax(0,1fr)_minmax(0,1fr)_minmax(16rem,1.1fr)] lg:items-center">
                ...
                {item.isNpcDj ? (
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <Select
                      value={moveTarget}
                      disabled={compatibleMoveSlots.length === 0 || moveBooking.isPending}
                      onValueChange={(value) => setMoveSlotByContract((current) => ({ ...current, [contractId]: value }))}
                    >
                      <SelectTrigger className="min-w-0 flex-1">
                        <SelectValue placeholder={compatibleMoveSlots.length ? "Move to another slot" : "No compatible empty slots"} />
                      </SelectTrigger>
                      <SelectContent>
                        {compatibleMoveSlots.map((slot) => <SelectItem key={slot.id} value={slot.id}>{slotLabel(slot)}</SelectItem>)}
                      </SelectContent>
                    </Select>
                    <Button
                      variant="outline"
                      size="sm"
                      disabled={!moveTarget || moveBooking.isPending}
                      onClick={() => moveBooking.mutate({
                        contractId,
                        targetStageSlotId: moveTarget,
                        currentStageSlotId: item.id,
                      })}
                    >
                      {moveBooking.isPending ? "Moving…" : "Move"}
                    </Button>
                  </div>
                ) : (
                  <span className="text-xs text-muted-foreground">Schedule link unavailable</span>
                )}
              </div>
            );
          })}
