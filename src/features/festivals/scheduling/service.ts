...
export const lockSchedule = (input: { editionId: string; revisionId: string; reason: string }) => rpc("festival_schedule_lock", { p_edition_id: input.editionId, p_revision_id: input.revisionId, p_reason: input.reason, p_idempotency_key: `lock:${input.revisionId}:${Date.now()}` });
export const reopenSchedule = (input: { editionId: string; revisionId: string; reason: string }) => rpc("festival_schedule_reopen", { p_edition_id: input.editionId, p_revision_id: input.revisionId, p_reason: input.reason, p_idempotency_key: `reopen:${input.revisionId}:${Date.now()}` });
export const discardDraftSchedule = (input: { editionId: string; revisionId: string; reason?: string }) => rpc("festival_schedule_discard_draft", { p_edition_id: input.editionId, p_revision_id: input.revisionId, p_reason: input.reason ?? null, p_idempotency_key: `discard:${input.revisionId}:${Date.now()}` });

export const reorderFestivalStage = (input: { editionId: string; revisionId: string; stageId: string; date: string; expectedRevisionVersion: number; items: {id: string; version: number; startsAt?: string}[] }) => rpc("festival_schedule_reorder_stage", { p_edition_id: input.editionId, p_revision_id: input.revisionId, p_stage_id: input.stageId, p_festival_date: input.date, p_expected_revision_version: input.expectedRevisionVersion, p_items: input.items });
