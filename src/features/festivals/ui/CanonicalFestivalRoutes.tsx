...
type PublicHistory={dates?:{startsOn?:string;endsOn?:string};attendance?:number;audienceRating?:number;sellOut?:boolean;profitabilityBand?:string;completedAt?:string};
function PublicEditionHistory({editionId,slug,year}:{editionId:string;slug:string;year?:number}){const q=useQuery({queryKey:["public-festival-history",editionId],queryFn:async()=>{const {data,error}=await (supabase.rpc as unknown as (name:string,args:Record<string,unknown>)=>Promise<{data:unknown;error:{message:string}|null}>)("get_public_festival_edition_history",{p_edition_id:editionId});if(error)throw error;return data as PublicHistory|null}});return <main className="mx-auto max-w-5xl space-y-5 p-6"><nav aria-label="Breadcrumb"><Link to={festivalRoutes.publicCompany(slug)}>Festival company</Link> / Annual edition</nav><h1 className="text-3xl font-bold">Festival {year}</h1>{q.isLoading?<p role="status">Loading immutable edition history…</p>:!q.data?<p>This edition has no completed public history yet.</p>:<div className="grid gap-4 sm:grid-cols-2"><Summary title="Dates" value={`${q.data.dates?.startsOn??"—"} – ${q.data.dates?.endsOn??"—"}`}/><Summary title="Attendance" value={(q.data.attendance??0).toLocaleString("en-GB")}/><Summary title="Audience rating" value={`${q.data.audienceRating??"—"}/100`}/><Summary title="Result" value={`${q.data.sellOut?"Sold out · ":""}${(q.data.profitabilityBand??"not disclosed").replaceAll("_"," ")}`}/></div>}<p className="text-sm text-muted-foreground">This public snapshot is frozen at settlement completion. Private contracts, medical information and exact financial totals are not published.</p></main>}
