...
import { normalizeGigReplayCommerceSnapshot } from "../events/commerce";
...
export async function getGigViewerReplay(gigId: string): Promise<GigViewerReplayResult> {
  ...
  if (payload?.commerce != null && !commerce) {
    logGigExperienceFallback(
      gigId,
      "replay",
      "gig_viewer_replays.event_payload.commerce",
      { code: "MALFORMED_REPLAY_COMMERCE", message: `Replay ${row.id} has an invalid commerce snapshot` },
      "use the authoritative result report",
    );
