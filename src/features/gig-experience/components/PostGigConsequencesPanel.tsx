...
export function PostGigConsequencesPanel({ consequences }: { consequences: GigPostConsequencesDTO }) {
  ...
  return <Card>
    ...
    <CardContent className="space-y-4 text-sm">
      ...
      {visible.length > 0 ? <div className="space-y-2">{visible.map(c => <details key={`${c.key}-${c.targetId ?? 'target'}`} className="rounded-md border p-2"><summary className="cursor-pointer font-medium">{c.category.replaceAll('_', ' ')} · {c.status}{typeof c.deltaValue === 'number' ? ` (${c.deltaValue > 0 ? '+' : ''}${c.deltaValue})` : ''}</summary><p className="mt-1 text-muted-foreground">{c.explanation}</p><p className="mt-1 text-xs text-muted-foreground">Sources: {c.sourceFactors.join(', ') || 'stored result'}</p></details>)}</div> : null}
