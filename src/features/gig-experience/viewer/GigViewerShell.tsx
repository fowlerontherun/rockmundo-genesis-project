...
export function GigViewerShell({ gigId, experience, open, onViewResult, onClose }: { gigId: string; experience?: GigExperienceDTO | null; open: boolean; onViewResult: () => void; onClose: () => void }) {
  const query = useGigViewerReplay(gigId, open); const { reducedMotion, setReducedMotion } = useGigViewerPreferences();
...
function ReadyReplay({ replay, experience, reducedMotion, setReducedMotion, onViewResult, onClose }: any) {
  const playback = useGigReplayPlayback(replay); const state = playback.state; const story = useMemo(() => buildStoryModel(replay, experience), [replay, experience]); const snapshot = useMemo(() => state ? deriveStorySnapshot(story, state.positionMs, reducedMotion) : null, [story, state?.positionMs, reducedMotion]);
