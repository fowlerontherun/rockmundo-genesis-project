...
export function GigViewerShell(props: GigViewerShellProps) {
  ...
  return (
    <GigViewerErrorBoundary
      resetKey={`${props.gigId}:${props.replayOverride?.id ?? "stored"}`}
      onResult={resultAction}
      onClose={props.onClose}
    >
      <GigViewerShellContent {...props} />
    </GigViewerErrorBoundary>
  );
}

function GigViewerShellContent({ gigId, experience, open, onViewResult, onClose, replayOverride }: GigViewerShellProps) {
  ...
  if (result.state === "unsupported_version") return <GigViewerFallback title="Unsupported replay version" body="This stored replay uses a viewer or event-schema version this client does not support. Use the report or text timeline fallback." onResult={resultAction} onClose={onClose} />;
  if (result.state === "failed") return <GigViewerFallback title={result.reason === "malformed_replay" ? "Malformed replay" : "Replay generation failed"} body="The stored replay cannot be rendered safely. The authoritative result report remains available." onRetry={() => query.refetch()} onResult={resultAction} onClose={onClose} />;
  if (!result.replay) return <GigViewerFallback title="Malformed replay" body="Replay metadata loaded without a valid payload." onResult={resultAction} onClose={onClose} />;
...
function ReadyReplay({ replay, experience, open, prefs, onViewResult, onClose }: any) {
  ...
  const playback = useGigReplayPlayback(replay); const state = playback.state; const story = useMemo(() => buildStoryModel(replay, experience), [replay, experience]); const resultAction = replay.resultAvailable !== false && story.resultOffsetMs !== null ? onViewResult : undefined; const snapshot = useMemo(() => state ? deriveStorySnapshot(story, state.positionMs, reducedMotion) : null, [story, state?.positionMs, reducedMotion]); const audio = useGigViewerAudio({ experience, snapshot, replaySeed: replay.simulationSeed, isPlaying: !!state?.isPlaying, speed: playback.speed, open });
  ...
  return <GigViewerErrorBoundary onResult={resultAction} onClose={onClose} resetKey={replay.id}><div ref={popoutRef}><Card><CardHeader><CardTitle>{resultAction ? "Gig Replay" : "Gig Viewer"} · {StageTypeLabels[stageType]}</CardTitle></CardHeader><CardContent className="space-y-3">{controls}<GigViewerAudioControls audio={audio} /><GigViewerStatus state={state} attendance={metricNumber(experience?.headline.attendance)} capacity={experience?.gig.venue.capacity} replay={replay} reducedMotion={reducedMotion} /><GigCurrentSongPanel snapshot={snapshot} />{empty ? <GigViewerFallback title="Empty event sequence" body="The presentation contains no events. Retry later or use the authoritative report when it is ready." onResult={resultAction} onClose={onClose} /> : <div className="grid gap-4 lg:grid-cols-[minmax(0,2fr)_minmax(18rem,1fr)]"><div className="space-y-4">{canvas}<GigResultRevealOverlay visible={snapshot.resultVisible} experience={experience} story={story} onResult={onViewResult} onRestart={playback.restart} onClose={onClose} /><GigCrowdMoodGraph story={story} positionMs={state.positionMs} onSeek={playback.seekMs} /></div><div className="space-y-4"><GigPerformerPanel replay={replay} experience={experience} playbackState={state} reducedMotion={reducedMotion} /><GigViewerTimeline events={playback.events} activeId={state.activeEvent?.id} completedIds={state.completedEventIds} onSelect={playback.seekToEvent} story={story} /></div></div>}</CardContent></Card></div></GigViewerErrorBoundary>;
}
