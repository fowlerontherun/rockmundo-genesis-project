...
export function useGigViewerAudio({ experience, snapshot, replaySeed, isPlaying, speed, open, excerptDurationSeconds }: { experience?: GigExperienceDTO | null; snapshot: StorySnapshot | null; replaySeed: string; isPlaying: boolean; speed: PlaybackSpeed; open: boolean; excerptDurationSeconds?: number }) {
  ...
  useEffect(() => { if (!silentForSpeed) controller.current?.setPlaybackRate(speed); }, [speed, silentForSpeed]);
  useEffect(() => {
    if (silentForSpeed || !open || !snapshot?.song || snapshot.resultVisible || !source.available) { controller.current?.stop(); rerender((v)=>v+1); return; }
    ...
    rerender((v)=>v+1);
  }, [open, source, snapshot?.song, snapshot?.resultVisible, isPlaying, activated, prefs.enabled, prefs.muted, prefs.volume, speed, silentForSpeed]);
  ...
  return { source, status: controller.current.status, error: controller.current.error, enabled: prefs.enabled, muted: prefs.muted, volume: prefs.volume, ambience: prefs.ambience, setAmbience: (ambience: boolean) => setPrefs((p) => ({ ...p, ambience })), silentForSpeed, enable: async () => { setActivated(true); setPrefs((p) => ({ ...p, enabled: true, muted: false })); controller.current?.setVolume(prefs.volume, false); if (!silentForSpeed && source.available) await controller.current?.play().catch(()=>{}); rerender((v)=>v+1); }, disable: () => { setPrefs((p) => ({ ...p, enabled: false })); controller.current?.pause(); }, setMuted: (muted: boolean) => setPrefs((p) => ({ ...p, muted })), setVolume: (volume: number) => setPrefs((p) => ({ ...p, volume })) };
}
