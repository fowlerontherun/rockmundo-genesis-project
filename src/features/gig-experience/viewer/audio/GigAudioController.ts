...
export class GigAudioController {
  ...
  load(source: ResolvedGigAudioSource, volume: number, muted: boolean, playbackRate = 1) {
    const key = `${source.songId}:${source.url}:${source.excerptStartSeconds}:${source.excerptDurationSeconds}`;
  ...
  setPlaybackRate(playbackRate: number) { if (this.audio) this.audio.playbackRate = Math.max(0.25, Math.min(4, playbackRate)); }
