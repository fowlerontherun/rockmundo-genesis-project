...
export default function GigStage3D({ replay, experience, playbackState, reducedMotion, cameraMode, tier, archetype, tuning, pyrotechnics, pyroIntensity }: {
  ...
  const appearances = useGigPlayerModels(plan.entities.flatMap(p => p.profileId ? [p.profileId] : []));
  const options = useMemo(() => concertOptions(plan, appearances.data ?? EMPTY_APPEARANCES, replay, experience, archetype), [plan, appearances.data, replay, experience, archetype]);
  ...
  return <div className="relative h-full min-h-0 w-full bg-slate-950" data-renderer="three" data-renderer-status={status}>
    ...
    {appearances.isError && <div role="status" className="absolute bottom-3 left-3 right-3 rounded bg-slate-950/90 p-2 text-xs text-slate-200">Saved outfits could not load; starter models are shown. <button className="underline" onClick={() => void appearances.refetch()}>Reload outfits</button></div>}
    {status === 'ready' && !appearances.isError && <Link to="/avatar-designer" className="absolute bottom-3 right-3 rounded bg-black/60 px-3 py-2 text-xs text-white hover:bg-black/80">Edit my stage model</Link>}
  </div>;
}
