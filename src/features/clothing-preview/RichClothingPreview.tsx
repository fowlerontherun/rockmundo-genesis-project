...
function makePatternTexture(spec: ReturnType<typeof buildRichGarmentVisualSpec>) {
  ...
  ctx.fillStyle = spec.secondaryColor; ctx.strokeStyle = spec.secondaryColor; ctx.lineWidth = Math.max(4, 18 / spec.patternScale);
  ...
  texture.colorSpace = T.SRGBColorSpace; texture.wrapS = texture.wrapT = T.RepeatWrapping; texture.repeat.set(spec.patternScale, spec.patternScale); texture.rotation = T.MathUtils.degToRad(spec.patternRotation); texture.center.set(.5, .5);
  return texture;
}

function garmentMaterial(spec: ReturnType<typeof buildRichGarmentVisualSpec>, texture: T.Texture | null) {
  return new T.MeshPhysicalMaterial({ color: texture ? '#ffffff' : spec.primaryColor, map: texture, roughness: spec.roughness, metalness: spec.metalness, sheen: spec.sheen, sheenColor: new T.Color(spec.secondaryColor), clearcoat: /vinyl|latex|patent|sequins|metallic/i.test(spec.material) ? .42 : .06, clearcoatRoughness: Math.min(.8, spec.roughness) });
}
...
function addDetail(group: T.Group, detail: any, index: number, spec: ReturnType<typeof buildRichGarmentVisualSpec>) {
  ...
  mesh.position.set(x, spec.y + yOffset, z + spec.z); mesh.rotation.z = T.MathUtils.degToRad(Number(detail?.rotation || 0)); mesh.castShadow = true; group.add(mesh);
}

function buildGarment(item: ClothingItem, variant?: ClothingPreviewVariant) {
  const spec = buildRichGarmentVisualSpec(item, variant);
  const group = new T.Group(); group.name = `rich-garment-${item.id}`;
  const texture = makePatternTexture(spec); const material = garmentMaterial(spec, texture);
  const add = (mesh: T.Mesh) => { mesh.castShadow = true; mesh.receiveShadow = true; group.add(mesh); };
  if (spec.slot === 'top') {
    const body = new T.Mesh(new T.CapsuleGeometry(spec.scaleX * .48, spec.scaleY * .78, 8, 18), material); body.scale.set(1, 1, spec.scaleZ / Math.max(.01, spec.scaleX * .48)); body.position.set(0, spec.y, spec.z); add(body);
    const garment = (item.garment_config || {}) as Record<string, any>; const sleeves = String(garment.sleeve || garment.sleeveStyle || garment.sleeve_style || 'short').toLowerCase();
    if (sleeves !== 'sleeveless' && sleeves !== 'none') { const long = /long|full/.test(sleeves); const sleeveLength = long ? .58 : .28; for (const side of [-1, 1]) { const sleeve = new T.Mesh(new T.CapsuleGeometry(.105 * spec.scaleX, sleeveLength, 6, 12), material); sleeve.position.set(side * spec.scaleX * .55, spec.y + .08, spec.z); sleeve.rotation.z = side * -.18; add(sleeve); } }
  } else if (spec.slot === 'bottom') {
    const garment = (item.garment_config || {}) as Record<string, any>; const skirtLike = /skirt|dress|a-line|wide/.test(`${item.category} ${garment.silhouette || ''}`.toLowerCase());
    if (skirtLike) { const skirt = new T.Mesh(new T.CylinderGeometry(spec.scaleX * .42, spec.scaleX * (.5 + spec.flare), spec.scaleY, 28, 1, false), material); skirt.position.set(0, spec.y, spec.z); add(skirt); }
    else for (const side of [-1, 1]) { const leg = new T.Mesh(new T.CapsuleGeometry(spec.scaleX * .22, spec.scaleY, 6, 14), material); leg.position.set(side * spec.scaleX * .25, spec.y, spec.z); add(leg); }
    ...
    const crown = new T.Mesh(new T.CylinderGeometry(spec.scaleX * .55, spec.scaleX * .62, spec.scaleY, 28), material); crown.position.set(0, spec.y, spec.z); add(crown); const brim = new T.Mesh(new T.CylinderGeometry(spec.scaleX * .82, spec.scaleX * .82, .025, 32), material); brim.position.set(0, spec.y - spec.scaleY * .48, spec.z); add(brim);
  } else if (spec.slot === 'eyewear') {
    const frameMaterial = new T.MeshStandardMaterial({ color: spec.primaryColor, roughness: .28, metalness: .45 }); for (const side of [-1, 1]) { const lens = new T.Mesh(new T.TorusGeometry(.105, .012, 8, 20), frameMaterial); lens.position.set(side * .13, spec.y, .18 + spec.z); add(lens); } const bridge = new T.Mesh(new T.BoxGeometry(.08, .012, .012), frameMaterial); bridge.position.set(0, spec.y, .18 + spec.z); add(bridge);
  } else {
  ...
  const details = Array.isArray(item.detail_layers) ? item.detail_layers.slice(0, 18) : []; details.forEach((detail, index) => addDetail(group, detail, index, spec));
  if (spec.distress > .05) { const distressMaterial = new T.MeshBasicMaterial({ color: '#151515', transparent: true, opacity: Math.min(.5, .12 + spec.distress * .35), wireframe: true }); const distress = new T.Mesh(new T.SphereGeometry(Math.max(.3, spec.scaleX * .58), 12, 8), distressMaterial); distress.scale.set(1.15, 1.3, .58); distress.position.set(0, spec.y, spec.z + .04); group.add(distress); }
...
export function RichClothingPreview({ appearance, item, variant }: { appearance: PlayerAppearance; item: ClothingItem; variant?: ClothingPreviewVariant }) {
  const canvas = useRef<HTMLCanvasElement>(null), api = useRef<PreviewApi | null>(null);
  const [status, setStatus] = useState<'loading' | 'ready' | 'error'>('loading'), [attempt, setAttempt] = useState(0);
  useEffect(() => {
    if (!canvas.current) return;
    let alive = true, raf = 0, garment: T.Group | null = null, library: ModelLibrary | null = null;
    ...
    try {
      ...
      scene.background = new T.Color('#101823'); const pmrem = new T.PMREMGenerator(renderer), room = new RoomEnvironment(); environment = pmrem.fromScene(room, .04); scene.environment = environment.texture; room.dispose(); pmrem.dispose();
      ...
      void loadModelLibrary((['masculine', 'feminine'] as const).flatMap(frameType => STYLES.map(style => modelFile(frameType, style)))).then(loaded => {
        ...
        library = loaded; const base = assemblePlayerModel(library, appearance); scene.add(base); garment = buildGarment(item, variant); garment.scale.y *= appearance.body.height; scene.add(garment);
        api.current = { rotate: angle => { camera.position.sub(controls!.target).applyAxisAngle(new T.Vector3(0,1,0), angle).add(controls!.target); controls!.update(); }, zoom: factor => { const offset = camera.position.clone().sub(controls!.target); offset.setLength(T.MathUtils.clamp(offset.length() * factor, controls!.minDistance, controls!.maxDistance)); camera.position.copy(controls!.target).add(offset); controls!.update(); }, reset: () => controls!.reset() };
        setStatus('ready');
