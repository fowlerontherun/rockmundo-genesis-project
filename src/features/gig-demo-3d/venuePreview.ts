import { stageAssignment, type InstrumentId } from './instrumentCatalog';
...
export function venuePreviewOptions(type: VenueKind, capacity: number, instrument?: InstrumentId): ConcertOptions {
  ...
  return { externalClock: false, venue, performers: band.map(([name, role, u, v], i) => ({ id: `preview-${i}`, displayName: name, role, ...(instrument && i === 1 ? stageAssignment(instrument) : {}), phase: i, appearance: defaultAppearance(name), position: stagePosition(p,u,v) })) };
}
