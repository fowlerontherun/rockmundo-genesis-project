...
import { BookOpen, GraduationCap, Handshake, RefreshCw, ShieldCheck, Users } from "lucide-react";
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [skillResult, mentorProfileResult, mentorResult, mentorshipResult, classResult, myClassResult] = await Promise.all([
        db.from("skill_progress").select("skill_slug,current_level,current_xp").eq("profile_id", profile.id).order("current_level", { ascending: false }),
        db.from("community_mentorship_profiles").select("*").eq("profile_id", profile.id).maybeSingle(),
        db.rpc("get_community_mentor_discovery", { p_profile_id: profile.id }),
        db.rpc("get_my_community_mentorships", { p_profile_id: profile.id }),
        db.rpc("get_player_education_classes", { p_profile_id: profile.id }),
        db.rpc("get_my_player_education_classes", { p_profile_id: profile.id }),
      ]);
      ...
      setMyTeaching(myClassResult.data?.teaching ?? []);
      setMyLearning(myClassResult.data?.learning ?? []);
      ...
      setMessage(error?.message ?? "Could not load player learning network.");
    } finally {
      setBusy(false);
    }
  }, [profile?.id]);
            ...
            <CardContent className="grid gap-4 md:grid-cols-2">
              ...
              <div className="flex items-end gap-2">
                <Button
                  ...
                  onClick={() => void run(() => db.rpc("set_community_mentorship_profile", {
                ...
                {mentorProfile?.is_open_to_mentor && (
                  <Button variant="outline" disabled={busy} onClick={() => void run(() => db.rpc("set_community_mentorship_profile", {
                    p_profile_id: profile.id,
                    p_is_open: false,
                    p_headline: mentorHeadline,
                    p_style: "supportive",
              ...
              return (
                <Card key={mentor.mentorship_profile_id}>
                  ...
                  <CardContent className="space-y-3 text-sm">
                    ...
                    <Button className="w-full" disabled={busy || !focus} onClick={() => void run(() => db.rpc("request_community_mentorship", {
                ...
                <CardContent className="flex flex-col gap-4 pt-5 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {amMentor && match.status === "pending" && <>
                      <Button size="sm" disabled={busy} onClick={() => void run(() => db.rpc("respond_community_mentorship", { p_match_id: match.id, p_accept: true }), "Mentorship accepted.")}>Accept</Button>
                      <Button size="sm" variant="outline" disabled={busy} onClick={() => void run(() => db.rpc("respond_community_mentorship", { p_match_id: match.id, p_accept: false }), "Mentorship declined.")}>Decline</Button>
                    </>}
                    {match.status === "active" && <Button size="sm" variant="outline" disabled={busy} onClick={() => void run(() => db.rpc("process_community_mentorship_progress", { p_match_id: match.id }), "Verified learning progress checked.")}>Check milestones</Button>}
                    {(match.status === "pending" || match.status === "active") && <Button size="sm" variant="ghost" disabled={busy} onClick={() => void run(() => db.rpc("leave_community_mentorship", { p_match_id: match.id }), "Mentorship ended with no penalty.")}>Leave</Button>}
                  </div>
                </CardContent>
              </Card>
            );
        ...
        <TabsContent value="classes" className="space-y-5">
          <Card>
            ...
            <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="md:col-span-2 lg:col-span-3"><Button disabled={busy || !classSkill || !classTitle || !classDate} onClick={() => void run(() => db.rpc("create_player_education_class", {
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {classes.map((playerClass) => <Card key={playerClass.id}>
                ...
                <CardContent className="space-y-3 text-sm">
                  ...
                  <Button className="w-full" disabled={busy || playerClass.is_enrolled || playerClass.status !== "open"} onClick={() => void run(() => db.rpc("enrol_player_education_class", { p_class_id: playerClass.id, p_student_profile_id: profile.id }), "Enrolled in player class.")}>{playerClass.is_enrolled ? "Enrolled" : "Join class"}</Button>
                </CardContent>
              </Card>)}
            </div>
          </div>
                ...
                {myLearning.map((entry) => <div key={entry.id} className="rounded-md border p-3 text-sm">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {entry.status === "enrolled" && <>
                      <Button size="sm" variant="outline" disabled={busy} onClick={() => void run(() => db.rpc("check_in_player_education_class", { p_class_id: entry.class_id, p_student_profile_id: profile.id }), "Checked in to class.")}>Check in</Button>
                      <Button size="sm" variant="ghost" disabled={busy} onClick={() => void run(() => db.rpc("cancel_player_education_class_enrolment", { p_class_id: entry.class_id, p_student_profile_id: profile.id }), "Class enrolment cancelled.")}>Cancel</Button>
                    </>}
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {myTeaching.map((entry) => <div key={entry.id} className="rounded-md border p-3 text-sm">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {entry.status === "open" && <>
                      <Button size="sm" disabled={busy} onClick={() => void run(() => db.rpc("complete_player_education_class", { p_class_id: entry.id, p_teacher_profile_id: profile.id }), "Class settlement processed.")}>Complete / settle</Button>
                      <Button size="sm" variant="ghost" disabled={busy} onClick={() => void run(() => db.rpc("cancel_player_education_class", { p_class_id: entry.id, p_teacher_profile_id: profile.id }), "Class cancelled.")}>Cancel</Button>
                    </>}
                  </div>
                </div>)}
