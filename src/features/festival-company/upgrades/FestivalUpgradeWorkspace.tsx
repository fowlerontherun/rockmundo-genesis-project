...
import {FESTIVAL_UPGRADE_MESSAGES,type FestivalLicenceProgress,type FestivalUpgradeCategory,type FestivalUpgradePreview} from "./types";
...
interface UpgradeCardProps {category:FestivalUpgradeCategory;selected:boolean;quotaAvailable:boolean;onToggle:()=>void;preview:FestivalUpgradePreview|undefined;previewLoading:boolean;previewError:boolean;onPurchase:()=>void;purchasePending:boolean}

export function UpgradeCard({category:c,selected,quotaAvailable,onToggle,preview,previewLoading,previewError,onPurchase,purchasePending}:UpgradeCardProps) {
 ...
 {complete?<p className="font-medium">Maximum level reached. No further purchase is available.</p>:<><p>Next milestone: {c.nextMilestoneName} at level {c.nextMilestoneLevel} ({c.levelsUntilMilestone} levels away).</p><p>Next level: {c.nextLevel} · {money(c.nextCostMinor!)} · upkeep {money(c.nextUpkeepMinor!)} weekly · construction {c.buildDurationHours} hours.</p>{c.effectDelta&&<ul aria-label="Effect changes">{Object.entries(c.effectDelta).map(([key,value])=><li key={key}>{effectLabel(key)}: {String(value.current)} → {String(value.next)} ({value.kind==="number"?`${value.delta>=0?"+":""}${value.delta}`:value.changed?"changes":"unchanged"})</li>)}</ul>}{previewLoading?<p role="status">Loading server quote…</p>:previewError?<p role="alert">The server quote could not be loaded.</p>:preview&&<><ul aria-label="Purchase blockers">{preview.reasonCodes.map(code=><li key={code} className="text-destructive">{FESTIVAL_UPGRADE_MESSAGES[code]??code}</li>)}{preview.category.missingRequirements.map(x=><li key={x.code} className="text-destructive">{x.message}</li>)}</ul><p>Balance after purchase: {money(preview.remainingBalanceMinor)}. Purchases are final.</p><Button disabled={!preview.eligible||!quotaAvailable||purchasePending} onClick={onPurchase}>{purchasePending?"Purchasing…":"Purchase next level"}</Button></>}</>}</CardContent>}</Card>;
}
