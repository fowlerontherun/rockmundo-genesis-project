...
const FestivalHeader = ({
  ...
  <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur">
    <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-3 py-3 md:px-5">
      ...
      <Button variant="outline" size="sm" disabled={leavePending} onClick={onLeave}>
        {leavePending ? "Leaving…" : "Leave festival"}
      </Button>
    </div>
  </header>
);
...
export const FestivalModeShell = ({
  ...
  if (isMobile) {
    return (
      <div
        data-festival-mode="true"
        data-festival-mode-device="mobile"
        className="rm-mobile min-h-[100dvh] bg-background text-foreground"
      >
  ...
  return (
    <div
      data-festival-mode="true"
      data-festival-mode-device="desktop"
      ...
      <div className="mx-auto grid max-w-7xl gap-4 p-5 md:grid-cols-[230px_minmax(0,1fr)]">
        <nav aria-label="Festival navigation" className="self-start rounded-xl border bg-card p-2">
          ...
          <p className="mt-3 px-3 text-xs text-muted-foreground">
            While checked in, normal RockMundo gameplay navigation is intentionally unavailable.
          </p>
        </nav>
