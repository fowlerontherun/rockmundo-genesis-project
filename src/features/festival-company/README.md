...
Phase 4 part 1 (artist programme, application windows, applications, invitations, offers, bookings and budget commitments) is now deployed to the live database, with `get_festival_artist_programme` and `save_festival_artist_programme` available to owners and admins. Stage references were retargeted to the deployed `festival_site_plan_stages` table and the payload digest uses `extensions.digest`. Phase 4 part 2 (the artist workflow RPCs: candidate search, application submission and review, invitations, offer negotiation and booking cancellation) is written but not yet deployed.
