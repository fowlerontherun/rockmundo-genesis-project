...
const canonical = { festivalCompanyId: id, legalCompanyName: "Company Ltd", publicName: "Festival", shortName: "Fest", tagline: "Hello", description: "Description", homeCity: city, festivalScale: "small" as const, plannedStartDate: "2030-06-01", plannedEndDate: "2030-06-03", durationDays: 3, setupStatus: "ready_for_planning" as const, currentStep: 4, configurationVersion: 2, updatedAt: "2030-01-01T12:00:00Z", canWrite: true, scales: [scale], cities: [city] };
describe("festival configuration boundary", () => {
  ...
  it.each([
    ...
    ["step", { currentStep: 5 }], ["scale catalogue", { scales: [{ ...scale, maximumCapacity: 1 }] }],
  ...
  it("converts canonical responses without retaining submitted values", () => { const draft = configurationToDraft({ ...canonical, publicName: " Server normalised " }); expect(draft.publicName).toBe(" Server normalised "); expect(draft.currentStep).toBe(4); expect(draft.complete).toBe(true); });
...
describe("festival wizard validation", () => {
  ...
  it("permits review when all sections are valid", () => expect(maximumReachableStep(validateFestivalDraft(draft, [city], [scale], "2029-01-01"))).toBe(4));
