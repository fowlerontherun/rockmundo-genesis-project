  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        <div className="space-y-4 rounded-lg border bg-muted/40 p-6">
          ...
          <div className="grid gap-4 text-sm sm:grid-cols-2">
            <div>
              <p className="font-medium text-muted-foreground">Projected earnings</p>
              <p className="text-lg font-semibold text-foreground">${activeReward.cash}</p>
            </div>
            <div>
              <p className="font-medium text-muted-foreground">XP reward</p>
              <p className="text-lg font-semibold text-foreground">{activeReward.experience} XP</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">{lastResult.locationName}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">Duration</span>
                <span className="font-medium">{lastResult.duration} minutes</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">Session window</span>
                <span className="font-medium">
                  {formatSessionWindow(lastResult.startedAt, lastResult.endsAt)}
                </span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">Performance</span>
                <span className="font-medium">{lastResult.performanceDescriptor}</span>
              </div>
              <div className="flex items-center justify-between border-t pt-3 text-sm font-semibold">
                <span>Rewards</span>
                <span className="text-primary">
                  {lastResult.xpGained} XP + ${lastResult.cashEarned}
                </span>
              </div>
            </CardContent>
          </Card>
      ...
      <section>
        ...
        {showHistory && buskingHistory && buskingHistory.length > 0 ? (
          <div className="mt-3 space-y-2">
            {buskingHistory.map((activity) => (
              <div
                key={activity.id}
          ...
          <p className="mt-3 text-sm text-muted-foreground">No busking history yet.</p>
        ) : null}
      </section>
    </div>
  );
