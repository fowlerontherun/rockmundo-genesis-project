...
import { AlertTriangle, Ban, ShieldAlert, Undo2, Volume2, VolumeX } from "lucide-react";
...
import { usePlayerBlockActions, usePlayerMuteActions, useReportPlayer, useSocialPermission } from "@/hooks/useSocialSafety";

export function SafetyActions({ targetProfileId, targetName = "this player", isBlockedByViewer = false, isMutedByViewer = false, compact = false }: { targetProfileId: string; targetName?: string; isBlockedByViewer?: boolean; isMutedByViewer?: boolean; compact?: boolean }) {
  ...
  const permission = useSocialPermission(targetProfileId);
  const actions = usePlayerBlockActions(targetProfileId);
  const muteActions = usePlayerMuteActions(targetProfileId);
  const report = useReportPlayer();
  const blocked = permission.data?.is_blocked_by_viewer ?? isBlockedByViewer;
  const muted = permission.data?.is_muted_by_viewer ?? isMutedByViewer;
  ...
  return <>
    {blocked ? <Button size="sm" variant="outline" onClick={() => window.confirm("Unblock this player? Previous friendships and requests will not be restored.") && actions.unblock.mutate()} disabled={actions.unblock.isPending}><Undo2 className="mr-1 h-4 w-4" />Unblock player</Button> : <Button size="sm" variant={compact ? "outline" : "destructive"} onClick={() => setBlockOpen(true)} disabled={actions.block.isPending}><Ban className="mr-1 h-4 w-4" />Block player</Button>}
    {!blocked && (muted ? <Button size="sm" variant="outline" onClick={() => muteActions.unmute.mutate()} disabled={muteActions.unmute.isPending}><Volume2 className="mr-1 h-4 w-4" />Unmute</Button> : <Button size="sm" variant="outline" onClick={() => muteActions.mute.mutate()} disabled={muteActions.mute.isPending}><VolumeX className="mr-1 h-4 w-4" />Mute</Button>)}
