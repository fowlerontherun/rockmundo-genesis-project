...
import { usePlayerBlockActions, usePlayerMuteActions, useReportPlayer } from "@/hooks/useSocialSafety";
...
export function SafetyActions({ targetProfileId, targetName = "this player", isBlockedByViewer = false, isMutedByViewer = false, compact = false }: { targetProfileId: string; targetName?: string; isBlockedByViewer?: boolean; isMutedByViewer?: boolean; compact?: boolean }) {
  ...
  return <>
    ...
    {!isBlockedByViewer && (isMutedByViewer ? <Button size="sm" variant="outline" onClick={() => muteActions.unmute.mutate()} disabled={muteActions.unmute.isPending}><Volume2 className="mr-1 h-4 w-4" />Unmute</Button> : <Button size="sm" variant="outline" onClick={() => muteActions.mute.mutate()} disabled={muteActions.mute.isPending}><VolumeX className="mr-1 h-4 w-4" />Mute</Button>)}
