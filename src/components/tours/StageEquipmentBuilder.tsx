...
const RATING_COLORS = ['bg-destructive', // 0-19'bg-orange-500', // 20-39'bg-yellow-500', // 40-59'bg-green-500', // 60-79'bg-primary', // 80-100
];
...
export function StageEquipmentBuilder({ selections, onChange, bandFame, showCount }: StageEquipmentBuilderProps) {
 ...
 <div>
 <p className="text-xs text-muted-foreground">Production Rating</p>
 ...
 <button
 className="w-full flex items-center justify-between p-3 text-left hover:bg-muted/50 transition-colors"onClick={() => setExpandedCategory(isExpanded ? null : cat.type)}
 >
