...
type RecordingReward = { awarded: boolean; score: number; quality_bonus: number; final_master_quality: number };
...
export function ActiveRecordingDialog({ open, onOpenChange, recordingSessionId, songTitle, onSaved }: ActiveRecordingDialogProps) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const { data, error } = await rpcClient.rpc("submit_active_recording_session", { p_profile_id: profileId, p_recording_session_id: recordingSessionId, p_score: 0, p_perfect_takes: gradeCounts.perfect, p_good_takes: gradeCounts.good, p_rough_takes: gradeCounts.rough });
  ...
  const reset = () => { setSection(0); setMeter(0); setDirection(1); setCaptured(null); setKept([]); setRetakesLeft(MAX_RETAKES); };
  const finished = kept.length === SECTIONS.length;
  const currentSection = SECTIONS[Math.min(section, SECTIONS.length - 1)];

  return <Dialog open={open} onOpenChange={(value) => { onOpenChange(value); if (!value) reset(); }}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle className="flex items-center gap-2"><Disc3 className="h-5 w-5 text-primary" />Active Recording</DialogTitle><DialogDescription>Capture five keeper takes for {songTitle}. This is optional and can add at most +2 to this completed standard master; it does not change the underlying song composition quality.</DialogDescription></DialogHeader><div className="space-y-5">
    <div className="flex items-center justify-between text-sm"><span>{finished ? "Session complete" : currentSection}</span><span>Retakes: {retakesLeft}</span></div><Progress value={(kept.length / SECTIONS.length) * 100} />
    {!finished && !captured && <div className="space-y-4"><div className="relative h-20 rounded-lg border bg-muted/30 overflow-hidden"><div className="absolute inset-y-0 left-[42%] right-[42%] bg-green-500/15 border-x border-green-500/40" /><div className="absolute inset-y-0 left-1/2 w-0.5 bg-green-500" /><div className="absolute top-1/2 h-12 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-foreground shadow-lg" style={{ left: `${meter}%` }} /></div><p className="text-center text-sm text-muted-foreground">Watch the input level and capture when the signal sits cleanly in the centre sweet spot.</p><Button size="lg" className="w-full h-14" onClick={captureTake}>CAPTURE TAKE</Button></div>}
    ...
    {finished && <div className="space-y-4"><div className="text-center space-y-1"><Sparkles className="h-7 w-7 mx-auto text-primary" /><p className="text-sm text-muted-foreground">Keeper-take grade score</p><p className="text-5xl font-black">{rewardScore}</p><p className="text-sm text-muted-foreground">90+ = +2 master quality · 70+ = +1 · below 70 = no bonus</p></div><div className="grid grid-cols-5 gap-1">{kept.map((take, index) => <div key={`${SECTIONS[index]}-${index}`} className="rounded border p-2 text-center"><p className="text-[10px] text-muted-foreground truncate">{SECTIONS[index]}</p><p className="font-semibold text-sm">{take.score}</p></div>)}</div><Button className="w-full" onClick={save} disabled={saving}>{saving ? "Saving…" : "Apply Take Polish"}</Button></div>}
