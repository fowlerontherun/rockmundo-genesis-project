...
const parseAchievementRow = (value: unknown): Achievement | null => {
  ...
  if (!id || !name || !category || !rarity) return null;
  return {
    id,
    name,
    description,
...
export function AchievementsSection() {
  ...
  const loadAchievements = useCallback(async () => {
    try {
      ...
      const parsedAchievements = rows.reduce<Achievement[]>((acc, item) => {
        ...
        if (achievement) acc.push(achievement);
        return acc;
      }, []);
  ...
  const loadPlayerAchievements = useCallback(async () => {
    ...
    try {
      ...
      const parsedPlayerAchievements = rows.reduce<PlayerAchievement[]>((acc, item) => {
        ...
        if (!achievement || !id || !achievementId) return acc;
        acc.push({
          id,
  ...
  const formatRequirements = (requirements: AchievementRequirements) => {
    return Object.entries(requirements).map(([key, value]) => {
      switch (key) {
        case "level": return `Reach level ${formatBasicValue(value)}`;
        case "guitar_skill": return `Reach ${formatBasicValue(value)} guitar skill`;
        case "vocals_skill": return `Reach ${formatBasicValue(value)} vocals skill`;
        case "total_spent": return `Spend $${formatCurrencyValue(value)}`;
        case "total_cash": return `Accumulate $${formatCurrencyValue(value)}`;
        case "total_earned": return `Earn $${formatCurrencyValue(value)}`;
        case "chart_position": return `Reach #${formatBasicValue(value)} on charts`;
        case "join": return "Join RockMundo";
        case "twaats_posted": return `Post ${formatBasicValue(value)} twaats`;
        case "twaat_likes": return `Get ${formatBasicValue(value)}+ likes on a twaat`;
        case "twaater_followers": return `Reach ${formatCurrencyValue(value)} Twaater followers`;
        case "dikcok_videos": return `Create ${formatBasicValue(value)} DikCok videos`;
        case "dikcok_views": return `Get ${formatCurrencyValue(value)} total DikCok views`;
        case "gigs_played": return `Play ${formatBasicValue(value)} gigs`;
        case "countries_played": return `Play in ${formatBasicValue(value)} countries`;
        case "festival_headline": return `Headline a festival`;
        case "gig_satisfaction": return `Get ${formatBasicValue(value)}%+ gig satisfaction`;
        case "releases": return `Release ${formatBasicValue(value)} release(s)`;
        case "albums_released": return `Release ${formatBasicValue(value)} album(s)`;
        case "singles_released": return `Release ${formatBasicValue(value)} single(s)`;
        case "single_release_sales": return `Sell ${formatCurrencyValue(value)} copies`;
        case "release_hype": return `Get ${formatBasicValue(value)}+ hype score`;
        case "media_interviews": return `Complete ${formatBasicValue(value)} media interview(s)`;
        case "media_features": return `Get featured in ${formatBasicValue(value)} outlets`;
        case "magazine_covers": return `Land ${formatBasicValue(value)} magazine cover(s)`;
        case "label_contracts": return `Sign ${formatBasicValue(value)} label contract(s)`;
        case "awards_won": return `Win ${formatBasicValue(value)} award(s)`;
        case "ceremonies_attended": return `Attend ${formatBasicValue(value)} ceremonies`;
        case "chart_position:1": return "Reach #1 on charts";
        case "duration": return `Hold for a ${formatBasicValue(value)}`;
        default: return `${key}: ${formatBasicValue(value)}`;
      }
    }).join(", ");
  };
  ...
  const formatRewards = (rewards: AchievementRewards) => {
    return Object.entries(rewards).map(([key, value]) => {
      switch (key) {
        case "experience": return `+${formatBasicValue(value)} XP`;
        case "cash": return `+$${formatCurrencyValue(value)}`;
        case "fame": return `+${formatBasicValue(value)} Fame`;
        default: return `${key}: ${formatBasicValue(value)}`;
      }
    }).join(", ");
  };
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="all">
                    ...
                    <Card key={achievement.id} className={unlocked ? '' : 'opacity-75'}>
                      <CardHeader>
                        <div className="flex items-start gap-4">
                          <div className={`p-3 rounded-full ${unlocked ? 'bg-primary/20 text-primary' : 'bg-muted text-muted-foreground'}`}>
                            {unlocked ? <CheckCircle className="h-8 w-8" /> : <Lock className="h-8 w-8" />}
                          </div>
                      ...
                      <CardContent className="space-y-3">
                        <div>
                          ...
                          <div className="text-sm">{formatRequirements(achievement.requirements)}</div>
                        </div>
                        <div>
                          <div className="text-sm font-medium text-muted-foreground mb-1">Rewards:</div>
                          <div className="text-sm text-success">{formatRewards(achievement.rewards)}</div>
                        </div>
        ...
        <TabsContent value="unlocked" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {achievements.filter(achievement => isUnlocked(achievement.id)).map((achievement) => {
              const unlockedDate = getUnlockedDate(achievement.id);
              return (
        ...
        <TabsContent value="rare" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {achievements.filter(achievement => ["rare", "epic", "legendary"].includes(achievement.rarity)).map((achievement) => {
              const unlocked = isUnlocked(achievement.id);
              const unlockedDate = getUnlockedDate(achievement.id);
              return (
                <Card key={achievement.id} className={unlocked ? '' : 'opacity-75'}>
