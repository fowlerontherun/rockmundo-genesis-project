...
export const SongNotesDisplay = ({
  ...
  const autoNotes = useMemo(() => {
    ...
    if (creativeBrief?.co_writers?.length) {
      const coWriterNames = creativeBrief.co_writers
        .map((w) => w.name)
        .join(",");
      parts.push({
    ...
    if (creativeBrief?.writing_mode) {
      ...
      parts.push({
        ...
        label: "Mode",
        value:
          modeLabels[creativeBrief.writing_mode] || creativeBrief.writing_mode,
      });
    }
  ...
  return (
    <div className="space-y-3">
      <Label className="flex items-center gap-2">📝 Song Notes</Label>
      ...
      <div className="rounded-md border bg-muted/30 p-3 space-y-2">
        {autoNotes.length > 0 ? (
          ...
          <p className="text-sm text-muted-foreground italic">
            Select genre, theme, and creative options to auto-populate song
            notes.
          </p>
        )}
      </div>
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          These notes will be used to guide AI lyrics generation and help you
          remember your creative vision.
        </p>
      </div>
    </div>
  );
...
export function generateSongNotesForAI(
  ...
  if (theme?.name)
    parts.push(`Theme: ${theme.name}${theme.mood ? ` - ${theme.mood}` : ""}`);
  if (theme?.description) parts.push(`Theme Description: ${theme.description}`);
  if (chordProgression?.name)
    parts.push(`Chord Progression: ${chordProgression.name}`);
  if (chordProgression?.progression)
    parts.push(`Chords: ${chordProgression.progression}`);
  if (creativeBrief?.mood_modifiers?.length)
    parts.push(`Mood: ${creativeBrief.mood_modifiers.join(",")}`);
  if (creativeBrief?.inspiration_modifiers?.length)
    parts.push(`Inspiration: ${creativeBrief.inspiration_modifiers.join(",")}`);
  if (creativeBrief?.co_writers?.length) {
    const writers = creativeBrief.co_writers
  ...
  if (creativeBrief?.session_musicians?.length)
    parts.push(
      `Session Musicians: ${creativeBrief.session_musicians.join(",")}`,
    );
