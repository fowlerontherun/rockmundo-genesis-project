      ...
      <CardContent className="p-3">
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs text-muted-foreground capitalize">
            {item.wearable_slot || item.category}
          </span>
          {rarity !== "common" && (
        ...
        {bonuses.length > 0 && (
          <div className="mb-3 rounded-md border border-primary/20 bg-primary/5 p-2">
            <div className="mb-1.5 flex items-center gap-1 text-[11px] font-semibold uppercase tracking-wide text-primary">
              <Sparkles className="h-3 w-3" />
              Equipped bonuses
            </div>
            <div className="flex flex-wrap gap-1">
              {bonuses.map((bonus) => (
                <Badge key={bonus.key} variant="secondary" className="gap-1 text-[10px] font-normal">
                  {bonus.icon}
                  {bonus.label}
                </Badge>
              ))}
            ...
            <p className="mt-1.5 text-[10px] leading-snug text-muted-foreground">
              Bonuses apply only while this item is equipped and are subject to outfit-wide caps.
            </p>
          </div>
        )}
