...
export function ReleasePipelineTab({
  artistEntities,
  territories,
}: ReleasePipelineTabProps) {
  const soloEntity = artistEntities.find((entity) => entity.type === "solo");
  const bandEntities = artistEntities.filter(
    (entity) => entity.type === "band",
  );
  const [createReleaseOpen, setCreateReleaseOpen] = useState(false);
  const [promotionDialogOpen, setPromotionDialogOpen] = useState(false);
  const [selectedRelease, setSelectedRelease] =
    useState<ReleaseWithRelations | null>(null);
  ...
  const { data, isLoading } = useQuery<{
    ...
    queryKey: [
      "label-releases",
      soloEntity?.id,
      bandEntities.map((band) => band.id).join(","),
    ],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Coordinate manufacturing, promotion, and territory rollout for every
            signed contract.
          </p>
        </div>
      ...
      {releases.length === 0 ? (
        <Card>
          <CardContent className="p-6 text-center text-sm text-muted-foreground">
            No upcoming releases yet. Start by planning a new single or album
            for your active contracts.
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-4 lg:grid-cols-2">
          {releases.map((release) => {
            const contract =
              contractLookup.get(release.contract_id) ??
              release.contract ??
              null;
            return (
              <Card key={release.id} className="flex flex-col">
                <CardHeader className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      variant={
                        STATUS_BADGES[release.status ?? "planning"] ?? "outline"
                      }
                    >
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
                    ...
                    {contract?.labels?.name ? (
                      <span>· {contract.labels.name}</span>
                    ) : null}
                    {contract?.releases_completed !== undefined &&
                    contract?.release_quota !== undefined ? (
                      <span>
                        · {contract.releases_completed}/{contract.release_quota}{" "}
                        releases delivered
                      </span>
                    ) : null}
                  </div>
                ...
                <CardContent className="flex-1 space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="text-sm text-muted-foreground">
                        Promotion budget $
                        {release.promotion_budget?.toLocaleString() ?? "0"}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        Masters cost $
                        {release.masters_cost?.toLocaleString() ?? "0"}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {release.label_promotion_campaigns &&
                    release.label_promotion_campaigns.length > 0 ? (
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleOpenPromotionDialog(release)}
                    >
                      Add promotion campaign
                    </Button>
                  </div>

                  {release.territory_strategy &&
                  Array.isArray(release.territory_strategy) ? (
                    <div className="space-y-2">
                      <div className="text-sm font-semibold">
                        Territory rollout
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {(release.territory_strategy as string[]).map(
                          (code) => {
