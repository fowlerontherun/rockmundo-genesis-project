...
export function FormatSelectionStep({
  ...
  const [formatConfigs, setFormatConfigs] = useState<Record<string, any>>({
    digital: {
      release_date: "",
      quantity: 0,
      retail_price: DEFAULT_RETAIL_PRICES.digital,
      distribution_fee_percentage: 30,
    },
    streaming: {
      release_date: "",
      quantity: 0,
      retail_price: DEFAULT_RETAIL_PRICES.streaming,
      distribution_fee_percentage: 30,
    },
    cd: {
      release_date: "",
      quantity: 100,
      retail_price: DEFAULT_RETAIL_PRICES.cd,
      distribution_fee_percentage: 30,
    },
    vinyl: {
      release_date: "",
      quantity: 100,
      retail_price: DEFAULT_RETAIL_PRICES.vinyl,
      distribution_fee_percentage: 30,
      vinyl_color: "black",
    ...
    cassette: {
      release_date: "",
      quantity: 100,
      retail_price: DEFAULT_RETAIL_PRICES.cassette,
      distribution_fee_percentage: 30,
    },
  });
  ...
  const projectFormatPL = (formatType: string) => {
    ...
    const distRate =
      Math.max(0, Math.min(50, cfg.distribution_fee_percentage ?? 30)) / 100;
    const taxRate = 0.1;
    const netRevenueDollars = grossDollars * (1 - taxRate - distRate);
    const mfgCostDollars =
      (selectedFormats.find((f) => f.format_type === formatType)
        ?.manufacturing_cost || 0) / 100;
  ...
  const calculateManufacturingCost = (formatType: string, quantity: number) => {
    ...
    if (formatType === "digital" || formatType === "streaming") {
      return isFirstRelease
        ? DIGITAL_FIRST_RELEASE_FEE
        : DIGITAL_SUBSEQUENT_FEE;
    }

    const costs =
      manufacturingCosts?.filter((c) => c.format_type === formatType) || [];
    const tier = costs.find(
      (c) =>
        quantity >= c.min_quantity &&
        (c.max_quantity === null || quantity <= c.max_quantity),
    );
  ...
  const getVolumeDiscountInfo = (formatType: string, quantity: number) => {
    ...
    const costs =
      manufacturingCosts
        ?.filter((c) => c.format_type === formatType)
        .sort((a, b) => a.min_quantity - b.min_quantity) || [];
    if (costs.length === 0) return null;

    const baseTier = costs[0];
    const currentTier = costs.find(
      (c) =>
        quantity >= c.min_quantity &&
        (c.max_quantity === null || quantity <= c.max_quantity),
    );
    ...
    const discountPercent = Math.round(
      (1 - currentTier.cost_per_unit / baseTier.cost_per_unit) * 100,
    );
  ...
  const isScheduledTooEarly =
    scheduledReleaseDate &&
    isBefore(scheduledReleaseDate, manufacturingCompleteDate);
  ...
  const toggleFormat = (formatType: string) => {
    const isSelected = selectedFormats.some(
      (f) => f.format_type === formatType,
    );

    if (isSelected) {
      onFormatsChange(
        selectedFormats.filter((f) => f.format_type !== formatType),
      );
    } else {
  ...
  const handleRevenueShareToggle = (checked: boolean) => {
    ...
    const updatedFormats = selectedFormats.map((f) => ({
      ...f,
      manufacturing_cost: calculateManufacturingCostWithShare(
        f.format_type,
        f.quantity || 0,
        checked,
      ),
  ...
  const calculateManufacturingCostWithShare = (
    formatType: string,
    quantity: number,
  ...
  const totalCost = selectedFormats.reduce(
    (sum, f) => sum + f.manufacturing_cost,
    0,
  );
  const originalCost = selectedFormats.reduce((sum, f) => {
    const baseCost = calculateManufacturingCostWithShare(
      f.format_type,
      f.quantity || 0,
      false,
    );
    ...
    <div className="space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">
                Reduce manufacturing costs by <strong>50%</strong> by sharing{" "}
                <strong>10%</strong> of sales revenue with manufacturers.
              </p>
              {revenueShareEnabled && (
                <p className="text-xs text-primary mt-2">
                  Manufacturing partners will receive 10% of all physical sales
                  revenue.
                </p>
              )}
            </div>
      ...
      {selectedFormats.length > 0 && (
        <Alert className="bg-muted/50">
          <AlertDescription className="text-sm">
            Manufacturing will complete by{" "}
            <strong>
              {formatDate(manufacturingCompleteDate, "MMM d, yyyy")}
            </strong>
      ...
      {isScheduledTooEarly && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Your scheduled release date is before manufacturing completes.
            Consider pushing back to after{" "}
            {formatDate(manufacturingCompleteDate, "MMM d, yyyy")}.
          </AlertDescription>
        </Alert>
      )}
                  ...
                  {isSelected && (
                    ...
                    >
                          ...
                          {formatConfigs.vinyl.is_limited_edition && (
                            <div>
                              ...
                              <Input
                                type="number"
                                value={
                                  formatConfigs.vinyl.edition_quantity ||
                                  formatConfigs.vinyl.quantity
                                }
                                onChange={(e) =>
                                  updateFormatConfig(
                                    "vinyl",
                      ...
                      <div>
                        ...
                        {formatConfigs[fmt.type].retail_price > 0 && (
                          <div className="text-xs text-muted-foreground mt-1">
                            €
                            {(
                              formatConfigs[fmt.type].retail_price *
                              CURRENCY_RATES.EUR
                            ).toFixed(2)}{" "}
                            | £
                            {(
                      ...
                      <div className="text-xs space-y-1">
                        {(() => {
                          const discountInfo = getVolumeDiscountInfo(
                            fmt.type,
                            formatConfigs[fmt.type].quantity,
                          );
                          const cost =
                            ...
                            <>
                                ...
                                {discountInfo &&
                                  discountInfo.discountPercent > 0 && (
                                    <span className="text-primary ml-1">
                                      ({discountInfo.discountPercent}% volume
                                      discount)
                                    </span>
                                  )}
                              </div>
                              {discountInfo && (
                                <div className="text-muted-foreground">
                                  ${(discountInfo.unitPrice / 100).toFixed(2)}
                                  /unit
                                  {discountInfo.nextTierQty && (
                                    <span className="ml-1">
                                      • Order {discountInfo.nextTierQty}+ for $
                                      {(
                                        discountInfo.nextTierPrice! / 100
                                      ).toFixed(2)}
                                      /unit
                                    </span>
                                  )}
                      ...
                      {(() => {
                        ...
                        return (
                          <div
                            className={`rounded-md border p-2 text-xs space-y-0.5 ${isLoss ? "border-destructive/50 bg-destructive/5" : "border-primary/30 bg-primary/5"}`}
                          >
                            <div className="flex justify-between font-medium">
                              ...
                              >
                                {isLoss ? "−" : "+"}$
                                {Math.abs(pl.projectedNet).toFixed(0)}
                              </span>
                            </div>
                            <div className="text-muted-foreground">
                              Expected sales:{" "}
                              {pl.expectedUnits.toLocaleString()} units (
                              {Math.round(pl.sellThrough * 100)}%)
                            </div>
                            <div className="text-muted-foreground">
                              Gross ${pl.grossDollars.toFixed(0)} − tax/dist =
                              net ${pl.netRevenueDollars.toFixed(0)} − mfg $
                              {pl.mfgCostDollars.toFixed(0)}
                            </div>
                            {overpressed && (
                              <div className="text-destructive font-medium pt-1">
                                ⚠ Pressing 10× projected sales — likely loss.
                                Reduce quantity or raise price.
                              </div>
                            )}
                          </div>
                        );
          ...
          <div className="text-right">
            ...
            {revenueShareEnabled && (
              <p className="text-xs text-primary">
                50% discount applied + 10% revenue share
              </p>
            )}
          </div>
        </div>
          ...
          <div className="border-t border-border pt-3 space-y-1.5">
            ...
            {songCount > 0 && totalCost > 0 && (
              <div className="flex justify-between text-sm border-t border-border/50 pt-1.5 mt-1.5">
                <span className="text-muted-foreground">
                  Cost per track ({songCount} song{songCount !== 1 ? "s" : ""})
                </span>
                <span className="font-medium">
                  ${(totalCost / 100 / songCount).toFixed(2)}
                </span>
              </div>
            )}
          </div>
