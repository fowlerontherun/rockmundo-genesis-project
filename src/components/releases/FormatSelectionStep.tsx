...
export function FormatSelectionStep({
 ...
 <Input
 type="date"value={formatConfigs[fmt.type].release_date}
 onChange={(e) => updateFormatConfig(fmt.type,"release_date", e.target.value)}
 className="h-8"min={formatDate(manufacturingCompleteDate,"yyyy-MM-dd")}
 />
 </div>
 ...
 <Input
 type="number"value={formatConfigs[fmt.type].quantity}
 ...
 <Input
 type="number"value={formatConfigs.vinyl.edition_quantity || formatConfigs.vinyl.quantity}
 onChange={(e) => updateFormatConfig("vinyl","edition_quantity", e.target.value)}
 min="1"max={formatConfigs.vinyl.quantity}
 className="h-8"/>
 </div>
 )}
 </>
 ...
 <Input
 type="number"step="0.01"value={formatConfigs[fmt.type].retail_price}
 ...
 const overpressed = fmt.type !=="digital"&& fmt.type !=="streaming"&& formatConfigs[fmt.type].quantity > pl.expectedUnits * 10;
 return (
 ...
 <p className="text-xs font-medium text-muted-foreground tracking-wide">Cost Breakdown</p>
