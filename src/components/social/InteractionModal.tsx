...
function ImpactPreview({ impacts }: { impacts: { label: string; change: number }[] }) {
 ...
 <Badge
 key={i}
 variant="outline"className={cn("text-[10px] gap-0.5",
...
export function InteractionModal({
 ...
 // Result screen
 <motion.div
 key="result"initial={{ opacity: 0, scale: 0.95 }}
 animate={{ opacity: 1, scale: 1 }}
 exit={{ opacity: 0 }}
 className="space-y-4 py-4">
 <div className={cn("flex items-center gap-3 p-4 rounded-lg border",
 result.success
 ?"bg-success/10 border-success/30":"bg-social-tension/10 border-social-tension/30",
 )}>
 ...
 <span className="text-xs font-semibold text-muted-foreground">Consequences</span>
 ...
 className={cn("flex items-center justify-between p-2 rounded border text-sm",
 impact.change > 0 ?"border-success/20":"border-social-tension/20",
 )}
 >
 <span>{impact.label}</span>
 <span className={cn("font-oswald font-bold",
 ...
 className={cn("w-full text-left p-3 rounded-lg border border-border/50 bg-muted/20","hover:bg-muted/40 hover:border-primary/30 transition-all duration-200","disabled:opacity-40 disabled:cursor-not-allowed",
