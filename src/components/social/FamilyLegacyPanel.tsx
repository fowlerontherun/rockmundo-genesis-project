...
export function FamilyLegacyPanel({ familyMembers = [], legacyPressure = 0, fameInheritance = 0, className }: FamilyLegacyProps) {
  ...
  const fallbackFamilySize = familyMembers.length;
  const familySize = tree.length || fallbackFamilySize;
        ...
        <CardContent className="space-y-5">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-lg border p-3 text-center">
              <ShieldCheck className="mx-auto h-5 w-5 text-social-trust" />
              <p className="mt-1 text-xl font-bold">{data?.socialCapital ?? 0}/25</p>
              <p className="text-[11px] text-muted-foreground">Inherited social capital</p>
            </div>
            <div className="rounded-lg border p-3 text-center">
              <Users className="mx-auto h-5 w-5 text-social-chemistry" />
              <p className="mt-1 text-xl font-bold">{familySize}</p>
              <p className="text-[11px] text-muted-foreground">Recorded family nodes</p>
            </div>
            <div className="rounded-lg border p-3 text-center">
              <Crown className="mx-auto h-5 w-5 text-social-warm" />
              <p className="mt-1 text-xl font-bold">{data?.milestones?.length ?? 0}</p>
              <p className="text-[11px] text-muted-foreground">Dynasty milestones</p>
            </div>
          </div>

          {(legacyPressure > 0 || fameInheritance > 0) && (
            <p className="rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
              Older client legacy estimates ({fameInheritance}% fame / {Math.round(legacyPressure)} pressure) are no longer used as authority. Dynasty progression now comes from the server record above.
            </p>
          )}
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-2">
            {visibleHistory.length === 0 ? <p className="text-sm text-muted-foreground">No family history yet.</p> : visibleHistory.map((event) => (
              <div key={event.id} className="flex items-start gap-3 rounded-lg border p-3">
                <div className="mt-0.5">{iconForEvent(event.type)}</div>
                <div className="min-w-0 flex-1"><p className="text-sm font-medium">{event.title}</p><p className="text-xs text-muted-foreground">{formatDistanceToNow(new Date(event.occurredAt), { addSuffix: true })}</p></div>
      ...
      {settingsDraft && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                ["announceWeddings", "Weddings", Heart],
                ["announceBirths", "Births", Baby],
                ["announceComingOfAge", "Coming of age", Crown],
              ].map(([key, label, Icon]) => {
                const settingKey = key as "announceWeddings" | "announceBirths" | "announceComingOfAge";
                const EnabledIcon = Icon as typeof Heart;
                return <div key={key as string} className="flex items-center justify-between rounded-lg border p-3"><div className="flex items-center gap-2"><EnabledIcon className="h-4 w-4" /><Label htmlFor={`family-${String(key)}`}>{label as string}</Label></div><Switch id={`family-${String(key)}`} checked={settingsDraft[settingKey]} onCheckedChange={(checked) => setSettingsDraft({ ...settingsDraft, [settingKey]: checked })} /></div>;
              })}
            </div>
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-muted/40 p-3 text-xs text-muted-foreground">
              <span className="flex items-center gap-1">{settingsDraft.announcementVisibility === "public" ? <Eye className="h-3.5 w-3.5" /> : <EyeOff className="h-3.5 w-3.5" />} Shared family events require matching public opt-in from every participant.</span>
              <Button size="sm" onClick={() => savePrivacy.mutate()} disabled={savePrivacy.isPending}>{savePrivacy.isPending ? "Saving…" : "Save privacy"}</Button>
            </div>
          </CardContent>
        </Card>
      )}
