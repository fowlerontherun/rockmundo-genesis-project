...
function ModifierBadge({ label, value, icon }: { label: string; value: number; icon: React.ReactNode }) {
 ...
 <div className={cn("flex items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs font-medium transition-all",
 colorMap[sentiment],
 )}>
 {icon}
 <span>{label}</span>
...
export function EmotionalStatePanel({ compact = false, className }: EmotionalStatePanelProps) {
 ...
 if (compact) {
 return (
 <Card className={cn("border-border/50 transition-all duration-500",
 ...
 <TooltipTrigger asChild>
 <div className={cn("rounded-lg border border-border/50 p-2.5 space-y-1 bg-muted/20 transition-all hover:bg-muted/40",
 ...
 <ScoreGauge
 value={val}
 label=""color={EMOTION_COLORS[emotion.key]}
 ...
 <ModifierBadge
 label="Songwriting"value={state.songwriting_modifier}
 icon={<Music className="h-3 w-3"/>}
 />
 <ModifierBadge
 label="Performance"value={state.performance_modifier}
 icon={<Mic className="h-3 w-3"/>}
 />
 <ModifierBadge
 label="Social"value={state.interaction_modifier}
