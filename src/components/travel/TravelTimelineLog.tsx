...
const formatEta = (iso: string | null) =>
  iso ? format(new Date(iso), "MMM d, HH:mm") : "—";
...
export const TravelTimelineLog = ({
  ...
  const { data: events, isLoading } = useQuery({
    queryKey: [
      "travel-timeline",
      { tourId, profileId, includeAllMembers, limit },
    ],
    queryFn: async () => {
      let query = supabase
 ...
 profile:profile_id(id, username, display_name)` as any,
  ...
  const grouped = (() => {
    ...
    const map = new Map<
      string,
      { profileId: string; name: string; events: TimelineEvent[] }
    >();
    ...
    <Card className={className}>
      <CardHeader className="pb-2">
        ...
        <CardDescription className="text-xs">
          Every leg change, catch-up action and ETA update for{" "}
          {tourId ? "this tour" : "your travel"}.
        </CardDescription>
      </CardHeader>
              ...
              {grouped.map((group) => (
                <div key={group.profileId}>
                  {tourId && includeAllMembers && (
                    <div className="text-[11px] font-semibold tracking-wide text-muted-foreground mb-1.5">
                      {group.name}
                    </div>
                  )}
                  <ol className="relative border-l border-border ml-2 space-y-2">
                    {group.events.map((ev) => {
                      const style =
                        EVENT_STYLE[ev.event_type] ??
                        EVENT_STYLE.status_changed;
                        ...
                        <li key={ev.id} className="ml-3 pl-2">
                          ...
                          {(from || to) && (
                            <div className="flex items-center gap-1 text-[11px] text-muted-foreground mt-0.5">
                              ...
                              {ev.metadata?.transport_type && (
                                <span className="ml-1">
                                  ·{" "}
                                  {String(ev.metadata.transport_type).replace(
                                    /_/g,
                                    "",
                                  )}
                                </span>
                          ...
                          {etaChanged && (
                            <div className="text-[11px] text-amber-600 dark:text-amber-400 mt-0.5">
                              ETA: {formatEta(ev.previous_eta)} →{" "}
                              {formatEta(ev.new_eta)}
                            </div>
                          )}
                          {!etaChanged &&
                            ev.new_eta &&
                            ev.event_type !== "arrived" && (
