...
const emptyForm: Omit<
  TicketTier,
  "id" | "event_id" | "tickets_sold" | "created_at"
> & { tickets_sold?: number } = {
  name: "",
  price: 0,
  quantity: 0,
...
export const TicketTierManager = ({
  eventId,
  onTiersChange,
  ...
  const totalCapacity = useMemo(
    () => tiers.reduce((total, tier) => total + (tier.quantity || 0), 0),
    [tiers],
  );
  ...
  const loadTiers = useCallback(async () => {
    ...
    if (error) {
      ...
      toast({
        ...
        description:
          "Supabase returned an error while fetching the ticket tiers for this event.",
        variant: "destructive",
      });
    } else {
  ...
  const upsertTier = async () => {
    if (!eventId) {
      toast({
        ...
        description:
          "Ticket tiers must be associated with an event ID before they can be saved.",
      });
      return;
    }
    ...
    if (!form.name.trim()) {
      toast({
        ...
        description:
          "Provide a descriptive label so fans know exactly what they are purchasing.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (error) {
      ...
      toast({
        ...
        description:
          "Supabase could not process this tier. Review your data and try again.",
        variant: "destructive",
      });
    } else if (data) {
    ...
    <Card>
      <CardHeader className="space-y-2">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Manage inventory and benefits directly in Supabase for sales
              readiness.
            </CardDescription>
          </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  setForm((prev) => ({
                    ...prev,
                    quantity: Number.parseInt(event.target.value) || 0,
                  }))
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  setForm((prev) => ({
                    ...prev,
                    tickets_sold: Number.parseInt(event.target.value) || 0,
                  }))
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  setForm((prev) => ({
                    ...prev,
                    fees: Number.parseFloat(event.target.value) || 0,
                  }))
                }
              />
            </div>
        ...
        <div className="space-y-3">
          {loading ? (
            <div className="flex items-center justify-center gap-3 rounded-lg border border-dashed p-6 text-muted-foreground">
              <Loader2 className="h-5 w-5 animate-spin" /> Loading tiers from
              Supabase...
            </div>
          ) : tiers.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Add a tier to sync pricing and capacity plans with your Supabase
                dataset.
              </p>
            </div>
          ) : (
            tiers.map((tier) => (
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {tier.quantity.toLocaleString()} capacity ·{" "}
                    {tier.tickets_sold.toLocaleString()} sold · Fees $
      ...
      <CardFooter className="flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          Link your pricing strategy with Supabase-backed inventory to unlock
