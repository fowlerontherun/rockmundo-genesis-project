...
export const EventPricingStrategy = ({
  ...
  const recommendedBasePrice = useMemo(() => {
    ...
    const breakEvenPerSeat =
      (strategy.breakEvenCost - sponsorSupport) / Math.max(totalCapacity, 1);
    const demandMultiplier = 1 + Math.min(lineupEnergyScore / 150, 0.35);

    return Math.max(
      5,
      Number((breakEvenPerSeat * demandMultiplier).toFixed(2)),
    );
  }, [
    lineupEnergyScore,
    sponsorSupport,
    strategy.basePrice,
    strategy.breakEvenCost,
    totalCapacity,
  ]);
  ...
  const marginProjection = useMemo(() => {
    ...
    sponsorSupport,
    strategy.breakEvenCost,
    strategy.marketingBudget,
  ]);

  const handleNumberChange = (
    key: keyof PricingStrategyState,
    value: number,
  ) => {
      ...
      <CardHeader className="space-y-2">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Balance audience access, premium tiers, and profitability targets
              with dynamic pricing controls.
            </CardDescription>
          </div>
          <Badge variant="secondary" className="text-sm">
            <Calculator className="mr-1 h-4 w-4" />{" "}
            {totalCapacity.toLocaleString()} seat capacity modelled
          </Badge>
        </div>
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-lg border p-3">
            <p className="text-xs text-muted-foreground">Margin outlook</p>
            <p
              className={`text-lg font-semibold ${marginProjection >= 0 ? "text-emerald-600" : "text-rose-600"}`}
            >
              {marginProjection >= 0 ? "+" : "-"}$
              {Math.abs(marginProjection).toLocaleString()}
            </p>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-4 rounded-lg border p-4">
            ...
            <div className="flex items-center justify-between rounded-lg border p-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  React to demand patterns and pacing by adjusting inventory
                  each hour.
                </p>
              </div>
              <Switch
                ...
                onCheckedChange={(checked) =>
                  onStrategyChange({ ...strategy, dynamicPricing: checked })
                }
              />
            </div>
            <Button
                ...
                onStrategyChange({
                  ...
                  lastReviewNote: (
                    <span className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      Adjusted to reflect demand score of {
                        lineupEnergyScore
                      }{" "}
                      and sponsor offsets.
                    </span>
            ...
            >
              Align to recommended base of $
              {recommendedBasePrice.toLocaleString()}
            </Button>
          </div>
          <div className="space-y-4">
            <div className="rounded-lg border p-4">
              <p className="text-xs text-muted-foreground">Demand signals</p>
              <p className="text-sm text-muted-foreground">
                Lineup energy score of{" "}
                <span className="font-semibold">{lineupEnergyScore}</span>{" "}
                suggests a{""}
                {lineupEnergyScore >= 75
                  ? "premium"
                  : lineupEnergyScore >= 50
                    ? "balanced"
                    : "value-driven"}{" "}
                pricing window.
              </p>
              ...
              <div className="mt-2 space-y-2 text-sm text-muted-foreground">
                {marginProjection < 0 && (
                  <p className="flex items-center gap-2 text-rose-600">
                    <AlertTriangle className="h-4 w-4" /> Pricing plan is{" "}
                    {Math.abs(marginProjection).toLocaleString()} below target
                    margin.
                  </p>
                )}
                {strategy.lastReviewNote && (
                  <div>{strategy.lastReviewNote}</div>
                )}
                {marginProjection >= 0 && !strategy.lastReviewNote && (
                  <p className="flex items-center gap-2 text-emerald-600">
                    <DollarSign className="h-4 w-4" /> Plan clears margin
                    expectations with current mix.
                  </p>
                )}
              </div>
            </div>
