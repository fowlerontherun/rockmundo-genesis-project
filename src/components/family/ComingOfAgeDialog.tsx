...
import { Sparkles, Crown, ArrowRight, Lock, UserPlus, User, Check } from"lucide-react";
...
export function ComingOfAgeDialog({ child, trigger, autoPrompt }: Props) {
 ...
 <span className="text-[11px] text-muted-foreground">Legacy bonuses</span>
 ...
 <div className="flex items-center justify-between">
 <p className="text-[11px] text-muted-foreground">Pick a character slot</p>
 {slots && (
 ...
 <Lock className="h-3 w-3"/> No free slot — purchase one or retire a character.
 </p>
 ) : (
 ...
 <button
 key={n}
 type="button"disabled={isOccupied}
 onClick={() => setSelectedSlot(n)}
 className={cn("rounded-md border px-2 py-1.5 text-left text-[11px] transition-colors flex items-center gap-1.5",
 isOccupied
 ?"border-border/40 bg-muted/30 text-muted-foreground cursor-not-allowed": isSelected
 ?"border-social-chemistry bg-social-chemistry/10 text-foreground":"border-border/60 hover:border-social-chemistry/60",
 )}
 >
 ...
 {isOccupied
 ? occupant?.display_name || occupant?.username ||"Occupied":"Empty"}
 </span>
 </button>
 );
 })}
 ...
 <Button
 size="sm"onClick={handleConvert}
 disabled={!canConvertNow || !slotChosenAndFree || convert.isPending}
 className="gap-1.5">
 <UserPlus className="h-3.5 w-3.5"/>
 {convert.isPending
 ?"Creating…": selectedSlot
