...
export const OnboardingWizard = () => {
  ...
  useEffect(() => {
    ...
    const normalizedGender =
      profileGender === "prefer_not_to_say" || profileGender === "unspecified"
        ? "other"
        : profileGender === "non-binary"
          ? "non_binary"
          : typeof profileGender === "string"
            ? profileGender
            : "";
  ...
  const saveProfileDraft = async () => {
    ...
    if (error) {
      throw new Error(`Failed to save profile details: ${error.message}`);
    }
  };
  ...
  const saveProgress = async () => {
    ...
    // Persist the profile-facing choices as the player progresses. Previously
    // these were only saved on the final click, so a refresh could restore the
    // wizard at step 9 with a blank name/gender and make completion fail.
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      toast.error("Could not save your character", {
        description:
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      // Force a clean profile/game-data hydration after creating an extra
      // character. Client-side navigation can otherwise briefly keep the
      // previous character cached and bounce the player back into onboarding.
      window.location.assign("/home");
    } catch (error) {
      console.error("Failed to complete onboarding:", error);
      toast.error("Could not begin your journey", {
        description:
          error instanceof Error ? error.message : "Please try again.",
      });
    } finally {
