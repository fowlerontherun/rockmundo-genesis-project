...
const GENRE_STYLE_DESCRIPTIONS: Record<string, string> = {
 Rock:"Leather jacket, ripped jeans, band tees","Modern Rock":"Leather jacket, ripped jeans, Chuck Taylors","Heavy Metal":"Studded leather, bullet belt, black everything","Metalcore/Djent":"Studded leather, spiked gauntlets, black boots","Punk Rock":"Mohawk, safety pins, tartan, combat boots","Hip Hop":"Oversized chains, designer streetwear, snapback",
 Trap:"Gold chains, face tattoos, grillz, baggy pants",
 Drill:"Dark streetwear, ski mask, puffer jacket","Lo-Fi Hip Hop":"Vintage hoodie, round glasses, beanie",
 Pop:"Bright colors, trendy fits, statement accessories","K-Pop/J-Pop":"Pastel colors, K-pop fashion, platform shoes",
 ...
 Flamenco:"Red and black, ruffled details, rose accent","World Music":"Eclectic cultural fabrics, colorful patterns","R&B":"Sleek satin, gold jewelry, designer shoes","Alt R&B/Neo-Soul":"Minimalist earth tones, statement jewelry","African Music":"Vibrant ankara patterns, bold colors","Afrobeats/Amapiano":"Ankara patterns, designer shades, gold","Indie/Bedroom Pop":"Thrift store cardigan, retro glasses",
...
export function AiAvatarCreator() {
 ...
 <img
 src={generatedAvatar}
 alt="Current avatar"className="h-full w-full object-cover"/>
 </div>
 ...
 <input
 ref={fileInputRef}
 type="file"accept="image/*"className="hidden"onChange={handleFileSelect}
 />
 {/* Camera capture input - separate with capture attribute */}
 <input
 ref={cameraInputRef}
 type="file"accept="image/*"capture="user"className="hidden"onChange={handleFileSelect}
 />
 ...
 <Button
 variant="outline"size="sm"onClick={() => cameraInputRef.current?.click()}
 >
 ...
 <img
 src={uploadedPhoto}
 alt="Uploaded photo"className="h-full w-full object-cover"/>
 </AspectRatio>
 </div>

 {/* Generated Avatar */}
 <div className="space-y-2">
 <p className="text-xs font-medium text-muted-foreground">AI Avatar</p>
 <AspectRatio ratio={3 / 4} className="overflow-hidden rounded-lg border border-border bg-muted">
 {generatedAvatar ? (
 <img
 src={generatedAvatar}
 alt="Generated avatar"className="h-full w-full object-cover"/>
 ) : isGenerating ? (
