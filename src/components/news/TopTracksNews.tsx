        ...
        {topTracks && topTracks.length > 0 ? (
          <ol className="divide-y divide-border/60">
            {topTracks.map((track, index) => (
              <li
                key={track.song_id}
                className="flex flex-col gap-2 py-2.5 sm:flex-row sm:items-center sm:gap-4"
              >
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  <span className="text-base w-8 flex-shrink-0 font-mono font-bold">
                    {getRankIcon(index)}
                  </span>
                  <div className="min-w-0">
                    <p className="font-serif font-semibold text-sm leading-snug break-words">
                      {track.title}
                    </p>
                    <p className="text-xs text-muted-foreground break-words">
                      ...
                      {track.quality_score
                        ? ` · quality ${Math.round(track.quality_score)}`
                        : ""}
                    </p>
                  </div>
                </div>
