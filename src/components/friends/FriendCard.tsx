...
import { FriendActionsBar } from "@/features/social-hub/components/FriendActionsBar";

export function FriendCard({ friend, context, myProfileId, disabled, onAccept, onDecline, onCancel, onRemove, onAdd }: { friend: FriendSummary; context: "friend" | "incoming" | "outgoing" | "suggestion"; myProfileId?: string | null; disabled?: boolean; onAccept?: () => void; onDecline?: () => void; onCancel?: () => void; onRemove?: () => void; onAdd?: () => void }) {
  return <Card aria-label={`${friend.characterName} friendship card`}><CardContent className="flex flex-col gap-3 p-4">
    ...
    {context === "friend" && myProfileId && (
      <div className="border-t pt-3">
        <FriendActionsBar myProfileId={myProfileId} otherProfileId={friend.id} otherName={friend.characterName} />
      </div>
    )}
  </CardContent></Card>;
}
