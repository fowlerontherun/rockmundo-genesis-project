...
export const UnderworldStoreTab = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="rounded-xl border border-primary/30 bg-primary/5 px-4 py-2">
            ...
            <span className="ml-2 text-lg font-bold text-primary">${userBalance.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {categories.map((cat) => {
            ...
            return (
              <Button key={cat.id} variant={selectedCategory === cat.id ? "default" : "outline"} size="sm" onClick={() => setSelectedCategory(cat.id)} className="gap-2">
      ...
      {productsLoading ? (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[...Array(6)].map((_, i) => <Skeleton key={i} className="h-64 rounded-xl" />)}</div>
        ...
        <motion.div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
          {filteredProducts.map((product, index) => (
            <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
              <ProductCard product={product} onPurchase={handlePurchase} userBalance={userBalance} />
            </motion.div>
          ))}
        </motion.div>
      )}

      <PurchaseDialog product={selectedProduct} open={purchaseDialogOpen} onOpenChange={setPurchaseDialogOpen} userBalance={userBalance} />
    </div>
  );
};
