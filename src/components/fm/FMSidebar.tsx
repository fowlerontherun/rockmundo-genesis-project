...
export const FMSidebar = () => {
  ...
  return (
    <aside className={cn("shrink-0 bg-fm-panel border-r border-fm-border flex flex-col transition-[width] duration-150", collapsed ? "w-12" : "w-56")}>
      <button onClick={() => navigate("/")} className="h-11 flex items-center gap-2 px-2.5 border-b border-fm-border hover:bg-fm-panel-2 transition-colors" title="Rockmundo home" aria-label="Go to Rockmundo home">
        <img src={logo} alt="Rockmundo" className="h-7 w-7 object-contain shrink-0" />
        {!collapsed && <span className="font-bebas text-[18px] tracking-[0.1em] text-fm-fg leading-none">ROCKMUNDO</span>}
      </button>
      <div className="h-9 flex items-center justify-between px-3 border-b border-fm-border">
        {!collapsed && <span className="text-[12px] font-medium tracking-tight text-fm-fg truncate">{shellLabel}</span>}
        <button onClick={toggleCollapsed} className="ml-auto p-1 rounded-[6px] hover:bg-fm-panel-2 text-fm-fg-muted" title={collapsed ? "Expand" : "Collapse"} aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"} aria-expanded={!collapsed}>
      ...
      <div className="flex-1 overflow-y-auto py-2">
        {groups.map((group) => (
          <div key={group.label} className="mb-2">
            {!collapsed && (
              <button onClick={() => toggleGroup(group.label)} className="w-full flex items-center justify-between px-3 py-1 text-[11px] text-fm-fg-muted hover:text-fm-fg" aria-expanded={!!openGroups[group.label]}>
            ...
            {(collapsed || openGroups[group.label]) && (
              <div className="space-y-px px-1">
                {group.items.map((item) => {
                  ...
                  return (
                    <button key={item.path} onClick={() => navigate(item.path)} title={collapsed ? item.label : undefined} aria-current={item.active ? "page" : undefined} className={cn("w-full flex items-center gap-2 px-2.5 py-1.5 rounded-[7px] text-[12px] transition-colors", item.active ? "bg-fm-accent/15 text-fm-accent" : "text-fm-fg-muted hover:text-fm-fg hover:bg-fm-panel-2")}>
