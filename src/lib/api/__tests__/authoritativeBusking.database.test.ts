...
describe("authoritative busking database contract", () => {
  it("keeps city spots and completed outcomes server owned", () => {
    ...
    expect(migration).toContain("v_roll := 0.85 + random() * 0.45");
  ...
  it("charges the mayor law through canonical finance and credits City Hall", () => {
    ...
    expect(migration).toContain("'city', v_profile.current_city_id");
  ...
  it("uses the real user id for scheduling and city development for demand", () => {
    expect(migration).toContain("public.check_scheduling_conflict(p_user_id, v_start, v_end, NULL)");
    expect(migration).toContain("public.city_gameplay_modifiers(v_profile.current_city_id)");
    expect(migration).toContain("audience_demand_multiplier");
    expect(migration).toContain("v_city_multiplier := greatest(0.85, least(1.15");
  });
});
