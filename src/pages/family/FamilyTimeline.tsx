...
export default function FamilyTimeline() {
 ...
 <Button
 size="sm"variant="ghost"className="h-7 text-[11px]"onClick={() => { setScope(defaultScope); setScopeTouched(false); }}
 >
 Reset
 </Button>
 )}
 ...
 <Button
 size="sm"variant={scope ==="all"?"default":"outline"}
 className="h-6 text-[10px] px-2"onClick={() => { setScope("all"); setScopeTouched(true); }}
 >
 All
 </Button>
 ...
 <Button
 key={r.id}
 size="sm"variant={scope === `request:${r.id}` ?"default":"outline"}
 className="h-6 text-[10px] px-2"onClick={() => { setScope(`request:${r.id}`); setScopeTouched(true); }}
 >
 ...
 <Button
 key={c.id}
 size="sm"variant={scope === `child:${c.id}` ?"default":"outline"}
 className="h-6 text-[10px] px-2"onClick={() => { setScope(`child:${c.id}`); setScopeTouched(true); }}
 >
 {c.name}
 </Button>
 ))}
 ...
 className="h-7 text-[11px] capitalize"onClick={() => setFilter(f)}
 >
 {f}
 </Button>
 ))}
 ...
 {dayItems.map(item => {
 const Icon = item.kind ==="interaction"? interactionIcon(item.eventType)
 : item.kind ==="school"? GraduationCap
 : Baby;
 const colorClass = item.kind ==="request"? requestColor(item.eventType)
 : item.kind ==="school"?"text-social-chemistry":"text-social-chemistry";
 return (
 ...
 {item.kind ==="school"? (item.eventType ==="parent_teacher_day"?"Parent-Teacher Day": item.eventType ==="stage_started"?"Stage Started": item.eventType ==="report_card"?"Report Card": item.eventType ==="graduation"?"Graduation 🎓": item.eventType.replace(/_/g,""))
