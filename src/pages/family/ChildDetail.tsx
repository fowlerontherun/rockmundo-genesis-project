...
export default function ChildDetail() {
 ...
 <div
 className="h-full bg-social-chemistry transition-all"style={{ width: `${stagePct}%` }}
 />
 </div>
 ...
 <Badge
 variant="outline"className="text-[11px] gap-1 border-social-chemistry/40 bg-social-chemistry/5 cursor-help">
 ...
 <p className="text-[10px] text-muted-foreground font-semibold">{groupLabel}</p>
 <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
 {items.map(({ type, label, icon: Icon, description, color }) => (
 <Button
 key={type}
 variant="outline"className="h-auto flex-col items-start gap-1 p-3 text-left"disabled={apply.isPending}
 ...
 <div
 key={type}
 className="rounded-md border border-dashed border-border/60 p-3 opacity-60 cursor-not-allowed"title={unlockHint ??"Locked"}
 >
 <div className="flex items-center gap-2 w-full">
 <Icon className={`h-4 w-4 ${color}`} />
 <span className="text-sm font-semibold">{label}</span>
 ...
 ?"They are now an independent, playable character — switch from your character menu.":"They are ready to be converted into a playable character with inherited bonuses."}
 </p>
 </div>
 {!child.child_profile_id ? (
 ...
 value={value}
 max={20}
 color="social-chemistry"variant="bar"size="sm"/>
 ))}
 </CardContent>
 </Card>
 )}
