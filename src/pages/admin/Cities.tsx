...
const CitiesAdmin = () => {
 ...
 .from("cities")
 .select("id, name, country, dominant_genre, population, music_scene, cost_of_living, local_bonus, venues, cultural_events, created_at, updated_at",
 )
 ...
 <Badge variant="outline"className="">Admin Tool</Badge>
 ...
 <Button
 variant="ghost"size="sm"className="h-6 w-6 p-0"onClick={() => handleCityExpand(city.id)}
 >
 ...
 <Button
 variant="ghost"size="icon"onClick={() => handleEdit(city)}
 aria-label={`Edit ${city.name ??"city"}`}
 >
 <Pencil className="h-4 w-4"/>
 </Button>
 <Button
 variant="ghost"size="icon"onClick={() => handleDelete(city)}
 ...
 <CardDescription>
 {editingCity
 ?"Update core city stats and cultural events to keep the world fresh.":"Create a new city profile with stats and cultural highlights."}
 </CardDescription>
 </CardHeader>
 <CardContent>
 <Form {...cityForm}>
 ...
 <FormField
 control={cityForm.control}
 name="dominant_genre"render={({ field }) => (
 <FormItem>
 ...
 <FormField
 control={cityForm.control}
 name="cost_of_living"render={({ field }) => (
 <FormItem>
 ...
 <FormField
 control={cityForm.control}
 name="cultural_events"render={({ field }) => (
 <FormItem>
 <FormControl>
