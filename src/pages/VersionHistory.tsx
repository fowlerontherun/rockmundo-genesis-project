...
const versionHistory: VersionEntry[] = [
 ...
 date:"2026-06-30",
 changes: [
 { type:'improvement', description:"**Global sentence-case sweep via shell-scoped CSS.** Added `.fm-shell main` overrides that neutralize stray ``, ``, and `` styling across the 109 page/component files still using them — without touching shadcn primitives (menus, command palette, context) that legitimately need micro-caps. Eyebrow rows (`text-xs `, `text-[10px] `, `text-[11px] `) now render in muted sentence case at weight 500, and all in-shell `h1/h2/h3` clamp to 600 to match spec §4.7's two-weight rule. Opt-out via `data-fm-keep-caps` for the rare intentional all-caps badge."},
 ],
 },
 {
 version:"1.1.420",
 ...
 { type:'improvement', description:"**Tabs switched to pill style.** `ModuleTabs` and `SubTabs` now render as 7px-radius pills with an accent-tinted background on the active tab and sentence-case labels — replacing the previous , , underlined treatment. Two font weights only (400/500)."},
 ...
 date:"2026-06-09",
 changes: [
 { type:'improvement', description:"Global FM24 re-skin of core UI primitives so every page inherits the Football Manager look without bespoke conversion. `Card` is now a flat fm-panel surface with a tinted `CardHeader` strip, 11px tracked-out `CardTitle`, square corners, and dense 12px padding. `CardFooter` mirrors the header strip."},
 ...
 { type:'improvement', description:"Table primitive rebuilt as a compact dense FM data table: 28px row height, sticky `fm-panel-2` header with 10px tracked labels, zebra rows (`odd:fm-panel even:fm-panel-2/40`), 1px `fm-border` separators, and an `fm-accent` selection/hover wash. The wrapper now draws the FM panel border so tables read as standalone widgets."},
 { type:'improvement', description:"`PageHeader` collapsed into a single 40px FM strip: back button, accent icon, title and small subtitle on one row with right-aligned actions, framed as an `fm-panel-2` bar. Page chrome no longer competes with the FM shell's status / tab / sub-tab strips."},
 ],
 },
 {
 version:"1.1.356",
 ...
 date:"2026-05-18",
 changes: [
 { type:'fix', description:"Song Market'List a Song'now shows songs pulled from blind boxes (and gifts). The sellable/purchased queries used to filter by auth user_id against songs.profile_id, missing every blind-box drop; they now match either the character profile_id or auth user_id."},
 ...
 date:"2026-05-16",
 changes: [
 { type:'fix', description:"Inbox no longer mixes characters: every system message generated by generate-system-inbox is now tagged with metadata.profile_id, and useInbox / useUnreadInboxCount filter to the active character (legacy untagged messages still show). This stops a poor secondary character's Low cash warning from appearing while you're playing your wealthy main."},
 { type:'fix', description:"Welcome message dedup is now per-character (profile_id), so each character gets exactly one welcome ever — not one daily — and creating a new character still gives that character its own welcome."},
 { type:'improvement', description:"Low cash warning dedup extended from 7 to 14 days and now identifies the affected character by name in the title and body, so multi-character players know which one is broke."},
 { type:'feature', description:"New Daily Summary inbox entry (once per character per day): rolls up gigs played in the last 24h with best performance grade, fame Δ, fans Δ, XP earned, attribute points granted, and cash Δ. Only posted when something actually happened, and uses a new profile_daily_snapshots table to compute day-over-day deltas."},
 { type:'improvement', description:"Sales milestone notifications now respect release.profile_id, so a release earned by one character won't notify a different character on the same account."},
 { type:'improvement', description:"Weekly recap is now per character and labelled with the character's name."},
 ],
 },
 {
 version:"1.1.312",
 ...
 date:"2026-04-12",
 changes: [
 { type:'fix', description:"Backfilled chemistry_contribution and skill_contribution for all band members — values were stuck at 0"},
 ],
 },
 {
 version:"1.1.165",
 ...
 date:"2026-04-12",
 changes: [
 { type:'fix', description:"Removed duplicate bands (Souldrip, Dakine, Shade) caused by retry errors during band creation"},
 ],
 },
 {
 version:"1.1.162",
...
export default function VersionHistory() {
 ...
 <Input
 placeholder="Search versions, dates, or keywords..."className="pl-9 pr-9"value={query}
 onChange={(e) => setQuery(e.target.value)}
 />
 {query && (
 <button
 onClick={() => setQuery("")}
 className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground"aria-label="Clear search">
 <X className="h-4 w-4"/>
 </button>
 )}
 </div>
 ...
 <Badge
 variant="outline"className="bg-primary/10 text-primary border-primary/30">
 v{entry.version}
 </Badge>
 </CardTitle>
