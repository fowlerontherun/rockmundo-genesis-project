...
const ExperimentsAnalytics = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (isMounted) {
          setError(
            "We couldn't load experiment analytics right now. Please try again in a moment.",
          );
        }
      } finally {
        if (isMounted) {
  ...
  const activeVariant = selectedExperiment?.variants.find(
    (variant) => variant.id === activeVariantId,
  );
  const leadingVariant =
    selectedExperiment?.variants.find((variant) => variant.isWinning) ??
    ...
    return (
      ...
      >
        <Alert>
          ...
          <AlertDescription>
            Launch your first campaign test to start benchmarking creative
            performance, conversion lift, and retention impact.
          </AlertDescription>
        </Alert>
      </FMPageScaffold>
    );
    ...
    >
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatNumber(summary.totalExperiments)}
            </div>
            <p className="text-xs text-muted-foreground">
              Historical experiments managed
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatNumber(summary.runningExperiments)}
            </div>
            <p className="text-xs text-muted-foreground">
              Live experiments targeting key segments
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatPercent(summary.avgConversionLift)}
            </div>
            <p className="text-xs text-muted-foreground">
              Mean conversion uplift vs control
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatNumber(summary.activeReach, { notation: "compact" })}
            </div>
            <p className="text-xs text-muted-foreground">
              Fans touched by active experiments
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant={
                (statusVariantMap[selectedExperiment.status] || "outline") as
                  | "default"
                  | "destructive"
                  | "outline"
                  | "secondary"
        ...
        <CardContent className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-4">
            <div>
              ...
              <p className="text-base font-semibold">
                {selectedExperiment.targetSegment}
              </p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Primary metric</p>
              <p className="text-base font-semibold">
                {selectedExperiment.primaryMetric}
              </p>
            </div>
            <div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {formatPercent(selectedExperiment.confidence, 0)} confidence
                level
              </p>
            </div>
          </div>
          <div className="space-y-4">
            <div className="flex items-start gap-3 rounded-lg border border-dashed border-border/50 p-4">
              ...
              <div>
                ...
                {leadingVariant ? (
                  <p className="text-xs text-muted-foreground">
                    {formatPercent(leadingVariant.conversionRate)} conversion
                    rate · {formatPercent(leadingVariant.uplift)} lift
                  </p>
                ) : null}
              </div>
            </div>
            <div className="rounded-lg bg-muted/40 p-4">
              ...
              <p className="text-xs text-muted-foreground">
                Monitoring fan behavior, drop-off points, and monetization
          ...
          <div className="space-y-4">
            <div className="rounded-lg border border-border/60 p-4">
              ...
              <p className="text-sm text-muted-foreground">
                Each variant represents a creative bet. Track revenue per
                visitor, retention, and engagement to decide which story scales
            ...
            <div className="rounded-lg border border-dashed border-border/50 p-4">
              ...
              <p className="text-sm text-muted-foreground">
                Improve subscriber capture, reduce bounce for cold traffic, and
                fuel downstream merch conversions.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              Compare how each variant is trending over time against the control
              baseline.
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Conversions and revenue (in thousands) recorded during the test
              window.
            </CardDescription>
          </CardHeader>
          <CardContent className="h-[320px]">
              ...
              >
                ...
                <Tooltip
                  formatter={(value: number, key) =>
                    key === "revenue"
                      ? [`$${value.toFixed(1)}k`, "Revenue"]
                      : [formatNumber(value), "Conversions"]
                  }
                />
                <Legend />
                <Bar
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Explore audience reactions, monetization signals, and qualitative
            takeaways to decide how to scale each creative direction.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {formatPercent(variant.uplift)} lift vs control
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          ${formatNumber(variant.totalRevenue)}
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="text-lg font-semibold">
                          {formatPercent(variant.retentionRate)}
                        </p>
                        <Progress
                          value={variant.retentionRate}
                          className="mt-2 h-2"
                        />
                      </div>
                      <div>
                        ...
                        <p className="text-lg font-semibold">
                          {formatPercent(variant.engagementRate)}
                        </p>
                        <Progress
                          value={variant.engagementRate}
                          className="mt-2 h-2"
                        />
                      </div>
                      <div>
                        ...
                        <p className="text-lg font-semibold">
                          {formatPercent(variant.bounceRate)}
                        </p>
                      </div>
                      <div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Quantitative snapshot to help prioritize rollout plans.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                {selectedExperiment.variants.map((variant) => (
                  <TableRow
                    key={variant.id}
                    className={variant.isWinning ? "bg-muted/40" : undefined}
                  >
