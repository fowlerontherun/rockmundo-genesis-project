...
type RadioSubmissionRow =
  Database["public"]["Tables"]["radio_submissions"]["Row"];
...
const normaliseGenre = (value: string | null | undefined) =>
...
export default function Radio() {
    ...
    useQuery<ProfileWithBand | null>({
      ...
      queryFn: async () => {
        ...
        const { data, error } = await (supabase as any)
          .from("profiles")
          .select(
            "id, display_name, bands!bands_leader_id_fkey(id, name, fame)",
          )
          .eq("user_id", user.id)
          .maybeSingle();
 ...
 radio_shows(id, show_name)`,
      ...
      enabled:
        !!selectedStation &&
        (activeTab === "submit" || activeTab === "analytics"),
      staleTime: 2 * 60 * 1000,
    });
  ...
  useEffect(() => {
    ...
    if (
      selectedSong &&
      releasedSongs.some((song: any) => song.id === selectedSong)
    )
      return;
  ...
  >({
    ...
    queryFn: async () => {
      ...
      const { data: bandSubmissions, error: bandSubmissionsError } =
        await supabase
          .from("radio_submissions")
          .select(
            "id, status, submitted_at, reviewed_at, rejection_reason, station_id, song_id, songs(title), radio_stations(name)",
      ...
      for (const submission of bandSubmissions ?? [])
        mergedSubmissions.set(submission.id, submission);
  ...
  const { data: topSongs, isLoading: topSongsLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("songs")
        .select(
          "id, title, genre, hype, audio_url, audio_generation_status, profiles(display_name), bands(name)",
        )
 ...
 radio_shows(show_name, host_name)`,
  ...
  const submitSong = useMutation({
    mutationFn: async () => {
      ...
      if (!genreMatches)
        throw new Error(
          "This station is not accepting the selected song's genre right now.",
        );
      ...
      if (existing && existing.length > 0) {
        throw new Error(
          "You've already submitted a song to this station this week.",
        );
      }
    ...
    onError: (error: unknown) => {
      const message =
        error instanceof Error
          ? error.message
          : "Unable to submit your song right now.";
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <label className="text-sm font-medium">
                      {t("radio.recordedSong")}
                    </label>
                    <Select
                      value={selectedSong}
                      onValueChange={setSelectedSong}
                      disabled={submitSong.isPending}
                    >
                      <SelectTrigger>
                    ...
                    {selectedSongData?.genre && (
                      <p className="text-xs text-muted-foreground">
                        Genre: {selectedSongData.genre} • Quality:{" "}
                        {selectedSongData.quality_score ?? "–"}
                      </p>
                    )}
                  </div>
                  <div className="space-y-2">
              ...
              {activeStation && !!activeStation.accepted_genres?.length && (
                <Alert
                  variant={genreMatches ? "default" : "destructive"}
                  className="py-2"
                >
                  {genreMatches ? (
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-xs text-muted-foreground sm:text-sm">
                  {primaryBand
                    ? `${t("radio.submittingAs")} ${primaryBand.name ?? "your band"}`
                    : t("radio.createBandForExposure")}
                </p>
                <Button
                ...
                <div className="space-y-3">
                  <div>
                    <p className="font-semibold">
                      {nowPlaying.songs?.title ?? "Untitled"}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {nowPlaying.songs?.genre ?? "Unknown"} •{" "}
                      {nowPlaying.songs?.bands?.name ?? "Independent"}
                    </p>
                  </div>

                  {(nowPlaying.songs?.audio_url ||
                    nowPlaying.songs?.audio_generation_status) && (
                    <div className="space-y-2">
                      <SongPlayer
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div className="rounded-md border bg-background/60 p-2">
                      <p className="text-lg font-bold text-emerald-500">
                        {(nowPlaying.listeners ?? 0).toLocaleString()}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {t("radio.listeners")}
                      </p>
          ...
          {primaryBand ? (
            ...
            <Card>
              <CardContent className="py-12 text-center text-muted-foreground">
                ...
                <p className="text-sm">
                  Create or join a band to receive radio invitations
                </p>
              </CardContent>
            </Card>
          )}
        ...
        <TabsContent value="analytics" className="mt-4 space-y-4">
          ...
          {!activeStation ? (
            <Alert>
              ...
              <AlertDescription>
                Choose a station above to view station-specific analytics.
              </AlertDescription>
            </Alert>
          ) : (
            <>
                      ...
                      <CardContent className="p-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-lg font-bold">
                            {activeStation.listener_base.toLocaleString()}
                          </span>
                        </div>
                        <p className="text-xs text-muted-foreground">
                          Listener Base
                        ...
                        <Card>
                          <CardContent className="p-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-lg font-bold">
                                {stationStats.avgListeners.toLocaleString()}
                              </span>
                            </div>
                            <p className="text-xs text-muted-foreground">
                              Avg Listeners
                        ...
                        <Card>
                          <CardContent className="p-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-lg font-bold">
                                {stationStats.totalHype.toLocaleString()}
                              </span>
                            </div>
                            <p className="text-xs text-muted-foreground">
                              Total Hype
                    ...
                    <div className="space-y-3">
                      <div>
                        <h3 className="font-semibold">
                          {nowPlaying.songs?.title || "Unknown"}
                        </h3>
                        <p className="text-sm text-muted-foreground">
        ...
        <TabsContent value="history" className="mt-4 space-y-4">
          {!activeStation ? (
            <Alert>
              <AlertTitle>Select a Station</AlertTitle>
              <AlertDescription>
                Choose a station to view play history.
              </AlertDescription>
            </Alert>
          ) : (
            <Card>
                  ...
                  <div className="space-y-3">
                    {recentPlays.map((play: any, idx: number) => (
                      ...
                      >
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          ...
                          <div className="flex flex-wrap items-center gap-2 text-sm">
                            <div className="flex items-center gap-1">
                              ...
                              <span>
                                {(play.listeners || 0).toLocaleString()}
                              </span>
                            </div>
                            <div className="flex items-center gap-1">
                        ...
                        {(play.songs?.audio_url ||
                          play.songs?.audio_generation_status) && (
                          <div className="mt-3 space-y-2 border-t pt-3">
                            ...
                            />
                            {play.songs?.id && (
                              <SongVoting songId={play.songs.id} compact />
                            )}
                          </div>
                        )}
                      </div>
        ...
        <TabsContent value="trending" className="mt-4 space-y-4">
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription className="text-sm">
                Top tracks dominating the airwaves this week.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {topSongsLoading ? (
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    Check back after the first wave of spins.
                  </AlertDescription>
                </Alert>
              ) : (
                <div className="space-y-3">
                      ...
                      {(song.audio_url || song.audio_generation_status) && (
                        <div className="mt-3 space-y-2 border-t pt-3">
                          <TrackableSongPlayer
                            ...
                            title={song.title}
                            artist={
                              song.bands?.name || song.profiles?.display_name
                            }
                            compact
