...
const FEATURES = [
 ...
 { icon: Mic2, title:"Live Performance", body:"Book gigs, open mics and arena tours. Tune your setlist, hire crew, and read the crowd."},
...
const Landing = () => {
 ...
 <img src={logo} alt="RockMundo"className="h-8 w-8 object-contain shrink-0"width={32} height={32} />
 <span className="font-bebas text-xl tracking-wide leading-none pt-0.5">ROCKMUNDO</span>
 </Link>
 <Badge variant="outline"className="hidden sm:inline-flex bg-warning/10 text-warning border-warning/30 text-[10px] px-1.5 py-0 font-oswald">
 Beta V1
 </Badge>
 <div className="flex-1"/>
 <Link to="/about"className="hidden sm:inline-block text-xs font-oswald text-muted-foreground hover:text-foreground px-2 py-1">
 About
 </Link>
 {isDev && (
 <Button
 variant="ghost"size="sm"className="h-9 px-2 text-xs font-oswald tracking-wide"onClick={() => navigate("/dashboard")}
 ...
 <Button
 size="sm"className="h-9 px-3 sm:px-4 font-oswald tracking-wide text-xs"onClick={() => setOpen(true)}
 >
 ...
 <span>
 <span className="font-semibold text-primary mr-1.5">Beta V1 has finished</span>
 ...
 <Button
 asChild
 size="sm"variant="outline"className="h-7 px-2.5 font-oswald tracking-wide text-[10px]">
 <a href={DISCORD_URL} target="_blank"rel="noopener noreferrer">
 <MessageCircle className="h-3.5 w-3.5 mr-1"/> Discord
 </a>
 </Button>
 <Button
 size="sm"className="h-7 px-2.5 font-oswald tracking-wide text-[10px]"onClick={() => { setBetaDone(false); setBetaError(""); setBetaOpen(true); }}
 >
 ...
 <img
 src={heroImage}
 alt="Concert stage with crowd and stage lights"width={1920}
 height={1080}
 ...
 <img src={logo} alt="RockMundo"className="h-24 sm:h-32 md:h-40 w-auto mx-auto object-contain drop-shadow-2xl mb-5"/>
 ...
 street corner to selling out arenas, running a label and shaping the global charts.
 </p>
 </div>

 <div className="flex flex-col sm:flex-row gap-3 justify-center max-w-md mx-auto sm:max-w-none">
 <Button
 size="lg"className="font-oswald tracking-wide w-full sm:w-auto"onClick={() => setOpen(true)}
 >
 ...
 {isDev ? (
 <Button
 size="lg"variant="outline"className="font-oswald tracking-wide w-full sm:w-auto"onClick={() => navigate("/dashboard")}
 >
 ...
 <Button
 asChild
 size="lg"variant="outline"className="font-oswald tracking-wide w-full sm:w-auto">
 <Link to="/auth">
 ...
 {STATS.map(s => (
 <Card key={s.label} className="bg-card/80 backdrop-blur-sm border-border/40">
 ...
 {FEATURES.map(f => (
 <Card key={f.title} className="bg-card/80 backdrop-blur-sm border-border/40 hover:border-primary/40 transition-colors">
 <CardContent className="p-4">
 <f.icon className="h-5 w-5 text-primary mb-3"/>
 <h3 className="font-bebas text-lg tracking-wide mb-1.5">{f.title}</h3>
 <p className="text-xs text-muted-foreground leading-relaxed font-oswald">{f.body}</p>
 </CardContent>
 </Card>
 ))}
 </div>
 ...
 <li key={k} className="flex items-center gap-3 sm:gap-4 px-4 py-3">
 <span className="text-[10px] sm:text-xs font-oswald text-primary w-16 sm:w-20 shrink-0">
 {k}
 </span>
 <span className="text-sm text-muted-foreground font-oswald">{v}</span>
 </li>
 ))}
 </ul>
 </CardContent>
 ...
 <Label htmlFor="landing-email"className="text-xs font-oswald text-muted-foreground">Email</Label>
 <Input
 id="landing-email"type="email"autoComplete="email"required
 value={email}
 ...
 <Label htmlFor="landing-password"className="text-xs font-oswald text-muted-foreground">Password</Label>
 <Link
 to="/auth"className="text-xs font-oswald text-muted-foreground hover:text-foreground"onClick={() => setOpen(false)}
 >
 Forgot?
 </Link>
 </div>
 <Input
 id="landing-password"type="password"autoComplete="current-password"required
 value={password}
 ...
 {isDev && (
 <Button
 type="button"variant="ghost"className="font-oswald tracking-wide text-xs"onClick={() => {
 setOpen(false);
 navigate("/dashboard");
 }}
 >
 ...
 <DialogFooter>
 <Button
 className="font-oswald tracking-wide w-full"onClick={() => setBetaOpen(false)}
 >
 Close
 </Button>
 </DialogFooter>
 ...
 <div className="space-y-1.5">
 <Label htmlFor="beta-email"className="text-xs font-oswald text-muted-foreground">Email</Label>
 <Input
 id="beta-email"type="email"required
 autoComplete="email"value={betaEmail}
 onChange={e => setBetaEmail(e.target.value)}
 placeholder="you@example.com"/>
 </div>
 <div className="space-y-1.5">
 <Label htmlFor="beta-name"className="text-xs font-oswald text-muted-foreground">Display name (optional)</Label>
 <Input
 id="beta-name"type="text"value={betaName}
 onChange={e => setBetaName(e.target.value)}
 placeholder="Stage name or handle"/>
 </div>
 <div className="space-y-1.5">
 <Label htmlFor="beta-discord"className="text-xs font-oswald text-muted-foreground">Discord handle (optional)</Label>
 <Input
 id="beta-discord"type="text"value={betaDiscord}
 ...
 <DialogFooter className="gap-2 sm:gap-0 pt-2">
 <Button
 type="submit"disabled={betaLoading}
