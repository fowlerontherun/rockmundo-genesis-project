...
const playerPages = [
 ...
 summary:"Lists all owned items grouped by category, shows condition bars, power output, and usage status with actions for load, repair, sell, and upgrade.",
 },
 {
 name:"Equipment Shop",
 url:"/city/:cityId/shop/equipment",
 summary:"Grid of available equipment for purchase in current city with pricing, stats, prerequisites, and purchase workflow.",
 },
 {
...
const gameplayHighlights = [
 ...
 details: ["Formula: (Σ (equipment_power * condition%) / 100) * (Variety Bonus) * (Transport Bonus)","Variety Bonus = +10% if at least one item from each major category.","Transport Bonus = +10% when equipment is properly loaded and co-located with the show city.",
 ],
 },
 {
 title:"Wear & Tear",
 details: ["Condition Loss = base_loss + (gig_intensity / 10)","base_loss ranges 1-3 based on gig size with slight randomized variance per item.",
 ],
 },
 {
 title:"Maintenance",
 details: ["Repair Cost = ((100 - condition) / 100) * maintenance_cost","If condition < 20%, each gig adds 10% breakdown chance marking equipment temporarily unusable.",
 ],
 },
];
...
const StageEquipmentSystemPlan = () => {
 return (
 <FMPageScaffold
 title="Stage Equipment System"subtitle="Full Deployment & Development Plan — Version 1.0 (November 2025)"icon={ClipboardList}
 backTo="/hub/band-live"headerActions={
 <Badge variant="outline"className="text-[10px]">Deployment Plan</Badge>
 }
 >
