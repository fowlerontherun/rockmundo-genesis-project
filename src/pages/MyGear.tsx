...
const MyGear: React.FC = () => {
 ...
 ?"vocal": gear.sections.includes("pedal")
 ?"pedal":"other";
 let assigned = false;
 ...
 return (
 <FMPageScaffold
 title="Personal Loadout Planner"subtitle="Build and tune your performance rig with mics, pedal boards, and auxiliary gear."icon={Guitar}
 backTo="/hub/career-business"headerActions={
 ...
 ?"We slotted your latest purchase into the first available position. Adjust placement if needed.":"All slots are filled—open the highlighted tab to choose where this gear should live."}
 </AlertDescription>
 </Alert>
 ) : null}
 ...
 ?"rounded-lg border border-primary p-4 shadow-[0_0_0_2px] shadow-primary/20":"rounded-lg border p-4";
 ...
 {needsRepair && (
 <Button
 size="sm"variant="outline"className="h-5 px-1.5 text-[10px] gap-1"disabled={repairingId === gear.id}
 ...
 <span className="text-right">
 {typeof gear.stock ==="number"? gear.stock <= 0
 ?"Sold out": gear.stock
 :"—"}
 </span>
 </div>
...
const OtherGearList: React.FC<OtherGearListProps> = ({
 ...
 <Button
 type="button"size="sm"variant="secondary"onClick={onAddItem}
 ...
 <Button
 type="button"variant="ghost"size="icon"onClick={() => onRemoveItem(item.id)}
