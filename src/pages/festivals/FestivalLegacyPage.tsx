...
import { formatMinorMoney } from "@/features/festivals/legacy/model";
...
export default function FestivalLegacyPage() {
  ...
  useEffect(() => {
    ...
    const request: Promise<unknown> = resultId ? festivalLegacyService.detail(resultId).then(value => { if(live)setDetail(value); })
      ...
      : section === "hall-of-fame" ? festivalLegacyService.hallOfFame().then(value=>{if(live)setItems(value)})
...
function LegacyCard({item,section}:{item:FestivalResult|FestivalAward|FestivalRecord;section:string}){
  ...
  return <Card><CardHeader><CardTitle className="flex items-center gap-2">{section==="awards"?<Award aria-hidden/>:section==="records"?<Radio aria-hidden/>:<History aria-hidden/>}{r.festivalName??a.winnerName??w.holderName}</CardTitle></CardHeader><CardContent className="space-y-2">
    {section!=="awards"&&section!=="records"&&<><p>{r.city}, {r.country} · {r.editionYear}</p><p className="text-2xl font-semibold">{r.overallRating}/100</p><p>{r.attendance.toLocaleString("en-GB")} attended · {r.sellOutPercentage.toFixed(1)}% capacity · {r.soldOut ? "Sold out" : "Did not sell out"}</p><p>{money(r.profitLossMinor,r.currencyCode)} profit/loss</p><Button asChild variant="outline"><Link to={`/festivals/results/${r.id}`}>View full result</Link></Button></>}
    {a.category&&<><Badge>{a.category.replaceAll("_"," ")}</Badge><p>{a.citation}</p><p>Score {a.score.toFixed(1)}</p></>}
    {w.category&&<><Badge>{w.category.replaceAll("_"," ")}</Badge><p className="text-2xl font-semibold">{Number(w.valueText).toLocaleString("en-GB")} {w.unit}</p><p>Set in {w.achievedYear}</p></>}
  </CardContent></Card>;
}
...
function Statistics({value:s}:{value:FestivalStatistics}){return <section aria-label="Festival statistics" className="space-y-4"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[["Editions",s.editions],["Attendance",s.attendance.toLocaleString("en-GB")],["Average rating",`${s.averageRating}/100`],["Sell-outs",s.sellOuts]].map(([k,v])=><Card key={k}><CardContent className="py-6"><BarChart3 aria-hidden/><p className="text-sm text-muted-foreground">{k}</p><p className="text-2xl font-bold">{v}</p></CardContent></Card>)}</div><div className="grid gap-3 sm:grid-cols-2">{s.moneyByCurrency.map(m=><Card key={m.currencyCode}><CardContent className="py-4"><p className="font-semibold">{m.currencyCode}</p><p>{money(m.revenueMinor,m.currencyCode)} revenue</p><p>{money(m.profitLossMinor,m.currencyCode)} profit/loss</p></CardContent></Card>)}</div><div className="overflow-x-auto"><table className="w-full min-w-[36rem] text-left"><caption className="sr-only">Statistics grouped by Festival</caption><thead><tr><th scope="col">Festival</th><th scope="col">Editions</th><th scope="col">Attendance</th><th scope="col">Rating</th></tr></thead><tbody>{s.groups.map(g=><tr key={g.label} className="border-t"><th scope="row" className="py-2">{g.label}</th><td>{g.editions}</td><td>{g.attendance.toLocaleString("en-GB")}</td><td>{g.averageRating}</td></tr>)}</tbody></table></div></section>}
