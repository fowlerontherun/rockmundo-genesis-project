...
function TeachingInner() {
  ...
  const handleStartSession = async () => {
    ...
    setIsStarting(true);
    const { session, error } = await startTeachingSession(
      selectedFriend,
      selectedSkill,
  ...
  const handleCancelSession = async (sessionId: string) => {
    ...
    if (success) {
      toast({
        ...
        description: "The teaching session has been cancelled.",
      });
    } else {
  ...
  const getSkillName = (slug: string) =>
    skillMap.get(slug)?.displayName ?? slug;
  ...
  const getRoleBadge = (
    ...
    return (
      <Badge
        variant="outline"
        className="text-xs bg-accent/50 text-accent-foreground border-accent/30"
      >
        Learning
      </Badge>
    );
  ...
  if (!teachingTier) {
    return (
      <FMPageScaffold
        title="Teaching"
        subtitle="Teach your skills to friends and earn bonus XP."
        icon={GraduationCap}
        backTo="/hub/career"
      >
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Unlock the Teaching skill tree to teach your skills to friends and
              earn bonus XP.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-semibold mb-2">
                Teaching Not Unlocked
              </h3>
              <p className="text-muted-foreground max-w-md mx-auto">
                You need to unlock the <strong>Basic Teaching</strong> skill to
                start teaching other players. Visit the Skills page to unlock
                it.
              </p>
                    ...
                    <Card key={tier} className="border-border/50">
                      <CardContent className="py-3 px-4">
                        <p className="font-medium text-sm">
                          {config.label} Teaching
                        </p>
                        <p className="text-xs text-muted-foreground">
                          Student: {config.studentXpRange[0]}-
                          {config.studentXpRange[1]} XP/day • Teacher:{" "}
                          {config.teacherXpRange[0]}-{config.teacherXpRange[1]}{" "}
                          XP/day •{config.maxStudents} student
                          {config.maxStudents > 1 ? "s" : ""} max
                        </p>
              ...
              <p className="text-xs text-muted-foreground mt-4">
                ...
                University awards 15-95 XP/day. Teaching surpasses that at every
                tier!
              </p>
            </div>
          </CardContent>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
              <div>
                ...
                <p className="font-bold text-primary">
                  {tierConfig.studentXpRange[0]}-{tierConfig.studentXpRange[1]}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Teacher XP/day</p>
                <p className="font-bold text-primary">
                  {tierConfig.teacherXpRange[0]}-{tierConfig.teacherXpRange[1]}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p
                  className={`font-bold ${activeTeachingCount >= tierConfig.maxStudents ? "text-destructive" : ""}`}
                >
        ...
        <TabsContent value="teach">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Choose a skill and a friend. Both of you earn XP daily — more
                than university!
              </CardDescription>
            </CardHeader>
              ...
              {loading || friendsLoading ? (
                ...
                <div className="text-center py-6">
                  ...
                  <p className="text-muted-foreground">
                    No skills high enough to teach (need level{" "}
                    {tierConfig?.minSkillLevel ?? 5}+). Level up your skills to
                    start teaching!
                  </p>
                </div>
              ) : (
                <>
                    ...
                    <Card className="border-border/50 bg-muted/30">
                      <CardContent className="py-3">
                        <p className="text-sm font-medium mb-1">
                          XP Preview ({selectedDuration} days)
                        </p>
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          <div>
                            ...
                            <span className="font-semibold text-primary">
                              {xpPreview.studentXpPerDay *
                                parseInt(selectedDuration)}{" "}
                              XP
                            </span>
                            <span className="text-xs text-muted-foreground">
                              {" "}
                              ({xpPreview.studentXpPerDay}/day)
                            </span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">
                            ...
                            {""}
                            <span className="font-semibold text-primary">
                              {xpPreview.teacherXpPerDay *
                                parseInt(selectedDuration)}{" "}
                              XP
                            </span>
                            <span className="text-xs text-muted-foreground">
                              {" "}
                              ({xpPreview.teacherXpPerDay}/day)
                            </span>
                          </div>
                        </div>
                      </CardContent>
                  ...
                  <Button
                    ...
                    disabled={
                      ...
                      !selectedFriend ||
                      isStarting ||
                      (preValidation !== null && !preValidation.valid)
                    }
                    className="w-full"
                  >
                    {isStarting ? (
                  ...
                  <Card key={session.id}>
                    <CardContent className="py-4 space-y-3">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <p className="font-medium">
                              {getSkillName(session.skill_slug)}
                            </p>
                            {getRoleBadge(session, profileId)}
                          </div>
                          <p className="text-sm text-muted-foreground mt-0.5">
                            {getProfileName(session.teacher_profile_id)} →{" "}
                            {getProfileName(session.student_profile_id)}
                          </p>
                        </div>
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                ...
                                {getSkillName(session.skill_slug)}. Any XP
                                already earned will be lost.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              <AlertDialogCancel>
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        <span>
                          Student: +{session.student_xp_earned} XP total
                        </span>
                        <span>
                          Teacher: +{session.teacher_xp_earned} XP total
                        </span>
                      </div>
                    </CardContent>
                  </Card>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="text-right flex-shrink-0">
                          <div className="flex items-center gap-1 text-sm">
                            ...
                            <span>
                              +{session.student_xp_earned} / +
                              {session.teacher_xp_earned} XP
                            </span>
                          </div>
                          <p className="text-xs text-muted-foreground">
                            {session.session_duration_days} days
                          </p>
                        </div>
                      </div>
                    </CardContent>
