...
import {
  ...
  getRecoveryProgramDetails,
  getDaysCleanMilestone,
...
export default function WellnessPage() {
  ...
  const restMutation = useMutation({
    ...
    onSuccess: ({ health, energy, hours }) => {
      ...
      toast.success(
        `Rested for ${hours} hour(s). Health: ${health}%, Energy: ${energy}%`,
      );
    },
  });
  ...
  const meditationMutation = useMutation({
    mutationFn: async (duration: number) => {
      ...
      if (mentalFocusGain > 0) {
        ...
        await supabase
          ...
          .update({
            mental_focus: Math.min(1000, currentFocus + mentalFocusGain),
          })
    ...
    onSuccess: ({ duration, stressReduction }) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["wellness-history"] });
      toast.success(
        `Meditated for ${duration} minutes. Stress reduced by ${stressReduction}%`,
      );
    },
  });
  ...
  const exerciseMutation = useMutation({
    mutationFn: async (intensity: "light" | "moderate" | "intense") => {
      ...
      if (energy < costs[intensity])
        throw new Error(`Not enough energy for ${intensity} exercise`);
      await supabase.from("experience_ledger").insert({
        user_id: profile.user_id,
        profile_id: profile.id,
        ...
        metadata: {
          intensity,
          healthGain: gains[intensity],
          enduranceGain: enduranceGains[intensity],
        },
      });
      ...
      await supabase
        .from("player_attributes")
        .update({
          physical_endurance: Math.min(
            1000,
            currentEndurance + enduranceGains[intensity],
          ),
        })
        .eq("profile_id", profile.id);
    ...
    onSuccess: ({ health, energy, intensity }) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["wellness-history"] });
      toast.success(
        `Completed ${intensity} exercise. Health: ${health}%, Energy: ${energy}%`,
      );
    },
  });
  ...
  const therapyMutation = useMutation({
    mutationFn: async () => {
      if (!profile) throw new Error("No profile");
      if (profile.cash < 50)
        throw new Error("Not enough money for therapy session ($50)");
      await supabase.from("experience_ledger").insert({
        user_id: profile.user_id,
        profile_id: profile.id,
  ...
  const nutritionMutation = useMutation({
    mutationFn: async (quality: "basic" | "healthy" | "premium") => {
      ...
      if (profile.cash < costs[quality])
        throw new Error(`Not enough money for ${quality} meal`);
      await supabase.from("experience_ledger").insert({
        user_id: profile.user_id,
        profile_id: profile.id,
  ...
  if (loading) {
    return (
      <div className="flex justify-center py-8">Loading wellness data...</div>
    );
  }
    ...
    >
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab}>
        ...
        <TabsContent value="overview" className="space-y-4">
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-base">
                  <Heart className="h-4 w-4" /> Physical Health
                </CardTitle>
              </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Your wellness journey over the past week
              </CardDescription>
            </CardHeader>
            <CardContent>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center justify-between p-4 rounded-lg border">
                ...
                <Button
                  ...
                  disabled={
                    therapyMutation.isPending || (profile?.cash ?? 0) < 50
                  }
                >
                  Session ($50)
                </Button>
            ...
            <CardContent>
              <div className="flex items-center justify-between p-4 rounded-lg border">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium">Eat a Meal</p>
                    <p className="text-sm text-muted-foreground">
                      Restore energy
                    </p>
                  </div>
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    disabled={
                      nutritionMutation.isPending || (profile?.cash ?? 0) < 10
                    }
                  >
                    Basic ($10)
                  </Button>
                  <Button
                    ...
                    disabled={
                      nutritionMutation.isPending || (profile?.cash ?? 0) < 25
                    }
                  >
                    Healthy ($25)
                  </Button>
                  <Button
                    ...
                    disabled={
                      nutritionMutation.isPending || (profile?.cash ?? 0) < 50
                    }
                  >
                    Premium ($50)
                  </Button>
        ...
        <TabsContent value="hospital" className="space-y-4">
          {activeHospitalization ? (
            <Card className="border-destructive">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Hospital className="h-5 w-5" /> Currently Hospitalized
                </CardTitle>
                <CardDescription>
                  You are recovering in hospital. All activities are blocked.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">
                      {format(
                        new Date(activeHospitalization.admitted_at),
                        "MMM d, HH:mm",
                      )}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="font-medium">
                      {format(
                        new Date(activeHospitalization.expected_discharge_at),
                        "MMM d, HH:mm",
                      )}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="font-medium">
                      ${activeHospitalization.total_cost}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">
                    ...
                    <p className="font-medium">
                      {activeHospitalization.health_on_admission}%
                    </p>
                  </div>
                </div>
                {isDischargeDue && (
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Hospital className="h-5 w-5" /> Hospital
                </CardTitle>
                <CardDescription>
                  Faster recovery when health is critical.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {nearestHospital ? (
                  <div className="p-4 rounded-lg border space-y-3">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Nearest hospital in your city
                        </p>
                      </div>
                      <Badge
                        variant={
                          nearestHospital.is_free ? "default" : "secondary"
                        }
                      >
                        {nearestHospital.is_free
                          ? "Free"
                          : `$${nearestHospital.cost_per_day}/day`}
                      </Badge>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>Effectiveness</span>
                      <span className="font-bold">
                        {nearestHospital.effectiveness_rating}/100
                      </span>
                    </div>
                    <Progress
                      value={nearestHospital.effectiveness_rating}
                      className="h-2"
                    />
                    <Button
                      onClick={() => checkIn({})}
                    ...
                    >
                      {health > 30
                        ? "Health must be below 30 to check in"
                        : isCheckingIn
                          ? "Checking in..."
                  ...
                  <p className="text-sm text-muted-foreground text-center py-4">
                    No hospital available in your current city.
                  </p>
                )}
                <Alert>
                  <ShieldAlert className="h-4 w-4" />
                  <AlertDescription>
                    If your health drops to 0, you'll be auto-admitted to the
                    nearest hospital.
                  </AlertDescription>
                </Alert>
              </CardContent>
        ...
        <TabsContent value="addictions" className="space-y-4">
          {addictions.length === 0 ? (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <Pill className="h-5 w-5" /> No Active Addictions
                </CardTitle>
                <CardDescription>
                  You're clean. Nightlife can trigger addictions.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Alert>
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    Heavy partying and afterparty attendance increase addiction
                    risk. Manage your lifestyle settings to stay safe.
                  </AlertDescription>
                </Alert>
              </CardContent>
            </Card>
                  ...
                  <CardHeader>
                    <CardTitle className="flex items-center justify-between">
                      <span className="flex items-center gap-2">
                        ...
                        {getAddictionTypeLabel(addiction.addiction_type)}{" "}
                        Addiction
                      </span>
                      <Badge
                        variant={
                  ...
                  <CardContent className="space-y-4">
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className={`font-bold ${severityInfo.color}`}>
                          {severityInfo.label} ({addiction.severity}/100)
                        </span>
                      </div>
                    ...
                    {milestone && (
                      <Alert>
                        ...
                        <AlertDescription>
                          🎉 Milestone: {milestone.label}! Reward: $
                          {milestone.reward}
                        </AlertDescription>
                      </Alert>
                    )}
                    ...
                    addiction.status === "relapsed" ? (
                      ...
                      <Button
                        ...
                        disabled={
                          isAttendingTherapy || (profile?.cash ?? 0) < 100
                        }
                        className="w-full"
                      >
                        {isAttendingTherapy
                          ? "In Session..."
                          : "Attend Therapy Session ($100)"}
                      </Button>
                    ) : (
                      <p className="text-sm text-muted-foreground text-center">
        ...
        <TabsContent value="holidays" className="space-y-4">
          {activeHoliday ? (
            (() => {
              const activeDest = HOLIDAY_DESTINATIONS.find(
                (d) => d.name === activeHoliday.destination,
              );
              return (
                    ...
                    <div className="relative h-48 overflow-hidden">
                      ...
                      <div className="absolute bottom-3 left-4 right-4">
                        ...
                        <p className="text-sm text-white/80">
                          {activeHoliday.destination} — Songwriting still
                          available
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-green-500">
                          +{activeHoliday.health_boost_per_day}/day
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="font-medium">
                          {format(
                            new Date(activeHoliday.ends_at),
                            "MMM d, HH:mm",
                          )}
                        </p>
                      </div>
            ...
            <>
              {!canBookHoliday && (
                <Alert>
                  ...
                  <AlertDescription>
                    Holiday cooldown active. You can book another holiday after
                    14 days.
                  </AlertDescription>
                </Alert>
              )}
              ...
              {/* Tier sections */}
              {(
                ["budget", "standard", "premium", "luxury", "ultra"] as const
              ).map((tier) => {
                const tierDests = HOLIDAY_DESTINATIONS.filter(
                  (d) => d.tier === tier,
                );
                            ...
                            <div className="pt-1 border-t border-border/50 space-y-1.5">
                              <p className="text-xs text-muted-foreground">
                                <span className="font-bold text-foreground">
                                  ${dest.costPerDay.toLocaleString()}
                                </span>
                                /day
                              </p>
                              <div className="flex flex-wrap gap-1.5">
                                {dest.durations.map((d) => {
                                  ...
                                  const canAfford =
                                    (profile?.cash ?? 0) >= total;
                                  return (
                                    <Button
                                      key={d}
                                      size="sm"
                                      variant={
                                        canAfford && canBookHoliday
                                          ? "default"
                                          : "outline"
                                      }
