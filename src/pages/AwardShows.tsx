...
const scheduleLabels: Record<
  keyof (typeof awardShows)[number]["schedule"],
  string
> = {
  ...
  return (
    ...
    >
      <div className="grid gap-4 rounded-lg border border-dashed border-primary/40 bg-muted/10 p-6 md:grid-cols-2">
        <div className="space-y-2 text-left">
          ...
          <p className="text-sm text-muted-foreground">
            Build momentum by aligning release cadences and tour stops with the
            nomination sourcing windows. These placeholders will later sync to
            real ticketing, stream deltas, and influencer reach metrics.
          </p>
        </div>
        <div className="space-y-2 text-left">
          <h2 className="flex items-center gap-2 text-lg font-semibold">
          ...
          <p className="text-sm text-muted-foreground">
            Attendance and wins layer onto your fame ledger. Plan for the
            ceremonies that best fit your sonic identity and performance
            capacity to maximize the reputation lift.
          </p>
        </div>
      </div>
            ...
            <CardContent className="space-y-6">
                  ...
                  {Object.entries(show.schedule).map(([key, value]) => (
                    ...
                    >
                      <p className="font-semibold text-foreground">
                        {scheduleLabels[key as keyof typeof scheduleLabels]}
                      </p>
                      <p>{value}</p>
                    </div>
                  ))}
              ...
              <section className="space-y-3">
                ...
                <div className="space-y-3 text-sm text-muted-foreground">
                  {show.categories.map((category) => (
                    <div
                      key={`${show.id}-${category.name}-source`}
                ...
                <div className="rounded-lg border border-dashed border-primary/40 bg-primary/5 p-4 text-sm text-muted-foreground">
                  <div className="flex flex-wrap gap-6 text-foreground">
                    <span className="font-semibold">
                      Player influence:{" "}
                      {(show.voting.playerWeight * 100).toFixed(0)}%
                    </span>
                    <span className="font-semibold">
                      NPC guilds: {(show.voting.npcWeight * 100).toFixed(0)}%
                    </span>
                    <span className="font-semibold">
                      Industry jury:{" "}
                      {(show.voting.industryJuryWeight * 100).toFixed(0)}%
                    </span>
                  </div>
              ...
              <section className="space-y-3">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="rounded-lg border border-border/60 bg-muted/30 p-4 text-sm text-muted-foreground">
                    ...
                    <p className="mt-2 text-xs tracking-wide text-primary/80">
                      Plan: Ensure check-in scans complete.
                    </p>
                  </div>
                  <div className="rounded-lg border border-border/60 bg-muted/30 p-4 text-sm text-muted-foreground">
                    <p className="text-sm font-semibold text-foreground">
              ...
              <section className="space-y-3">
                ...
                {show.performanceSlots.length !== 4 && (
                  <p className="text-xs font-semibold tracking-wide text-destructive">
                    Placeholder roster must maintain exactly four bands with
                    two-song sets.
                  </p>
                )}
              </section>
      ...
      <footer className="rounded-lg border border-dashed border-border/60 bg-muted/20 p-6 text-sm text-muted-foreground">
        These award profiles will evolve as Rockmundo ingests live partner data,
        unlocking automatic applications, campaign reminders, and real-time
        voting analytics. Until then, use these scaffolds to map out your London
