...
const MerchDesignerCanvas = ({ settings }: MerchDesignerCanvasProps) => {
 ...
 {settings.productShape ==="poster"?"Poster mock simulated in canvas":"Garment mock simulated in canvas"}
 </div>
 </div>
 );
};
...
export default function MerchCommercePage() {
 ...
 return (
 <FMPageScaffold
 title="Merch Designer & Control Room"subtitle={`Prototype new drops, monitor velocity, and keep ${bandName}'s inventory dialed-in without leaving your command center.`}
 icon={Sparkles}
 backTo="/hub/business">
 <section>
 ...
 <button
 key={color}
 type="button"onClick={() => handleSettingChange("fabricColor", color)}
 ...
 <Input
 type="color"value={designerSettings.fabricColor}
 ...
 <button
 key={color}
 type="button"onClick={() => handleSettingChange("accentColor", color)}
 className={`${swatchClasses} ${
 ...
 <Input
 type="color"value={designerSettings.accentColor}
 onChange={(event) => handleSettingChange("accentColor", event.target.value)}
 className="h-8 w-12 cursor-pointer rounded-md border border-border/50 p-1"/>
 </div>
 <div className="flex items-center gap-3">
 <Label className="text-xs text-muted-foreground">Print color</Label>
 <Input
 type="color"value={designerSettings.inkColor}
 ...
 <Input
 id="designer-title"value={designerSettings.title}
 ...
 <Input
 id="designer-tagline"value={designerSettings.tagline}
 ...
 <Button
 key={pattern}
 type="button"variant={designerSettings.pattern === pattern ?"default":"outline"}
 ...
 <div className="space-y-3">
 <Label className="text-xs text-muted-foreground">Product silhouette</Label>
 <div className="grid grid-cols-3 gap-2">
 {productShapes.map((shape) => (
 <Button
 key={shape}
 type="button"variant={designerSettings.productShape === shape ?"default":"outline"}
 ...
 <div className="space-y-3">
 <Label className="text-xs text-muted-foreground">Texture intensity</Label>
 <Slider
