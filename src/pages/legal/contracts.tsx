...
const ContractsPage = () => {
 ...
 return (
 <FMPageScaffold
 title="Contract Command Center"subtitle="Review active label agreements, understand clause expectations, and coordinate negotiation moves with your team."icon={FileText}
 ...
 ?"border-primary shadow-sm":"border-border/80"}
 >
 ...
 {terms.summary && (
 <div>
 <p className="text-xs text-muted-foreground">Label baseline</p>
 ...
 <Button
 variant="ghost"size="sm"className="px-2"onClick={() => setActiveClauseId(clause.id)}
 >
 Focus clause
 </Button>
 </CardContent>
 ...
 <Textarea
 id="proposal"value={proposalNotes}
 onChange={(event) => setProposalNotes(event.target.value)}
 placeholder="Outline the terms you're requesting"rows={5}
 />
 </div>
 ...
 <Textarea
 id="counter"value={counterNotes}
 onChange={(event) => setCounterNotes(event.target.value)}
 placeholder="Capture the latest label response"rows={4}
 />
 </div>
