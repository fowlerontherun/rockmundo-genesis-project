...
export default function BlindBoxAnalytics() {
 ...
 : range ==="30d"? Date.now() - 30 * 86400000
 : 0;
 ...
 return (
 <FMPageScaffold
 title="Blind Box Analytics"subtitle="Pull rates, average quality, and your progression over time."icon={BarChart3}
 backTo="/blind-boxes"backLabel="Back to Store"headerActions={
 ...
 <span
 className="h-2 w-2 rounded-full"style={{ backgroundColor: TIER_HSL[d.key] }}
 />
 ...
 <Legend wrapperStyle={{ fontSize: 11 }} />
 <Area
 yAxisId="left"type="monotone"dataKey="avgQuality"name="Avg Quality"stroke="hsl(var(--primary))"fill="url(#qGrad)"/>
 <Area
 yAxisId="right"type="monotone"dataKey="epicPlus"name="Cumulative Epic+"stroke={TIER_HSL.epic}
 fill={TIER_HSL.epic}
 fillOpacity={0.15}
 />
 </AreaChart>
 ...
 <Input
 type="date"className="h-8"value={dateFrom}
 onChange={(e) => { setDateFrom(e.target.value); setPage(0); }}
 placeholder="From"/>
 <Input
 type="date"className="h-8"value={dateTo}
 onChange={(e) => { setDateTo(e.target.value); setPage(0); }}
 placeholder="To"/>
 <Input
 type="number"inputMode="numeric"className="h-8"placeholder="Min quality"value={qMin}
 onChange={(e) => { setQMin(e.target.value); setPage(0); }}
 />
 <Input
 type="number"inputMode="numeric"className="h-8"placeholder="Max quality"value={qMax}
 ...
 <TableCell>
 <Badge
 variant="outline"className="text-[10px]"style={{ borderColor: TIER_HSL[o.tier], color: TIER_HSL[o.tier] }}
 >
 ...
 <Button
 size="sm"variant="outline"className="h-7 px-2"onClick={() => setPage((p) => Math.max(0, p - 1))}
 disabled={page === 0}
 >
 ...
 <Button
 size="sm"variant="outline"className="h-7 px-2"onClick={() => setPage((p) => Math.min(totalPages - 1, p + 1))}
