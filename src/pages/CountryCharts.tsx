...
const ChartTable = ({ entries, isLoading, chartType, timeRange, releaseCategory }: ChartTableProps) => {
 ...
 <div
 key={entry.id}
 className={cn("grid grid-cols-8 sm:grid-cols-12 gap-1 sm:gap-2 px-2 sm:px-3 py-2 sm:py-3 rounded-lg hover:bg-accent/50 transition-colors items-start cursor-pointer",
 ...
 <span
 className={cn("font-bold text-base sm:text-lg",
...
export default function CountryCharts() {
 ...
 return (
 <FMPageScaffold
 title="Charts"subtitle={`Top 50 ${isAlbumView ?"albums":"songs"} by region, genre, and sales type`}
 icon={Music}
 backTo="/hub/music">
