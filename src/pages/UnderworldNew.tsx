...
const loreSections = [
  {
    title: "Veil Rift",
    description:
      "A neon cavern where traders whisper rates that never surface above ground.",
  ...
  },
  {
    title: "Gutter Bazaar",
    description:
      "Tinkerers swap contraband gear beside graffiti-marked freight crates.",
  ...
  },
  {
    title: "Echo Atrium",
    description:
      "Sound designers test mixes against the stone cathedral's infinite reverb.",
  ...
  },
  {
    title: "Courier Loop",
    description:
      "Trusted runners route artifacts and IOUs through hidden tram tunnels.",
...
const callToActions = [
  {
    ...
    description:
      "Scout safe travel lanes and avoid watchful drones before heading topside.",
    href: "/travel",
    icon: MapPin,
  },
  {
    ...
    description:
      "Check World Pulse briefings to anticipate when Underworld gates reopen.",
    href: "/world-pulse",
    icon: BookOpen,
  },
  {
    ...
    description:
      "Reserve rehearsal rooms with reinforced walls for late-night experiments.",
    href: "/rehearsals",
    icon: Shield,
  },
];
...
const PortfolioPanel = ({ profileId }: { profileId: string }) => {
  ...
  const totalValue = holdings.reduce(
    (sum, h) => sum + h.quantity * (h.token?.current_price || 0),
    0,
  );
  const totalCost = holdings.reduce(
    (sum, h) => sum + h.quantity * h.average_buy_price,
    0,
  );
        ...
        <div className="space-y-1.5 max-h-40 overflow-y-auto">
          {holdings.map((h) => {
            ...
            return (
              <div
                key={h.id}
                className="flex items-center justify-between text-sm rounded-lg px-2 py-1 bg-muted/30"
              >
                <div>
      ...
      >
        ...
        <div>
          ...
          <p className="text-sm text-red-300">
            {ruggedSymbols.join(",")}{" "}
            {ruggedSymbols.length === 1 ? "has" : "have"} been rugged! Price
            dropped to $0.00
          </p>
        </div>
      </motion.div>
...
export const UnderworldContent = ({
  tokens,
  tokensLoading,
}: {
  tokens: CryptoToken[];
  ...
  const { holdings, buyToken, sellToken, isBuying, isSelling } =
    useCryptoTokens(profileId ?? undefined);
  ...
  useEffect(() => {
    if (tokens.length > 0) {
      ...
      tokens.forEach((t) => {
        newPrices[t.symbol] = t.current_price;
      });
  ...
  const activeTokens = useMemo(
    () => tokens.filter((t) => t.is_active && !t.is_rugged),
    [tokens],
  );
  const selectedTokenData = useMemo(
    () =>
      activeTokens.find((t) => t.symbol === selectedToken) ||
  ...
  const totalMarketCap = useMemo(
    () => activeTokens.reduce((s, t) => s + (t.market_cap || 0), 0),
    [activeTokens],
  );
  const totalVolume = useMemo(
    () => activeTokens.reduce((s, t) => s + (t.volume_24h || 0), 0),
    [activeTokens],
  );
    ...
    >
      <div className="relative overflow-hidden rounded-lg bg-gradient-to-b from-background via-slate-950 to-background text-foreground">
        ...
        <div className="relative space-y-10 px-6 py-8">
          {/* Hero */}
          <section
            className="grid gap-8 rounded-3xl border border-primary/10 bg-background/70 p-8 shadow-xl backdrop-blur"
            id="hero"
          >
            <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              <div className="space-y-4 lg:max-w-2xl">
                ...
                <h1 className="text-4xl font-bold leading-tight lg:text-5xl">
                  Shadow market for off-ledger deals
                </h1>
                <p className="text-lg text-muted-foreground">
                  Exchange tokens, broker rare merch drops, and follow whispered
                  routes that keep your band ahead of corporate scouts.
                </p>
              ...
              <div className="w-full lg:max-w-sm">
                <Card className="border-primary/30 bg-primary/5 shadow-lg">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Real-time market data. Prices auto-refresh.
                    </CardDescription>
                  </CardHeader>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h2 className="text-3xl font-semibold">
                  Acquire power from the depths
                </h2>
                <p className="text-muted-foreground">
                  Consumables, boosters, and forbidden knowledge.
                </p>
              </div>
            </div>
          ...
          <section id="lore" className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h2 className="text-3xl font-semibold">
                  Navigate the tunnels without losing the plot
                </h2>
              </div>
            </div>
              ...
              {loreSections.map((entry) => {
                ...
                return (
                  <motion.div
                    ...
                    className="rounded-2xl border border-muted/50 bg-background/60 p-5 shadow-lg"
                  >
          ...
          <section id="market" className="space-y-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="space-y-1">
                <p className="text-sm tracking-wide text-primary">Token desk</p>
                <h2 className="text-3xl font-semibold">
                  Trade routes, charts, and liquidity
                </h2>
                <p className="text-muted-foreground">
                  Choose a token to trade. Prices are volatile — trade at your
                  own risk.
                </p>
              </div>
              <Badge variant="outline" className="text-xs">
                <AlertTriangle className="h-3 w-3 mr-1" /> Tokens can rug at any
                time
              </Badge>
            </div>
            ...
            <Card className="border-primary/20 bg-background/70">
              <CardHeader className="pb-2">
                ...
                <CardDescription>
                  Live token prices — auto-refreshing every 30s
                </CardDescription>
              </CardHeader>
                      ...
                      <TableBody>
                        {activeTokens.map((token) => {
                          ...
                          const priceChange24h =
                            hist.length > 1
                              ? ((token.current_price - hist[0].price) /
                                  hist[0].price) *
                                100
                              : 0;
                            ...
                            >
                              ...
                              <TableCell className="text-right">
                                <PriceCell
                                  ...
                                  previousPrice={previousPrices[token.symbol]}
                                />
                              </TableCell>
                              <TableCell
                                className={cn(
                              ...
                              <TableCell
                                className={cn(
                                  "text-right text-xs hidden md:table-cell capitalize",
                                  tierColors[token.volatility_tier] || "",
                                )}
                              >
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="flex gap-2 mt-1">
                          ...
                          >
                            {selectedTokenData.volatility_tier?.replace(
                              "_",
                              "",
                            )}
                          </Badge>
                          ...
                          >
                            {formatPrice(selectedTokenData.current_price)}
                          </Badge>
                        </div>
                      </div>
                      <Button
                    ...
                    <Tabs defaultValue="buy" className="w-full">
                      ...
                      <TabsContent value="buy" className="space-y-4">
                              ...
                              {buyQuantity
                                ? formatPrice(
                                    parseFloat(buyQuantity || "0") *
                                      selectedTokenData.current_price,
                                  )
                                : "—"}
                            </div>
                          </div>
                        ...
                        <Button
                          className="w-full"
                          onClick={handleBuy}
                          disabled={isBuying || !profileId || !buyQuantity}
                        >
                          {isBuying
                            ? "Processing..."
                            : `Buy ${selectedTokenData.symbol} at ${formatPrice(selectedTokenData.current_price)}`}
                        </Button>
                        {!profileId && (
                      ...
                      <TabsContent value="sell" className="space-y-4">
                        <div className="grid gap-4 md:grid-cols-2">
                          <div>
                            ...
                            {holdings &&
                              holdings.find(
                                (h) => h.token_id === selectedTokenData.id,
                              ) && (
                                <p className="text-xs text-muted-foreground mt-1">
                                  ...
                                  {holdings
                                    .find(
                                      (h) =>
                                        h.token_id === selectedTokenData.id,
                                    )!
                                    .quantity.toFixed(4)}
                                </p>
                              )}
                        ...
                        <Button
                          ...
                          variant="destructive"
                          onClick={handleSell}
                          disabled={isSelling || !profileId || !sellQuantity}
                        >
                          {isSelling
                            ? "Processing..."
                            : `Sell ${selectedTokenData.symbol} at ${formatPrice(selectedTokenData.current_price)}`}
                        </Button>
                      </TabsContent>
                    </Tabs>
                  </div>
          ...
          <section id="actions" className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h2 className="text-3xl font-semibold">
                  Move quietly between trusted nodes
                </h2>
              </div>
            </div>
