...
type MerchandiseRecord =
  Database["public"]["Tables"]["player_merchandise"]["Row"];
...
const Merchandise = () => {
  ...
  const { data: merchandise = [], isLoading: loadingMerch } = useQuery<
    MerchandiseRecord[]
  >({
    queryKey: ["player-merchandise", bandId],
    queryFn: async () => {
  ...
  const { data: todaySalesMap = {} } = useQuery<Record<string, number>>({
    ...
    queryFn: async () => {
      ...
      (data || []).forEach((o: any) => {
        map[o.merchandise_id] =
          (map[o.merchandise_id] || 0) + (o.quantity || 0);
      });
      return map;
    },
    enabled: Boolean(bandId),
  ...
  const selectedProduct = useMemo(
    () =>
      (Array.isArray(merchandise)
        ? merchandise.find((item) => item.id === selectedProductId)
        : null) ?? null,
    [merchandise, selectedProductId],
  );
  ...
  useEffect(() => {
    if (selectedProduct) {
      ...
      setEditForm({
        ...
        cost: (
          blueprint?.cost ?? safeNumber(selectedProduct.cost_to_produce)
        ).toString(),
  ...
  const summary = useMemo(() => {
    ...
    const avgMargin =
      totals.revenue > 0 ? (totals.revenue - totals.cost) / totals.revenue : 0;
  ...
  const categoryMetrics = useMemo(() => {
    const categoryMap = new Map<
      string,
      { units: number; revenue: number; skus: number }
    >();
  ...
  const restockAlerts = useMemo(
    () =>
      Array.isArray(merchandise)
        ? merchandise
            .filter(
              (product) => getStatus(product.stock_quantity) !== "in_stock",
            )
            .sort(
              (a, b) =>
                safeNumber(a.stock_quantity) - safeNumber(b.stock_quantity),
            )
            .slice(0, 5)
        : [],
    [merchandise],
  ...
  const addProductMutation = useMutation({
    mutationFn: async ({
      ...
      if (!blueprint || blueprint.category !== category) {
        throw new Error(
          "Choose a valid product type for the selected category",
        );
      }
  ...
  const catalogAddProductMutation = useMutation({
    mutationFn: async ({
      ...
      const quality = calculateMerchQuality(
        item.base_quality_tier,
        bandFame,
        false,
      );
  ...
  const createMerchWithDesign = async (
    designId: string,
    designName: string,
  ) => {
    if (!bandId) {
  ...
  const handleArtworkUploadClick = () => {
    toast({
      ...
      description:
        "Use the outline generator below to brief your designer in the meantime.",
    });
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="flex items-center gap-3 text-muted-foreground">
          <Loader2 className="h-5 w-5 animate-spin" /> Loading merchandise
          operations...
        </div>
      </div>
    );
      ...
      <div className="container mx-auto px-4 py-12">
        <Card className="mx-auto max-w-2xl">
          <CardHeader>
            ...
            <CardDescription>
              Merchandise inventory, pricing, and drop scheduling live inside
              your band workspace. Create or join a band to start tracking
              products.
            </CardDescription>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground">
            Once you are part of an active band you can load in-stock items, log
            restocks, and plan limited drops from this hub.
          </CardContent>
        </Card>
      </div>
    );
                ...
                <div className="flex items-center gap-2">
                  {isAdmin() && (
                    <Button
                      ...
                      onClick={() => {
                        const missing = (merchandise || []).filter(
                          (p: any) => !p.design_preview_url,
                        );
                        if (missing.length === 0) {
                          toast({
                            title: "All images generated",
                            description: "Every product already has an image.",
                          });
                          return;
                        }
                        generateAllMissing(
                  ...
                  <span className="text-sm text-muted-foreground">
                    Real-time totals pulled from your Supabase merchandise
                    table.
                  </span>
                </div>
              </CardTitle>
              <CardDescription>
                Track on-hand value, average margin, and low stock alerts for
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <Card className="border-dashed">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Unique merch items in circulation.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                ...
                <Card className="border-dashed">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Current sellable inventory across all drops.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                ...
                <Card className="border-dashed">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Revenue if every unit sells at full price.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <p className="text-2xl font-bold">
                      {currencyFormatter.format(summary.potentialRevenue)}
                    </p>
                  </CardContent>
                </Card>
                <Card className="border-dashed">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Weighted by current stock levels.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <p className="text-2xl font-bold">
                      {percentFormatter.format(summary.avgMargin)}
                    </p>
                  </CardContent>
                </Card>
              </div>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <Card className="lg:col-span-2">
              <CardHeader>
                ...
                <CardDescription>
                  Costs, pricing, and stock — live from Supabase.
                </CardDescription>
              </CardHeader>
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                  <div className="rounded-lg border border-dashed bg-muted/30 p-4">
                    ...
                    <p className="text-2xl font-semibold">
                      {currencyFormatter.format(summary.avgPrice)}
                    </p>
                  </div>
                  <div className="rounded-lg border border-dashed bg-muted/30 p-4">
                    <p className="text-xs text-muted-foreground">
                    ...
                    <TableBody>
                      {Array.isArray(merchandise) &&
                        merchandise.map((product) => {
                          ...
                          const unitProfit =
                            safeNumber(product.selling_price) -
                            safeNumber(product.cost_to_produce);
                          const potential =
                            safeNumber(product.selling_price) *
                            safeNumber(product.stock_quantity);
                          const status = getStatus(product.stock_quantity);
                          const qualityTier =
                            (product as any).quality_tier || "basic";
                          const qualityInfo =
                            QUALITY_TIERS[
                              qualityTier as keyof typeof QUALITY_TIERS
                            ] || QUALITY_TIERS.basic;
                          const recommendedPrice = getRecommendedPrice(
                            safeNumber(product.cost_to_produce),
                            qualityTier as any,
                          );
                          const pricingImpact = getPricingImpact(
                            safeNumber(product.selling_price),
                            recommendedPrice,
                          );
                          ...
                          return (
                            <TableRow key={product.id}>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button
                                      onClick={(e) => {
                                        ...
                                        generateImage({
                                          merchId: product.id,
                                          itemType:
                                            product.item_type ?? "merchandise",
                                          designName:
                                            product.design_name ?? "Product",
                                          bandName,
                                          qualityTier:
                                            (product as any).quality_tier ||
                                            "basic",
                                        });
                                      }}
                                      disabled={
                                  ...
                                  <div className="flex flex-col">
                                    ...
                                    <span className="text-xs text-muted-foreground">
                                      {percentFormatter.format(margin)} margin ·{" "}
                                      {currencyFormatter.format(unitProfit)} /
                                      unit
                                    </span>
                                  </div>
                                </div>
                              ...
                              <TableCell className="hidden xl:table-cell">
                                {formatProductType(product.item_type)}
                              </TableCell>
                              <TableCell>
                                <Badge
                                  variant="outline"
                              ...
                              <TableCell className="hidden md:table-cell">
                                <div className="flex flex-col gap-0.5">
                                  <Badge
                                    variant="outline"
                                    className={`text-xs ${pricingImpact.color}`}
                                  >
                                    {pricingImpact.label}
                                  </Badge>
                                  <span className="text-[10px] text-muted-foreground">
                                    Rec: ${recommendedPrice} ·{" "}
                                    {pricingImpact.salesMultiplier}x sales
                                  </span>
                                </div>
                              </TableCell>
                              <TableCell>
                                ...
                                <div className="flex items-center gap-1.5">
                                  ...
                                  {(status === "sold_out" ||
                                    ...
                                    <RestockPopover
                                      ...
                                      productName={
                                        product.design_name ?? "Item"
                                      }
                                      currentStock={safeNumber(
                                        product.stock_quantity,
                                      )}
                                      costPerUnit={safeNumber(
                      ...
                      {(!merchandise ||
                        ...
                        <TableRow>
                          ...
                          >
                            No merchandise found yet. Add your first product to
            ...
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription className="text-xs">
                    Higher quality = better sales velocity
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-2">
                  {Object.entries(QUALITY_TIERS).map(([tier, info]) => {
                    const count = Array.isArray(merchandise)
                      ? merchandise.filter(
                          (p: any) => (p.quality_tier || "basic") === tier,
                        ).length
                      : 0;
                    return (
                      <div
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Where current stock and revenue potential sit.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  {categoryMetrics.length === 0 && (
                    <p className="text-sm text-muted-foreground">
                      Add merchandise to see category insights.
                    </p>
                  )}
                  {categoryMetrics.map((category) => (
                    ...
                    >
                      <div className="flex items-center justify-between text-sm font-semibold">
                        ...
                        <span>
                          {numberFormatter.format(category.skus)} SKU(s)
                        </span>
                      </div>
                      <p className="text-xs text-muted-foreground">
                        {numberFormatter.format(category.units)} units ·{" "}
                        {currencyFormatter.format(category.revenue)} potential
                      </p>
                    </div>
                  ))}
                </CardContent>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Low inventory or sold out items prioritized by urgency.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  {restockAlerts.length === 0 && (
                    <p className="text-sm text-muted-foreground">
                      All set — no low stock products at the moment.
                    </p>
                  )}
                  {restockAlerts.map((product) => (
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {formatProductType(product.item_type)} ·{" "}
                          {statusLabels[getStatus(product.stock_quantity)]} ·{" "}
                          {numberFormatter.format(
                            safeNumber(product.stock_quantity),
                          )}{" "}
                          units
        ...
        <TabsContent value="sales" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Track merchandise revenue, top products, and sales performance
                across channels.
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="add-product">
          <MerchCatalog
            ...
            onAddProduct={(item, designName, price, stock) => {
              catalogAddProductMutation.mutate({
                item,
                designName,
                price,
                stock,
        ...
        <TabsContent value="manage-product">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Restock, reprice, or retire by performance.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="rounded-lg border border-dashed bg-muted/30 p-4 text-sm text-muted-foreground">
                Updating merchandise here syncs with gig outcome calculations
                and future sales simulations.
              </div>
              <div className="space-y-2">
                <label
                  className="text-sm font-medium"
                  htmlFor="manage-product-select"
                >
                  Select Product
                </label>
                <Select
              ...
              {selectedProduct ? (
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        {findBlueprintCategory(editForm.category)?.items.map(
                          (item) => (
                  ...
                  <div className="space-y-2">
                    ...
                    {(() => {
                      ...
                      const qualityTier = ((selectedProduct as any)
                        ?.quality_tier || "basic") as any;
                      const rec = getRecommendedPrice(cost, qualityTier);
                      const impact = getPricingImpact(
                      ...
                      return (
                        <div className="text-xs space-y-1">
                          ...
                          <Badge
                            variant="outline"
                            className={`text-xs ${impact.color}`}
                          >
                            {impact.label} — {impact.salesMultiplier}x sales
                            {impact.fameEffect !== 0 &&
                              ` · ${impact.fameEffect > 0 ? "+" : ""}${impact.fameEffect} fame/day`}
                            {impact.fanEffect !== 0 &&
                              ` · ${impact.fanEffect > 0 ? "+" : ""}${impact.fanEffect} fans/day`}
                          </Badge>
                        </div>
                      );
                    })()}
                ...
                <p className="text-sm text-muted-foreground">
                  Select a product from the list above to update pricing, stock,
                  or retire the item.
                </p>
              )}
            </CardContent>
