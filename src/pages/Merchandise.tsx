...
const finishLabels: Record<DesignerForm["finish"], string> = {
 ...
 premium:"Premium heavyweight","limited-edition":"Limited edition drop",
};
...
function RestockPopover({ productId, productName, currentStock, costPerUnit, bandId }: {
 ...
 <Button
 size="sm"className="w-full"onClick={() => restockMutation.mutate()}
...
const Merchandise = () => {
 ...
 finish ==="limited-edition"?"Use numbered heat-press tags and metallic ink for limited drop feel.": finish ==="premium"?"Opt for heavyweight blanks and puff ink for tactile depth.":"Use soft-wash blanks with eco inks for fast turnaround.",
 story
 ...
 return (
 <FMPageScaffold
 title="Merchandise Operations"subtitle={`Manage ${bandName}'s merch catalogue, monitor sell-through, and prep your next drop without leaving the control room.`}
 backTo="/hub/career-business"backLabel="Back to Career & Business"icon={Shirt}
 >
 ...
 {isAdmin() && (
 <Button
 size="sm"variant="outline"disabled={isGeneratingAll || !merchandise?.length}
 onClick={() => {
 ...
 <div className="rounded-lg border border-dashed bg-muted/30 p-4">
 <p className="text-xs text-muted-foreground">Inventory Cost Basis</p>
 ...
 className="h-10 w-10 rounded object-cover border border-border cursor-pointer hover:opacity-80 transition-opacity"onClick={(e) => {
 e.stopPropagation();
 ...
 className="h-10 w-10 rounded border border-dashed border-muted-foreground/30 flex items-center justify-center hover:bg-muted/50 transition-colors"title="Generate AI image">
 ...
 <Button
 size="sm"variant="outline"onClick={() => setSelectedProductId(product.id)}
 >
 Review
 </Button>
 </div>
 ...
 <Input
 id="edit-design-name"value={editForm.designName}
 onChange={(event) =>
 ...
 <Input
 id="edit-cost"type="number"min={0}
 step={1}
 value={editForm.cost}
 readOnly
 required
 ...
 <Input
 id="edit-price"type="number"min={0}
 step={1}
 ...
 <Input
 id="edit-stock"type="number"min={0}
 step={1}
 ...
 <Button
 type="button"variant="destructive"className="w-full sm:w-auto"onClick={() => deleteProductMutation.mutate()}
