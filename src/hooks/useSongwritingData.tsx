...
export const useSongwritingData = (userId?: string | null) => {
  ...
  const songwritingProjectExtendedMetadataSupportedRef = useRef(true);
  const songwritingProjectAttributeScoresSupportedRef = useRef(true);
  const songwritingProjectRatingFlagsSupportedRef = useRef(true);
  const songwritingSessionExtendedTrackingSupportedRef = useRef(true);
      ...
      const buildSelect = (
        ...
        if (includeSessions) {
          const sessionFields = [
            ...
            "notes",
            "created_at",
            // Extended session tracking features disabled for database compatibility
          ].filter((field): field is string => typeof field === "string" && field.length > 0);
  ...
  const createProject = useMutation({
    mutationFn: async (projectData: CreateProjectInput) => {
      ...
      // Extended metadata fallback handling removed for database compatibility
  ...
  const updateProject = useMutation({
    mutationFn: async ({ id, ...updates }: UpdateProjectInput) => {
      ...
      // Extended metadata processing removed for database compatibility
      ...
      const buildPayload = (includeEstimated: boolean) => {
        ...
        (Object.entries(updates) as Array<[keyof UpdateProjectInput, unknown]>).forEach(([key, value]) => {
          ...
          // Extended metadata support - columns exist but handling logic pending
          // Purpose, mode, co-writers, production potential, attribute scores
