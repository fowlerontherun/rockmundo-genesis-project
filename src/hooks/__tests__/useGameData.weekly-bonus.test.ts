...
import type { XpLedgerEntry } from "../useGameData";
...
const createLedgerEntry = (
  overrides: Partial<XpLedgerEntry> = {}
): XpLedgerEntry => ({
  id: "ledger-1",
  profile_id: "profile-1",
  event_type: WEEKLY_BONUS_REASON,
  xp_delta: 250,
  balance_after: 1_250,
  attribute_points_delta: 0,
...
describe("evaluateWeeklyBonusState", () => {
  it("marks weekly bonus as fresh when acknowledgement is missing or stale", () => {
    const entry = createLedgerEntry({ created_at: "2024-11-01T15:30:00.000Z" });
  ...
  it("suppresses alerts when acknowledgement is current", () => {
    ...
    const entry = createLedgerEntry({ created_at: recordedAt });
  ...
  it("persists a synthetic acknowledgement when ledger lacks a timestamp", () => {
    const entry = createLedgerEntry({ created_at: "" });
...
describe("resolveWeeklyBonusAcknowledgementTimestamp", () => {
  ...
  it("falls back to the provided clock when the ledger timestamp is invalid", () => {
    ...
    const entry = createLedgerEntry({ created_at: "not-a-date" });
