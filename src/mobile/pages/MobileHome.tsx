...
function MobileDay({
  ...
  useEffect(() => {
    if (!skillSlug && skillOptions[0]?.[0]) setSkillSlug(skillOptions[0][0]);
    ...
    <div className="space-y-4">
      ...
      <MobileSectionCard title="Quick tasks" subtitle="Tasks suitable for short mobile sessions.">
        <div className="grid grid-cols-2 gap-2">
          <Button variant="outline" onClick={() => navigate("/mobile/me/wellness")}>Recover now</Button>
          <Button variant="outline" onClick={() => navigate("/mobile/world/travel")}>Check travel</Button>
          <Button variant="outline" onClick={() => navigate("/mobile/social/messages")}>Messages</Button>
          <Button variant="outline" onClick={() => navigate("/mobile/social/twaater")}>Twaater</Button>
        </div>
      </MobileSectionCard>

      <MobileSectionCard title="Recent outcomes" subtitle="Latest activity outcomes available to the current character.">
        {recentOutcomes.length ? (
          <div className="space-y-2">
            {recentOutcomes.map((outcome: any, index: number) => (
              <MobileEntityCard
                key={outcome.id ?? `${outcome.activity_type}-${index}`}
                title={outcome.message ?? humanise(outcome.activity_type ?? "Recent activity")}
                subtitle={outcome.created_at ? new Date(outcome.created_at).toLocaleString() : (outcome.status ?? "Recent")}
                icon={<Zap className="h-5 w-5" />}
                meta={<MobileStatusBadge tone={outcome.status === "completed" || outcome.status === "success" ? "success" : "neutral"}>{outcome.status ?? "Update"}</MobileStatusBadge>}
              />
            ))}
          </div>
